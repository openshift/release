...
GitHub token already mounted by the manifest ref. `BASH_ENV` runs that bootstrap
...
the mounted token through a temporary askpass helper which is removed after
the fetch. The CI identity still needs read access to the private PIXAA repo;
