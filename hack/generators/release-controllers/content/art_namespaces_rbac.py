    ...
    resources.append({
        ...
        'metadata': {
            'annotations': {} if not context.private else {
                f'serviceaccounts.openshift.io/oauth-redirectreference.{context.rc_api_serviceaccount_name}': '{"kind":"OAuthRedirectReference","apiVersion":"v1","reference":{"kind":"Route","name":"%s"}}' % context.rc_api_route_name
            },
            'name': context.rc_serviceaccount_name,
            'namespace': context.config.rc_deployment_namespace,
        }
