...
FIXTURE = Path(__file__).parent / "fixtures" / f"{PHASE0_NAME}.in"
MASTER_NAME = "quay-quay-master.yaml"
MASTER_FIXTURE = Path(__file__).parent / "fixtures" / f"{MASTER_NAME}.in"
MIXED_DIR = Path(__file__).parent / "fixtures" / "mixed"
ARM64_CANARY_NAME = "quay-quay-redhat-3.18__aws-arm64-ocp422-e2e-install.yaml"
ARM64_CANARY_FIXTURE = Path(__file__).parent / "fixtures" / f"{ARM64_CANARY_NAME}.in"
...
def test_expand_matrix_cells() -> None:
    matrix = yaml.safe_load((GENERATOR_DIR / "matrix.yaml.in").read_text())
...
def test_jinja_renders_ocp_version() -> None:
    ...
    rendered = render_template(env, "base.yaml.j2", _phase0_cell().context())
...
def test_mixed_golden_groups_periodic_and_presubmit_into_one_file() -> None:
    results, _retired = generate_all(
        matrix_path=MIXED_DIR / "matrix.yaml.in", templates_dir=MIXED_DIR / "templates"
    )
    assert len(results) == 1
    ...
    dumped = dump_config(config)
    assert dumped == (MIXED_DIR / "expected.yaml.in").read_text()
...
def test_missing_cloud_arch_overlay_raises() -> None:
    ...
    with pytest.raises(ValueError, match="failed to render template clouds/gcp-arm64.yaml.j2"):
        build_config(cell, GENERATOR_DIR / "templates")
...
MASTER_ARM64_FIXTURE = Path(__file__).parent / "fixtures" / f"{MASTER_ARM64_NAME}.in"
