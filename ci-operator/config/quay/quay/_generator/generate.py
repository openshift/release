...
#   templates/base.yaml.j2 (or templates/presubmit/base.yaml.j2 for kind: presubmit)
#   templates/sources/<source>.yaml.j2 (kind: periodic only)
#   templates/clouds/<cloud>.yaml.j2
...
"""Generate Quay ci-operator configs from matrix.yaml.in."""
...
#
# It is generated by ci-operator/config/quay/quay/_generator/generate.py.
# Changes here will be overwritten the next time the generator runs, and
# may be stripped by `make ci-operator-config` (determinize-ci-operator).
#
# Edit these instead:
#   - matrix.yaml.in (releases, clouds, tiers, source, branch/job env, as)
...
def build_config(cell: Cell, templates_dir: Path) -> YamlMap:
    ...
    cloud_template = f"{template_root}clouds/{cell.cloud}.yaml.j2"
    if template_root and not (templates_dir / cloud_template).exists():
        ...
        cloud_template = f"clouds/{cell.cloud}.yaml.j2"
    layers = [render_template(env, f"{template_root}base.yaml.j2", context)]
    if cell.kind == "periodic":
        layers.append(render_template(env, f"sources/{cell.source}.yaml.j2", context))
    layers.append(render_template(env, cloud_template, context))
    if cell.arch != "amd64":
        layers.append(render_template(env, f"arches/{cell.arch}.yaml.j2", context))
        layers.append(render_template(env, f"clouds/{cell.cloud}-{cell.arch}.yaml.j2", context))
    layers.append(render_template(env, f"tests/{cell.test}.yaml.j2", context))
    presubmit_test_template = f"{template_root}tests/{cell.test}.yaml.j2"
...
def generate_all(
    ...
    matrix_path = matrix_path or generator_dir / "matrix.yaml.in"
    templates_dir = templates_dir or generator_dir / "templates"
    matrix = load_yaml(matrix_path)
    if not isinstance(matrix, dict):
        raise ValueError("matrix.yaml.in must be a YAML mapping")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate Quay ci-operator configs from matrix.yaml.in"
    )
