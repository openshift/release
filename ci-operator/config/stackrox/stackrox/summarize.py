...
"""
The script reads ci-operator yaml config files from the current directory and outputs an overview table for visual
controlling the configuration between multiple files.
"""
...
class JobEntry:
    ...
    def dump_all_interesting_attrs(self):
        ...
        subset = {
            ...
            ["always_run", "run_if_changed", "skip_if_only_changed", "skip_report", "max_concurrency"] if
            key in self.raw_entry
        }
...
class Data:
    ...
    def jobs_dir(self):
        ...
        dir = dir.parent.parent.parent  # Dir will be at repo_root/ci-operator
...
def safe_get_attr(raw_data, attr_key):
    ...
    `attr` can be dot-separated, in which case each part serves as a key for the corresponding level of nested maps.
...
def load_and_massage_data(dir):
    ...
    d.configs = load_raw_yamls(d.config_dir, ConfigFile)
    ...
    d.jobs_files = load_raw_yamls(d.jobs_dir, JobsFile)
...
def assign_unordered_tests(configs):
    for c in configs:
        c.unordered_entries = []
        for test in safe_get_attr(c.raw_parsed_yaml, "tests") or []:
            te = TestEntry(raw_entry=test)
            if te.name:
...
def assign_unordered_jobs(jobs_files):
    for j in jobs_files:
        ...
        if len(presubmits_dict) > 1:
            j.load_error = "Too many items under presubmits"
            continue
        j.unordered_entries = [JobEntry(raw_entry=e) for e in list(presubmits_dict.values())[0]]


def extract_and_sort_unique_names(items):
    """
    Collects name for each entry returned by entriesFn for each item in items.
...
def render_load_error(error):
    ...
    return render_collapsible(btn_text, html.escape(error.__str__()))
...
def render_test_entry(entry):
    ...
    return render_collapsible(btn_text, html.escape(yaml_dump))
...
def render_job_entry(entry):
    ...
    return render_collapsible(btn_text, html.escape(entry.dump_all_interesting_attrs()))


def render_collapsible(button_text, contents):
    # This ensures each collapsible has a unique id.
    ...
    return f"""
        <button
            ...
            title="Click to toggle showing contents"
...
<html lang="en">
  <head>
    ...
    <style>
        ...
        .collapsing {
            /* Set animation duration to zero otherwise collapsing collapsible is inconvenient. */
...
def render_job_summary_table(all_job_names, jobs_files):
    pass
