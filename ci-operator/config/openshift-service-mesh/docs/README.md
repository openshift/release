...
*Note: As job definition YAML files are automatically regenerated and comments are removed, this section captures important maintenance notes for our OpenShift CI jobs.*

### OCP Installation Methods

#### HyperShift Installation
Our preferred installation method for faster deployment and cost savings.

**Advantages:**
- Faster installation time compared to IPI
- Cost-effective cluster provisioning
- Includes credentials for `quay-proxy.ci.openshift.org/openshift`

**Limitations:**
- **No ARM64 support yet** (Platform team working on this)
- **No pre-GA version support** (cannot install pre-release OpenShift versions)
...
```
**Works** - HyperShift includes necessary credentials for quay-proxy access.

**Step Registry Reference:** `servicemesh-istio-e2e-hypershift`

#### IPI Installation
Traditional installation method used for specific scenarios.

**When to Use:**
- ARM64 clusters (required until HyperShift supports ARM64)
- Pre-GA OpenShift versions
- Specific testing scenarios requiring full cluster control

**Important Limitation:**
The `ipi-install` step always uses the installer from `release:latest`, even when `OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE` and `RELEASE_IMAGE_LATEST` are set. This means **one YAML file per OCP version is required** to avoid installer version mismatches.

**Builder Image Configuration:**
```yaml
# FAILS - No quay-proxy credentials in IPI
MAISTRA_BUILDER_IMAGE: quay-proxy.ci.openshift.org/openshift/ci:ci_maistra-builder_upstream-1.27

# WORKS - Use upstream image instead
MAISTRA_BUILDER_IMAGE: gcr.io/istio-testing/build-tools:release-1.27-56a42ed008b9070d88166dbec82e798d9eca43de
```

**Error when using quay-proxy with IPI:**
```
Failed to pull image "quay-proxy.ci.openshift.org/openshift/ci:ci_maistra-builder_upstream-1.27":
initializing source docker://quay-proxy.ci.openshift.org/openshift/ci:ci_maistra-builder_upstream-1.27:
unable to retrieve auth token: invalid username/password: authentication required
```

### Docker in OpenShift

Our jobs can run Docker commands within OpenShift clusters by:

1. **Privileged Pod Creation**: Jobs create privileged pods with necessary permissions
2. **Entrypoint Command**: Use the `entrypoint` command available in the `build-tools` image
3. **Remote Execution Pattern**: Execute Docker commands via `oc rsh` into the privileged pod

**Example Pattern:**
```bash
oc rsh -n "${MAISTRA_NAMESPACE}" "${MAISTRA_SC_POD}" \
  bash -c "export BUILD_WITH_CONTAINER=0 && entrypoint <docker-command>"
```

### Image Registry Strategy

**Recommended Approach:**
- **HyperShift jobs**: Use `quay-proxy.ci.openshift.org/openshift/ci:ci_maistra-builder_*` images
- **IPI jobs**: Use upstream `gcr.io/istio-testing/build-tools:*` images
- **Version alignment**: Ensure builder image version matches the target service mesh version
