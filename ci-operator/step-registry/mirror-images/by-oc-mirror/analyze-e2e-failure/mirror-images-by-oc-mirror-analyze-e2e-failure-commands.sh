...
# ---------------------------------------------------------------------------
# Redact sensitive information before it ever reaches CI logs/artifacts
# ---------------------------------------------------------------------------
# Claude has Bash/Read/WebFetch access and reads build-log/gather/must-gather
# artifacts that may themselves contain leaked credentials/tokens. Never
# stream its raw tool output to stdout (Prow's build-log.txt is public);
# only a redacted report/transcript is ever left in ARTIFACT_DIR.
redact_secrets() {
  sed -E \
...
# and agentic-ci log (which can contain full tool-call inputs/outputs from
# artifacts it read, e.g. build-log/gather/must-gather), in place before any
# of them are left in ARTIFACT_DIR for GCS upload.
for f in "${ARTIFACT_DIR}/failure-analysis.md" "${ARTIFACT_DIR}/claude-failure-analysis.json" "${ARTIFACT_DIR}/claude-agentic-ci.log"; do
  if [[ -f "${f}" ]]; then
