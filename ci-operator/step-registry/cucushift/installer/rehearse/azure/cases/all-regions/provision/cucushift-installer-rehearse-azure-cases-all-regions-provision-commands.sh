...
function post_step_actions()
{
  ...
  echo "--- RESULTS ---"
  echo -e "region\tcluster_name\tinfra_id\tinstall\thealth_check\tdestroy_result"
...
function generate_regions()
{
  if [[ ${CUSTOM_REGION_LIST} == "" ]]; then
    ...
    az provider show -n Microsoft.Resources -ojson | jq -r '.resourceTypes[] | select(.resourceType=="resourceGroups") | .locations[]' | sort > ${ARTIFACT_DIR}/all_regions_with_display_name.txt
    while read -r region_display_name; do
        az account list-locations --query "[?displayName == '${region_display_name}'].name" -otsv >> ${ARTIFACT_DIR}/all_regions.txt
    done <  ${ARTIFACT_DIR}/all_regions_with_display_name.txt

    cp ${ARTIFACT_DIR}/all_regions.txt ${REGIONS_LIST}
    ...
    if [[ ${SPILT_REGIONS} != "" ]]; then
      echo "Spliting regions into ${REGION_SPILT_NUMBER} parts ..."
      pushd /tmp/
      total_region_count=$(cat $REGIONS_LIST | wc -l)
      each_part_count=$((${total_region_count}/${REGION_SPILT_NUMBER}+1))
...
EOF
  ...
  if [[ ${COMPUTE_NODE_TYPE} != "" ]]; then
    yq-v4 eval -i '.compute[0].platform.azure.type = env(COMPUTE_NODE_TYPE)' "${config}"
  fi
...
# Create cluster
# -------------------------------------------------------------------------------------
arm_region_lists=$(az vm list-skus --size Standard_D4ps_v5 --query '[].locationInfo[].location' -otsv)
...
while IFS= read -r region; do
    ...
    if ! echo "${arm_region_lists}" | grep -qi "^${region}$"; then
        echo "Region $region does not support arm64 architecture, install with amd64 arch!"
        OCP_ARCH="amd64"
    fi
