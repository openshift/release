...
function compose_expected_err_msg()
{
  ...
  local instance_type on_host_maintenance supported_cc_types unsupported_cc_types cc_type1 cc_type2

  instance_type=$(yq-go r "${install_config_file}" compute[0].platform.gcp.type)
  if [[ -z "${instance_type}" ]]; then
    instance_type=$(yq-go r "${install_config_file}" platform.gcp.defaultMachinePlatform.type)
  fi

  on_host_maintenance=$(yq-go r "${install_config_file}" compute[0].platform.gcp.onHostMaintenance)
  if [[ -z "${on_host_maintenance}" ]]; then
    on_host_maintenance=$(yq-go r "${install_config_file}" platform.gcp.defaultMachinePlatform.onHostMaintenance)
  fi
