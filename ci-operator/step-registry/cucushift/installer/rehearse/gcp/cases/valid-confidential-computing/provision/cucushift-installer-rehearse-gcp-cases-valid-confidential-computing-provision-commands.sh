...
function compose_expected_err_msg()
{
  ...
  local supported_cc_types unsupported_cc_types
...
function create_manifests()
{
  ...
  local cluster_name install_dir compute_cc_type control_plane_cc_type default_machine_platform_cc_type result
...
EOF
  ...
  result=0
  echo "$(date -u --rfc-3339=seconds) - INFO 'openshift-install create manifests --dir ${install_dir}'"
