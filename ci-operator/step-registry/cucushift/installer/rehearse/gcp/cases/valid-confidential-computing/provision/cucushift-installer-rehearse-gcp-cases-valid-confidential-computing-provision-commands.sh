...
function compose_expected_err_msg()
{
  ...
  local -r default_machine_platform_cc_type="$4"
  local -r use_default_flag="$5"
  local -r on_host_maintenance="$6"
  local supported_cc_types unsupported_cc_types cc_type1 cc_type2
  ...
  if ${use_default_flag}; then
    cc_type1="${default_machine_platform_cc_type}"
    cc_type2="${default_machine_platform_cc_type}"
  else
    cc_type1="${control_plane_cc_type}"
    cc_type2="${compute_cc_type}"
  fi

  expected_err_msg=""
  if [[ "${on_host_maintenance}" != "Terminate" ]]; then
    expected_err_msg="controlPlane.platform.gcp.onHostMaintenance: Invalid value: \"${on_host_maintenance}\": onHostMaintenace must be set to Terminate when confidentialCompute is ${cc_type1}"
  fi

  set +o errexit
  if echo "${unsupported_cc_types}" | grep -qw "${cc_type1}"; then
    if [[ "${expected_err_msg}" != "" ]]; then
      expected_err_msg="${expected_err_msg}, "
    fi
    expected_err_msg="${expected_err_msg}controlPlane.platform.gcp.type: Invalid value: \"${instance_type}\": Machine type do not support ${cc_type1}. Machine types supporting ${cc_type1}: ${cc_type_machine_series_dict[${cc_type1}]}"
  fi

  if [[ "${on_host_maintenance}" != "Terminate" ]]; then
    expected_err_msg="${expected_err_msg}, compute[0].platform.gcp.onHostMaintenance: Invalid value: \"${on_host_maintenance}\": onHostMaintenace must be set to Terminate when confidentialCompute is ${cc_type2}"
  fi

  if echo "${unsupported_cc_types}" | grep -qw "${cc_type2}"; then
    if [[ "${expected_err_msg}" != "" ]]; then
      expected_err_msg="${expected_err_msg}, "
    fi
    expected_err_msg="${expected_err_msg}compute[0].platform.gcp.type: Invalid value: \"${instance_type}\": Machine type do not support ${cc_type2}. Machine types supporting ${cc_type2}: ${cc_type_machine_series_dict[${cc_type2}]}"
  fi
  set -o errexit
}
...
function create_manifests()
{
  ...
  local cluster_name install_dir compute_cc_type control_plane_cc_type default_machine_platform_cc_type result tmp_output use_default_flag rand_int
...
controlPlane:
  ...
  platform:
    gcp:
      ...
      onHostMaintenance: ${on_host_maintenance}
EOF
    if [[ $rand_int -gt 80 ]]; then
      echo "$(date -u --rfc-3339=seconds) - Also configure under 'platform.gcp.defaultMachinePlatform'..."
      cat >> "${patch}" << EOF
platform:
  gcp:
...
EOF
  ...
  if ! "${success_flag}"; then
    compose_expected_err_msg "${instance_type}" "${compute_cc_type}" "${control_plane_cc_type}" "${default_machine_platform_cc_type}" "${use_default_flag}" "${on_host_maintenance}"    
