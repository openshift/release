...
do
  ...
  if [[ "${unsupported_osdisk_types}" != "" ]]; then
    ...
    do
      echo "$(date -u --rfc-3339=seconds) - Scenario ${num}: Testing with instance type '${instance_type}' + unsupported OS disk type '${osdisk_type}'..."
  ...
  if [[ "${supported_osdisk_types}" != "" ]]; then
    ...
    for osdisk_type in "${osdisk_types_array[@]}"
    do
      echo "$(date -u --rfc-3339=seconds) - Scenario ${num}: Testing with instance type '${instance_type}' + supported OS disk type '${osdisk_type}'..."
