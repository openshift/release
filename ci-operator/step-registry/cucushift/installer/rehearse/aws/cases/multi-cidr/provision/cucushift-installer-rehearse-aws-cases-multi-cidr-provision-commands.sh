...
create_install_config $cluster_name1 ${subnet_file1} "10.134.0.0/16" $install_dir1
create_install_config $cluster_name2 ${subnet_file2} "10.190.0.0/16" $install_dir2
...
# Cluster 1 MCS internal traffic from cluster, expect 10.134.0.0/16
...
if [ ${cidr_ip1} != "10.134.0.0/16" ]; then
  echo "Error: cluster 1 apiserver-lb sg: except 10.134.0.0/16 for port 22623, but got ${cidr_ip1}"
  ret=$((ret+1))
else
  echo "Pass: cluster 1 apiserver-lb sg: 10.134.0.0/16 for port 22623"
fi


# Cluster 1 MCS internal traffic from cluster, expect 10.190.0.0/16
...
if [ ${cidr_ip2} != "10.190.0.0/16" ]; then
  echo "Error: cluster 2 apiserver-lb sg: except 10.190.0.0/16 for port 22623, but got ${cidr_ip2}"
  ret=$((ret+1))
else
  echo "Pass: cluster 2 apiserver-lb sg: 10.190.0.0/16 for port 22623"
fi
