...
function post_actions() {
  ...
  # Copying install logs
  pushd "$INSTALL_DIR"
  cp metadata.json "${SHARED_DIR}"/
  cp log-bundle-*.tar.gz "${ARTIFACT_DIR}"/
  cp .openshift_install.log "${ARTIFACT_DIR}"/
  sed -i 's/password: .*/password: REDACTED/; s/X-Auth-Token.*/X-Auth-Token REDACTED/; s/UserData:.*,/UserData: REDACTED,/;' "${ARTIFACT_DIR}"/.openshift_install.log
  set -o errexit
  popd
