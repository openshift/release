...
v414=" ${v413} MachineAPI Build DeploymentConfig"
latest_defined="v414"
...
for cap in $enabled_capability_set; do
    if [[ "${cap}" == "Build" ]] || [[ "${cap}" == "DeploymentConfig" ]]; then
        ...
        [[ ! $(oc get ${resource} -A) ]] && echo "ERROR: capability ${cap}: resources ${resource} -- not found!" && check_result=1
        continue
    fi
...
for cap in $disabled_capability_set; do
    if [[ "${cap}" == "Build" ]] || [[ "${cap}" == "DeploymentConfig" ]]; then
        resource="${caps_resource[$cap]}"
        [[ $(oc get ${resource} -A) ]] && echo "ERROR: capability ${cap}: resources ${resource} -- found!" && check_result=1
        continue
    fi
    for op in ${caps_operator[$cap]}; do
        if [[ `grep -e "^${op} " ${co_content}` ]]; then
