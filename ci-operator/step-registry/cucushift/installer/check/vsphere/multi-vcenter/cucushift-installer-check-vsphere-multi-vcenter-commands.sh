...
declare vsphere_url
declare vsphere_user
declare vsphere_password
...
function check_multi_vcenter() {
	...
	for vcenter in $(jq -c '.vcenters[]' "${VSPHERE_INFO}")
        do
          set -x
	  vsphere_url=$(echo $vcenter | jq '.vcenter')
	  export GOVC_URL="${vsphere_url}"
	  set +x
	  vsphere_user=$(echo $vcenter | jq '.user')
	  export GOVC_USERNAME="${vsphere_user}"
	  vsphere_password=$(echo $vcenter | jq '.pass')
	  export GOVC_PASSWORD="${vsphere_password}"
	  set -x
          if [[ "${GOVC_URL}" == "${fd_server}" ]];then		  
