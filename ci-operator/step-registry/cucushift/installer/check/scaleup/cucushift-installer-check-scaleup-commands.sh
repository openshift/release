...
# Running command oc wait --all=true machine.machine.openshift.io -n openshift-machine-api --for=jsonpath='{.status.phase}'=Running --timeout=10m 
# alway get timeout issue, so waiting for 2 mins to make sure status is up to date
