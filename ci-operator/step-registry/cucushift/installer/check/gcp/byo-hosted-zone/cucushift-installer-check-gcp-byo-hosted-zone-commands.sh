...
if [[ "${PRE_CREATE_PRIVATE_ZONE}" == "yes" ]] && [[ "${zone_description}" != "Created By OpenShift Installer" ]]; then
  echo "$(date -u --rfc-3339=seconds) - A pre-create DNS private zone is in use."
elif [[ "${PRE_CREATE_PRIVATE_ZONE}" == "no" ]] && [[ "${zone_description}" == "Created By OpenShift Installer" ]]; then
  echo "$(date -u --rfc-3339=seconds) - Installer provisioned DNS private zone is in use."
else
  echo "$(date -u --rfc-3339=seconds) - Something went wrong, PRE_CREATE_PRIVATE_ZONE '${PRE_CREATE_PRIVATE_ZONE}' and zone description '${zone_description}'."
  ret=1
fi
