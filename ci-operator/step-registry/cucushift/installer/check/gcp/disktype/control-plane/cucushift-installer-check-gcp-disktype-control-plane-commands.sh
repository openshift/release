...
if [ ! -f "${SHARED_DIR}/control-plane-osdisk-disktype" ]; then
...
expected_disk_type=$(cat "${SHARED_DIR}/control-plane-osdisk-disktype")
...
if [[ ${#disks[@]} == 0 ]]; then
  echo "$(date -u --rfc-3339=seconds) - Zero control-plane node found."
  exit ${ret}
fi
