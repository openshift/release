...
function check_multi_disks() {
    data="{
        \"filter\":\"vm_name==$node\"
    }"
    node_json=$(curl -ks -u "${un}":"${pw}" -X POST "${api_ep}" -H "Content-Type: application/json" -d @- <<<"${data}")
    disk_adapter_type=$(echo "${node_json}" | jq -r ".entities[].status.resources.disk_list[] | select(.disk_size_mib==$(echo "$DATA_DISK_SIZE" | tr -d "Mi")) | .device_properties.disk_address.adapter_type")

    if [[ $disk_adapter_type == "$DATA_DISK_ADAPTER_TYPE" ]]; then
        echo "Pass: passed to check node multi_disks"
    else
        echo "Fail: failed to check node multi_disks"
