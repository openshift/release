...
identity_type_master=$(yq-go r ${INSTALL_CONFIG} 'controlPlane.platform.azure.identity.type')
identity_type_worker=$(yq-go r ${INSTALL_CONFIG} 'compute[0].platform.azure.identity.type')

expected_identity_type_master=""
expected_identity_type_worker=""
...
if [[ -n "${identity_type_default}" ]]; then
    expected_identity_type_master="${identity_type_default}"
    expected_identity_type_worker="${identity_type_default}"
    if [[ "${identity_type_default}" == "UserAssigned" ]] && [[ ${AZURE_USER_ASSIGNED_IDENTITY_NUMBER} -gt 0 ]]; then
...
if [[ -n "${identity_type_master}" ]]; then
    expected_identity_type_master="${identity_type_master}"
    if [[ "${identity_type_master}" == "UserAssigned" ]] && [[ ${AZURE_USER_ASSIGNED_IDENTITY_NUMBER} -gt 0 ]]; then
        expected_identity_id_master=""
        for num in $(seq 0 ${end_number}); do
...
if [[ -n "${identity_type_worker}" ]]; then
    expected_identity_type_worker="${identity_type_worker}"
    if [[ "${identity_type_worker}" == "UserAssigned" ]] && [[ ${AZURE_USER_ASSIGNED_IDENTITY_NUMBER} -gt 0 ]]; then
        expected_identity_id_worker=""
        for num in $(seq 0 ${end_number}); do
...
if [[ "${expected_identity_type_master}" == "UserAssigned" ]] && [[ -z "${expected_identity_id_master}" ]]; then
    echo "control plane's identity type is UserAssigned, but expected identity ids are empty, please check..."
    exit 1
fi

if [[ "${expected_identity_type_worker}" == "UserAssigned" ]] && [[ -z "${expected_identity_id_worker}" ]]; then
    echo "compute's identity type is UserAssigned, but expected identity ids are empty, please check..."
    exit 1
fi

if ([[ -z "${expected_identity_type_master}" ]] || [[ -z "${expected_identity_type_worker}" ]]) && [[ -z "${cluster_identity_id}" ]]; then
    echo "No identity is specified on master or worker, installer should create an identity by default, but not found it, exit..."
    exit 1
fi

if [[ "${expected_identity_type_master}" == "None" ]] && [[ "${expected_identity_id_worker}" == "None" ]]; then
    if [[ -n "${cluster_identity_id}" ]]; then
        echo "Identity type for both control plane and compute is set to None, installer does not create any identity, but found it, exit..."
        exit 1
    else
        echo "Identity type is set to None for both control plane and compute, no need to check identity, skip..."
        exit 0
    fi
fi
