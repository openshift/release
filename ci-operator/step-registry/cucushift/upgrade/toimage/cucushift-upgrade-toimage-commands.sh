...
function check_failed_operator(){
    ...
    if [[ "${latest_ver_in_history}" != "${TARGET_VERSION}" ]]; then
        ...
        if [[ ${failing_status} == "True" ]]; then
            failing_operator=$(oc get clusterversion version -ojson|jq -r '.status.conditions[]|select(.type == "Failing").message'|grep -oP 'operator \K.*?(?= is)')
            failing_operators=$(oc get clusterversion version -ojson|jq -r '.status.conditions[]|select(.type == "Failing").message'|grep -oP 'operators \K.*?(?= are)'|tr -d ',')
        ...
        if [[ -n "${failing_operators}" ]]; then
            UPGRADE_FAILURE_TYPE="${failing_operators}"
        fi
    fi
}
...
function cco_annotation(){
    ...
    if (( wait_time_loop_var >= 5 )); then
        ...
        UPGRADE_FAILURE_TYPE="cloud-credential"
        return 1
    fi
}
...
function admin_ack() {
    ...
    if (( wait_time_loop_var >= 5 )); then
        ...
        UPGRADE_FAILURE_TYPE="admin_ack"
        return 1
    fi
}
