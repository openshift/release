...
function extract_oc(){
    ...
    while ! (no_proxy='*' ; oc adm release extract -a "${CLUSTER_PROFILE_DIR}/pull-secret" --command=oc --to=${tmp_oc} ${TARGET});
    do
...
do
    ...
    # shellcheck disable=SC2034
    TARGET_VERSION="$(no_proxy='*' ; oc adm release info "${TARGET}" --output=json | jq -r '.metadata.version')"
    extract_oc
