...
function mirror_image(){
    ...
    channel_name=`echo ${target_version} |awk -F '.' 'BEGIN{OFS="."} {print $1,$2}'`
      ...
      platform:
        channels:
          - name: stable-\${channel_name}
            minVersion: \${target_version}
            maxVersion: \${target_version}
    END

    cmd="oc mirror -c ${SHARED_DIR}/image_set_config.yaml   docker://${MIRROR_RELEASE_IMAGE_REPO}  --dest-skip-tls "
    run_command "${cmd} | tee ${MIRROR_OUT_FILE}"
    if [[ "${apply_sig_together=}" == "true" ]]; then
       cat ${mirror_output} |tail -n 1  |awk  '{print $NF}' > "${tmp_mirror_output}"
       cp  ${tmp_mirror_output}/release-signatures/*  ${SAVE_SIG_TO_DIR}/
    if
}
...
function update_icsp(){
    ...
    if [[ "${source_release_image_repo}" != "${MIRROR_RELEASE_IMAGE_REPO}" ]] && ! oc get ImageContentSourcePolicy example -oyaml; then
        ...
        cat ${mirror_output} |tail -n 1  |awk  '{print $NF}' > "${tmp_mirror_output}"
        run_command "oc create -f ${tmp_mirror_output}/imageContentSourcePolicy.yaml"
    fi
}
...
if echo $OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE |grep "nightly"; then
