...
check_volumeattributeclass() {
    local response
    response=$(oc get --raw /apis/storage.k8s.io/v1beta1 2>/dev/null)
    if [[ -n "$response" ]]; then
...
if [[ -n "$CHECK_ENABLED_FEATURE_GATES" ]]; then
    log "Checking enabled feature gates..."
    read -ra gates_array <<< "$CHECK_ENABLED_FEATURE_GATES"
