...
function set_kubeconfig() {
    ...
    #Limitation: we always & only select the first hostedcluster to add idp. "
...
function check_idp() {
    ...
    current_idp_config=$(oc get hostedclusters $cluster_name -n clusters -o jsonpath='{range .spec.configuration.oauth.identityProviders[*]}{.name}{" "}{.type}{"\n"}{end}')
...
function configure_openldap() {
    ...
    LDAP_SECRET=$(tr </dev/urandom -dc 'a-z0-9' | fold -w 12 | head -n 1 || true)
...
kind: NetworkPolicy
metadata:
  name: allow-ldap-to-hypershift-ci
...
spec:
  ...
  - from:
    - namespaceSelector:
        matchLabels:
          kubernetes.io/metadata.name: ldap
  egress:
  - to:
    - namespaceSelector:
        matchLabels:
...
EOF
    ...
    # current generation
    gen=$(oc get deployment oauth-openshift -n "clusters-$cluster_name" -o jsonpath='{.metadata.generation}')
            ...
            "oauth":{
                "identityProviders": [{
                    "ldap": {
                        ...
                        "insecure": true,
                        "url": "ldap://ldap.ldap.svc.cluster.local:389/ou=people,ou=rfc2307,dc=example,dc=com?cn"
                    },
                    "mappingMethod": "claim",
                    "name": "ldapidp",
                    "type": "LDAP"
    ...
    # Apply the patch
    if ! oc patch hostedclusters $cluster_name -n "$HYPERSHIFT_NAMESPACE" --type=merge -p "$new_ldap_idp"; then
    ...
    wait_auth=true
    expected_replicas=$(oc get deployment oauth-openshift -n "clusters-$cluster_name" -o jsonpath='{.spec.replicas}')
    while $wait_auth; do
        available_replicas=$(oc get deployment oauth-openshift -n "clusters-$cluster_name" -o jsonpath='{.status.availableReplicas}')
        new_gen=$(oc get deployment oauth-openshift -n "clusters-$cluster_name" -o jsonpath='{.metadata.generation}')
