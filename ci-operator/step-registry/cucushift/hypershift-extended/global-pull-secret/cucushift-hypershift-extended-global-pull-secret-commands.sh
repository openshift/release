...
retry_until_success() {
    ...
    for i in $(seq 1 "$retries"); do
        echo "Attempt $i/$retries: running $cmd"
        if bash -c "$cmd"; then
    ...
    echo "$cmd did not succeed after $retries attempts"
    return 1
}
...
oc get namespace "$DS_NAMESPACE" >/dev/null 2>&1 || oc create namespace "${DS_NAMESPACE}"
...
for pod in $pods; do
  ...
  retry_until_success 10 5  "oc get event --ignore-not-found -n ${DS_NAMESPACE} --field-selector involvedObject.name=$pod,involvedObject.kind=Pod -ojsonpath='{range .items[?(@.reason==\"Failed\")]}{.message}{\"\n\"}{end}' 2>&1 | grep -q 'authentication required'"
done
...
oc get secret additional-pull-secret -n kube-system >/dev/null 2>&1 || oc create secret docker-registry additional-pull-secret -n kube-system "--docker-server=$REG_ROUTE" "--docker-username=$REG_USER" "--docker-password=$REG_PASS"

# there is a global-pull-secret in kube-system now, which has the reg auth
retry_until_success 10 5 "oc get --ignore-not-found secret global-pull-secret -n kube-system -o jsonpath='{.metadata.name}' | grep global-pull-secret"

# all pods should work now
for pod in $pods; do
  echo "Checking pod $pod that should be Running"
  retry_until_success 10 5 "oc get pod $pod -n ${DS_NAMESPACE} -o jsonpath='{.status.phase}'|grep Running"
done
...
if ! jq --arg auth "$REG_ROUTE" -e '.auths[$auth]' <<< "$global_ps" >/dev/null; then
  echo "$REG_ROUTE does not exist in the global-pull-secret"
  exit 1
fi
...
for node in $nodes; do
  ...
  if ! jq --arg auth "$REG_ROUTE" -e '.auths[$auth]' <<< "$secret_json" >/dev/null; then
    echo "$REG_ROUTE does not exist in the node: $node"
    exit 1
  fi
done
...
retry_until_success 10 5 "oc get --ignore-not-found secret global-pull-secret -n kube-system --no-headers | grep -q '^' || echo \"0\""

# in each node, there is no more auth added here.
for node in $nodes; do
  echo "Checking the /var/lib/kubelet/config.json file in node $node for authentication information (after delete)..."
  secret_json=$(oc debug "node/$node" -q -- chroot /host cat /var/lib/kubelet/config.json)
  if jq --arg auth "$REG_ROUTE" -e '.auths[$auth]' <<< "$secret_json" >/dev/null; then
    echo "$REG_ROUTE still exist in the node: $node"
    exit 1
  fi
done
