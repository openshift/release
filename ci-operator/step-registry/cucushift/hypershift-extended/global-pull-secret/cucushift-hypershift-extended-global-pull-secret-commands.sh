...
function check_auth_not_exists_ps_node() {
  local node="$1"
  local auth="$2"
  echo "Checking the /var/lib/kubelet/config.json file in node $node for authentication information (for removal)..."
  secret_json=$(oc debug "node/$node" -q -- chroot /host cat /var/lib/kubelet/config.json)
  if jq --arg auth "$auth" -e '.auths[$auth]' <<< "$secret_json" >/dev/null; then
    echo "$auth still exist in the node: $node"
    return 1
  fi
  return 0
}
...
retry_until_success 10 5 check_pods_running

# check that the reg auth is in the global-pull-secret
retry_until_success 10 5 check_auth_exists_global_ps "$REG_ROUTE"

# in all nodes, the /var/lib/kubelet/config.json file should contain reg auth
nodes=$(oc get nodes -o jsonpath='{.items[*].metadata.name}')
for node in $nodes; do
  retry_until_success 10 5 check_auth_exists_ps_node "$node" "$REG_ROUTE"
done
