...
platform=$(oc --kubeconfig "${SHARED_DIR}"/kubeconfig get hostedcluster "$CLUSTER_NAME" -n "$HYPERSHIFT_NAMESPACE" --ignore-not-found -o=jsonpath='{.spec.platform.type}')
platform_lower=$(echo "$platform" | tr '[:upper:]' '[:lower:]')

# The pull secret name for the hosted cluster
CLUSTER_PULL_SECRET=$(oc --kubeconfig "${SHARED_DIR}"/kubeconfig get hostedcluster "${CLUSTER_NAME}" -n "${HYPERSHIFT_NAMESPACE}" -o jsonpath='{.spec.pullSecret.name}')
...
function delete_image_registry_from_pull_secret() {
  ...
  if oc --kubeconfig $kubeconfig get secret $secret_name -n $namespace 2>/dev/null; then
    local auths="$(oc --kubeconfig $kubeconfig get secret $secret_name -n $namespace -o jsonpath='{.data.\.dockerconfigjson}'|base64 -d)"
    local updated_auths="$(echo $auths | jq --arg registry $registry 'del(.auths[$registry])')"
    local new_auths_b64="$(echo $updated_auths | jq -c '.' | base64 -w0)"
...
function get_existing_registry_from_secret() {
  ...
  local kubeconfig="$3"
  # Get the pull secret
  local ps=$(oc --kubeconfig "$kubeconfig" get secret "$secret_name" -n "$namespace" -o jsonpath='{.data.\.dockerconfigjson}' | base64 -d)
  if echo "$ps" | jq -e '.auths | type == "object" and (length > 0)' >/dev/null 2>&1; then
    # ps is not empty
    local registry=$(echo "$ps" | jq -r '.auths | to_entries | .[0]' 2>/dev/null)
...
function create_or_update_pull_secret() {
  ...
  local new_auth_base64=$(echo -n "$username:$password" | base64)
  ...
  if oc --kubeconfig "$kubeconfig" get secret "$secret_name" -n "$namespace" &>/dev/null; then
    ...
    local auths="$(oc --kubeconfig $kubeconfig get secret $secret_name -n $namespace -o jsonpath='{.data.\.dockerconfigjson}'|base64 -d)"
    local new_auths="$(echo $auths | jq --arg registry $registry --argjson new_auth "$new_auth"  '.auths[$registry] = $new_auth')"
    local new_auths_b64="$(echo $new_auths | jq -c '.' | base64 -w0)"
    oc --kubeconfig "$kubeconfig" patch secret "$secret_name" -n "$namespace" --type='merge' -p "{\"data\": {\".dockerconfigjson\": \"$new_auths_b64\"}}"
  else
    # secret does not exist, create it
    local auths=$(echo "{}" | jq --arg registry "$registry" --argjson new_auth "$new_auth" '.auths[$registry] = $new_auth')
    local auths_b64=$(echo "$auths" | jq -c '.' | base64 -w 0)
...
function verify_credentials_in_ps() {
  ...
  local registry="$2"
  local expected_auth="$3"
  local whole_auth_encoded=$(echo "$whole_ps" | jq -r --arg reg "$registry" '.auths[$reg].auth' 2>/dev/null)
...
function verify_credentials_in_global_ps() {
  ...
  local global_ps=$(oc get secret global-pull-secret -n kube-system -o jsonpath='{.data.\.dockerconfigjson}' | base64 -d)
  if [ -z "$global_ps" ]; then
...
create_or_update_pull_secret "additional-pull-secret" "kube-system" "$KUBECONFIG" "$REG_ROUTE" "$REG_USER" "$REG_PASS"
...
create_or_update_pull_secret "additional-pull-secret" "kube-system" "$KUBECONFIG" "$new_auth_test_url" "$new_auth_test_user" "$new_auth_test_pass"
...
for node in $nodes; do
  ...
  retry_until_success 10 5 check_auth_not_exists_ps_node "$node" "$new_auth_test_url" "$new_registry_auth"
done
...
function test_update_additional_pull_secret_with_conflicts() {
  ...
  set +x
  local existing_auth_json=$(get_existing_registry_from_secret "pull-secret" "openshift-config" "$KUBECONFIG")
  local existing_registry=$(echo "$existing_auth_json" | jq -r '.key')
  local existing_auth=$(echo "$existing_auth_json" | jq -r '.value.auth')
  ...
  local fake_user="fakeuser"
  local fake_pass="fakepass"
  create_or_update_pull_secret "additional-pull-secret" "kube-system" "$KUBECONFIG" "$existing_registry" "$fake_user" "$fake_pass"
...
function test_update_additional_pull_secret_without_conflicts() {
  ...
  local new_registry="test-additional-ps-no-conflict.com"
  local new_user="test-user-additional-ps"
  local new_pass="test-pass-additional-ps"
  create_or_update_pull_secret "additional-pull-secret" "kube-system" "$KUBECONFIG" "$new_registry" "$new_user" "$new_pass"
  echo "Updated additional-pull-secret with new registry: $new_registry"
  local test_new_registry_auth=$(base64_encode_auth "$new_user" "$new_pass")
...
function test_update_original_pull_secret_with_conflicts() {
  ...
  local new_registry="test-additional-ps.com"
  local new_user="test-user-additional-ps"
  local new_pass="test-pass-additional-ps"
  create_or_update_pull_secret "additional-pull-secret" "kube-system" "$KUBECONFIG" "$new_registry" "$new_user" "$new_pass"
  echo "Updated additional-pull-secret with new registry: $new_registry"
  ...
  local new_mgmt_user="test-mgmt-user"
  local new_mgmt_pass="test-mgmt-pass"
  local new_mgmt_auth=$(base64_encode_auth "$new_mgmt_user" "$new_mgmt_pass")
  local mgmt_kubeconfig="${SHARED_DIR}/kubeconfig"
  create_or_update_pull_secret "$CLUSTER_PULL_SECRET" "$HYPERSHIFT_NAMESPACE" "$mgmt_kubeconfig" "$new_registry" "$new_mgmt_user" "$new_mgmt_pass"
  ...
  oc delete secret additional-pull-secret -n kube-system
  retry_until_success 10 5 bash -c "oc get --ignore-not-found secret global-pull-secret -n kube-system --no-headers | grep -q '^' || echo \"0\""
  delete_image_registry_from_pull_secret "$CLUSTER_PULL_SECRET" "$HYPERSHIFT_NAMESPACE" "$mgmt_kubeconfig" "$new_registry"
...
function test_update_original_pull_secret_without_conflicts() {
  ...
  local new_mgmt_registry="test-mgmt-ps.com"
  local new_mgmt_user="test-mgmt-user"
  local new_mgmt_pass="test-mgmt-pass"
  local new_mgmt_auth=$(base64_encode_auth "$new_mgmt_user" "$new_mgmt_pass")
  local mgmt_kubeconfig="${SHARED_DIR}/kubeconfig"
  create_or_update_pull_secret "$CLUSTER_PULL_SECRET" "$HYPERSHIFT_NAMESPACE" "$mgmt_kubeconfig" "$new_mgmt_registry" "$new_mgmt_user" "$new_mgmt_pass"
  ...
  oc delete secret additional-pull-secret -n kube-system
  retry_until_success 10 5 bash -c "oc get --ignore-not-found secret global-pull-secret -n kube-system --no-headers | grep -q '^' || echo \"0\""
  delete_image_registry_from_pull_secret "$CLUSTER_PULL_SECRET" "$HYPERSHIFT_NAMESPACE" "$mgmt_kubeconfig" "$new_mgmt_registry"
...
function test_new_node_pools_inplace_replace_upgrade() {
  ...
  local new_registry="test-ps-upgrade.com"
  local new_user="test-user-ps"
  local new_pass="test-pass-ps"
  create_or_update_pull_secret "additional-pull-secret" "kube-system" "$KUBECONFIG" "$new_registry" "$new_user" "$new_pass"
  echo "Updated additional-pull-secret with new registry: $new_registry"
  local test_new_registry_auth=$(base64_encode_auth "$new_user" "$new_pass")
  # wait until the new registry has been synced to global-pull-secret and nodes
  ...
  hypershift create nodepool "${platform_lower}" --name test-np-inplace --namespace "$HYPERSHIFT_NAMESPACE" --cluster-name "$CLUSTER_NAME" --replicas 1 --node-upgrade-type InPlace
  # Create another nodepool with Replace upgrade type
  hypershift create nodepool "${platform_lower}" --name test-np-replace --namespace "$HYPERSHIFT_NAMESPACE" --cluster-name "$CLUSTER_NAME" --replicas 1 --node-upgrade-type Replace
  # wait until the nodes are ready
  oc wait nodepool test-np-inplace -n "$HYPERSHIFT_NAMESPACE" --for=condition=AllNodesHealthy --timeout=30m
  oc wait nodepool test-np-replace -n "$HYPERSHIFT_NAMESPACE" --for=condition=AllNodesHealthy --timeout=30m
  ...
  for node in $replace_nodes; do
    ...
    retry_until_success 30 10 check_auth_exists_ps_node "$node" "$new_registry" "$test_new_registry_auth"
  done
  ...
  export KUBECONFIG="${SHARED_DIR}/kubeconfig"
  hypershift delete nodepool test-np-inplace --namespace "$HYPERSHIFT_NAMESPACE"
  # Create another nodepool with Replace upgrade type
  hypershift delete nodepool test-np-replace --namespace "$HYPERSHIFT_NAMESPACE"
  # switch back to the hosted cluster
  export KUBECONFIG="${SHARED_DIR}/nested_kubeconfig"
...
echo "=== Running tests of updating ${CLUSTER_PULL_SECRET} for cluster: ${CLUSTER_NAME} ==="
