...
function test_new_node_pools_inplace_replace_upgrade() {
  ...
  if [[ "${platform_lower}" == "azure" ]]; then
      ...
      hypershift create nodepool "${platform_lower}" --name test-np-inplace --namespace "$HOSTEDCLUSTER_NAMESPACE" --cluster-name "$HOSTEDCLUSTER_NAME" --replicas 1 --node-upgrade-type InPlace --nodepool-subnet-id "$SUBNET_ID"
      # Create another nodepool with Replace upgrade type
      hypershift create nodepool "${platform_lower}" --name test-np-replace --namespace "$HOSTEDCLUSTER_NAMESPACE" --cluster-name "$HOSTEDCLUSTER_NAME" --replicas 1 --node-upgrade-type Replace --nodepool-subnet-id "$SUBNET_ID"
  else
