...
oc get namespace "$REGISTRY_NAMESPACE" >/dev/null 2>&1 || oc create namespace "$REGISTRY_NAMESPACE"

# Create the tls secret in the registry namespace if not created yet
oc get secret "$TLS_SECRET_NAME" -n "$REGISTRY_NAMESPACE" >/dev/null 2>&1 || oc create secret tls "$TLS_SECRET_NAME" \
...
oc get secret htpasswd -n "$REGISTRY_NAMESPACE" >/dev/null 2>&1 || oc create secret generic htpasswd --from-file=htpasswd="${tempfile}" -n "$REGISTRY_NAMESPACE"
rm -f "${tempfile}"
...
# Check the image registry is running
RETRIES=20
SLEEP_SEC=5
IS_RUNNING="false"
for i in $(seq 1 $RETRIES); do
    pod_status=$(oc get pods -l "app=$REGISTRY_NAME" -n "$REGISTRY_NAMESPACE" -o jsonpath='{.items[*].status.phase}')
    if [[ "$pod_status" == *"Running"* ]]; then
        echo "Pod is Running!"
        IS_RUNNING="true"
        break
    fi
    echo "Attempt $i/$RETRIES: pod not Running yet, sleeping $SLEEP_SEC seconds..."
    sleep "$SLEEP_SEC"
done

if [[ "$IS_RUNNING" == "false" ]]; then
    echo "Image registry pod failed to start."
    exit 1
fi
