...
if [[ ${HYPERSHIFT_AZURE_MARKETPLACE_ENABLED} == "true" ]]; then
  echo "SKIP for makeplace enabled when creating hosted cluster ....."
  exit 0
fi
...
target_offer=$(eval "oc -n openshift-machine-config-operator get configmap/coreos-bootimages -oyaml |     /tmp/yq-v4  '.data.stream | fromjson | .architectures.x86_64.rhel-coreos-extensions.marketplace.azure.no-purchase-plan.\"hyperV$gen\".offer' -")
target_sku=$(eval "oc -n openshift-machine-config-operator get configmap/coreos-bootimages -oyaml |     /tmp/yq-v4  '.data.stream | fromjson | .architectures.x86_64.rhel-coreos-extensions.marketplace.azure.no-purchase-plan.\"hyperV$gen\".sku' -")
target_version=$(eval "oc -n openshift-machine-config-operator get configmap/coreos-bootimages -oyaml |     /tmp/yq-v4  '.data.stream | fromjson | .architectures.x86_64.rhel-coreos-extensions.marketplace.azure.no-purchase-plan.\"hyperV$gen\".version' -")
