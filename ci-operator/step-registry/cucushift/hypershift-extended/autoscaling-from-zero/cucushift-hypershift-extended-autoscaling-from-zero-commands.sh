...
function use_case_1_check_scale_from_zero_feature() {
    ...
    node_pool_name="test-np-initial"
    ...
    # The nodepool's replicas was set to 1, which is in between the new min/max, so we  see 1 node after enabling autoscaling. TODO: Is this expected ?
...
function use_case_2_scale_up_and_down() {
    trap 'return 1' ERR
    echo "=========================================="
    echo "USE CASE 2: Auto scaling from/to zero with ScaleUpAndScaleDown"
    echo "=========================================="
...
function use_case_3_scale_up_only() {
    trap 'return 1' ERR
    echo "=========================================="
    echo "USE CASE 3: Auto scaling from zero with ScaleUpOnly"
    echo "=========================================="
...
function use_case_4_autoscaling_only() {
    trap 'return 1' ERR
    echo "=========================================="
    echo "USE CASE 4: Workloads only affect autoscaling-enabled nodepools"
    echo "=========================================="
...
function use_case_5_node_labels_and_taints() {
    trap 'return 1' ERR
    echo "=========================================="
    echo "USE CASE 5: NodePool with nodeLabels and taints"
    echo "=========================================="
...
# Use Case 2
echo "Executing Use Case 2: Auto scaling from/to zero with ScaleUpAndScaleDown"
use_case_2_scale_up_and_down
echo ""

# Use Case 3 only executed on clusters 4.18+
...
if [[ "$(printf '%s\n' "4.18" "$CLUSTER_VERSION" | sort -V | head -n1)" == "4.18" ]]; then
    echo "Cluster version ${CLUSTER_VERSION} supports ScaleUpOnly, executing Use Case 3"
    echo "Executing Use Case 3: Auto scaling from zero with ScaleUpOnly"
    use_case_3_scale_up_only
else
    echo "Cluster version ${CLUSTER_VERSION} does not support ScaleUpOnly, skipping Use Case 3 of scale-up-only test"
fi
echo ""

# Use Case 4
echo "Executing Use Case 4: Workloads only affect autoscaling-enabled nodepools"
use_case_4_autoscaling_only
echo ""

# Use Case 5
echo "Executing Use Case 5: NodePool with nodeLabels and taints"
use_case_5_node_labels_and_taints
echo ""
