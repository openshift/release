...
# After purchase, capacity blocks reservations that start in the future always begin at 11:30 AM UTC.
# After purchase, the capacity block can only be automatically canceled after 1 day, and manual cancellation will not take effect.
# So when manually trigger aws-ipi-ovn-hypershift-capacity-reservation-gpu-guest-f999 job, please do it 2h ago before current day 11:30 AM UTC.
function find_and_purchase_capacity_blocks() {
    ...
    while true; do
        ...
        if [ "${CURRENT_TIMESTRAMP}" -ge "${CB_START_TIMESTRAM}" ]; then
