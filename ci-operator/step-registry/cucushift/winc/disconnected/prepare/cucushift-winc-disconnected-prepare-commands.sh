...
main() {
# 1. Check Windows nodes status
echo "### Checking Windows nodes ###"
# List Windows nodes specifically
oc get nodes -l kubernetes.io/os=windows
# List all nodes with labels to verify Windows node labeling
oc get nodes --show-labels | grep windows

# 2. Check MachineSet status - verify Windows machine configurations
echo -e "\n### Checking MachineSets ###"
# List all machinesets
oc get machinesets -n openshift-machine-api
# Check replicas configuration in machinesets
oc get machinesets -n openshift-machine-api -o yaml | grep -A 5 "replicas"

# 3. Check Machine status - verify if Windows machines are being created
echo -e "\n### Checking Machines ###"
# List Windows machines
oc get machines -n openshift-machine-api | grep windows
# Check machine events for potential issues
oc describe machines -n openshift-machine-api | grep -A 10 "Events:"

# 4. Check Windows Machine Config Operator (WMCO) status
echo -e "\n### Checking WMCO pods ###"
# Verify WMCO pods are running
oc get pods -n openshift-windows-machine-config-operator
# Check WMCO logs for any errors
oc logs -n openshift-windows-machine-config-operator deployment/windows-machine-config-operator

# 5. Check related events for troubleshooting
echo -e "\n### Checking Events ###"
# Check machine-api events
oc get events -n openshift-machine-api | grep -i windows
# Check WMCO events
oc get events -n openshift-windows-machine-config-operator

# 6. Wait for Windows nodes to become ready
echo -e "\n### Waiting for Windows nodes ###"
# Wait up to 15 minutes for Windows nodes to be ready
