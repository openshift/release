...
getMachinesetName() {
  ...
  case "$platform" in
    aws|gcp)
      # Get reference machineset name from existing worker machinesets
      local ref_machineset_name
      ...
      # Replace 'worker' with 'winworker' in the name
      echo "${ref_machineset_name/worker/winworker}"
      ;;
    azure)
      # Azure uses "windows" as the machineset name
      # naming-conventions-for-computer-domain-site-ou - vsphere/nutanix/Azure use short names for windows machines
      echo "windows"
      ;;
    vsphere|nutanix)
      # vSphere and Nutanix use fixed name "winworker"
      echo "winworker"
      ;;
    *)
      echo ""
