...
echo "Waiting for Windows MachineSet nodes to come up in Running state"
...
if [[ -n "${BYOH_NODE_COUNT:-}" ]]; then
    ...
    timeout 15m bash -c '
        while [[ $(oc get nodes -l "kubernetes.io/os=windows,windowsmachineconfig.openshift.io/byoh=true" --no-headers | wc -l) != "${BYOH_NODE_COUNT}" ]]; do
            echo "Current BYOH nodes status:"
