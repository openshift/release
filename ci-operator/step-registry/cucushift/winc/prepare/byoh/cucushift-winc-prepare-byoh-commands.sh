...
function setup_image_mirroring()
{
  ...
  # PowerShell: Server 2022 (ltsc2022)
...
# Windows nodes are already provisioned and Ready
# - For AWS: windows-byoh-provision waits for nodes (BYOH_READY_TIMEOUT)
# - For vSphere: upi-vsphere-windows-pre provisions VMs and they join via WMCO

# Wait for Windows nodes to appear and become Ready before creating workloads
# WMCO needs time to configure VMs and join them to the cluster
echo "Waiting for Windows nodes to appear and become Ready..."
timeout 30m bash -c '
  while true; do
    node_count=$(oc get nodes -l kubernetes.io/os=windows --no-headers 2>/dev/null | wc -l)
    if [[ ${node_count} -gt 0 ]]; then
      ready_count=$(oc get nodes -l kubernetes.io/os=windows --no-headers 2>/dev/null | awk "$2 == \"Ready\" || $2 ~ /^Ready,/" | wc -l)
      echo "Windows nodes: ${ready_count}/${node_count} Ready"
      if [[ ${ready_count} -eq ${node_count} ]]; then
        echo "All Windows nodes are Ready"
        break
      fi
    else
      echo "No Windows nodes found yet, waiting..."
    fi
    sleep 10
  done
'
...
# For vSphere: upi-conf-vsphere-ova-windows saves template name
...
elif [[ -f "${SHARED_DIR}/windows_vm_template.txt" ]]; then
    windows_os_image_id=$(cat "${SHARED_DIR}/windows_vm_template.txt")
    echo "Using Windows VM template from SHARED_DIR: ${windows_os_image_id}"
else
    # Fallback for other platforms: query node info (requires nodes to exist)
    windows_os_image_id="byoh-windows-$(oc get nodes -l 'kubernetes.io/os=windows' -o=jsonpath="{.items[0].status.nodeInfo.osImage}" | sed -n 's/.*Windows Server \([0-9]*\).*/\1/p' || echo '2022')"
    echo "Using Windows OS image placeholder: ${windows_os_image_id}"
fi

# Use Windows Server 2022 container image
# vSphere only supports Windows Server 2022 (2019 is EOL)
