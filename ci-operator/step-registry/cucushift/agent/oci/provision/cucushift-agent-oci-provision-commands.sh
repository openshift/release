...
STACK_ID=$(<"${SHARED_DIR}"/stack-id.txt)
JOB_ID=$(<"${SHARED_DIR}"/job-id.txt)
...
extract_build02_auth=$(jq -c '.auths."registry.build03.ci.openshift.org"' ${build02_secrets})
final_pull_secret=$(jq -c --argjson auth "$extract_build02_auth" '.auths["registry.build03.ci.openshift.org"] += $auth' "${pull_secret_path}")
...
--stack-id "${STACK_ID}" \
--variables '{"openshift_image_source_uri":"'"${IMAGE_URI}"'",
"zone_dns":"'"${DNS_ZONE}"'",
...
/tmp/openshift-install --dir="${INSTALL_DIR}" agent wait-for install-complete 2>&1 | stdbuf -o0 grep -v password &
