...
ODF_VERSION_MAJOR_MINOR=${ODF_VERSION_MAJOR_MINOR:-$(oc::getClusterVersionXY)}
...
if [[ ! -f "${ODF_QUAY_CREDENTIALS_FILE}" ]]; then
  echo_debug "ERROR: ODF Quay credentials file not found"
  exit 1
fi

echo "🐶 Get pull secret from cluster"
oc::getClusterPullSecret > "${CLUSTER_PULL_SECRETS_ORIGINAL}"
...
echo "Extract IDMS from the catalog image (retry 4x)"
retry_backoff 4 1 oc image extract "${ODF_CATALOG_IMAGE}" --file /idms.yaml

# Create an IDMS if applicable
if [ -e "idms.yaml" ] ; then
  echo "Found IDMS, creating..."
  oc apply --filename="idms.yaml"
  sleep 30
  echo "Waiting for MCP to be updated..."
  wait_for_mcp_to_update 60
fi
...
echo " 🚮 Remove is-default-class and is-default-virt-class annotations from all the storage classes"
oc get sc -o name | xargs -I{} oc annotate {} storageclass.kubernetes.io/is-default-class- storageclass.kubevirt.io/is-default-virt-class-

echo " ⭐ Make ${DEFAULT_STORAGE_CLASS} the default storage class"
oc annotate storageclass "${DEFAULT_STORAGE_CLASS}" storageclass.kubevirt.io/is-default-virt-class=true storageclass.kubernetes.io/is-default-class=true
