...
while IFS= read -r NAME; do
	...
	oc -v=6 --request-timeout=60s -n default debug "node/${NAME}" -- sh -c 'ls -l /host/etc/systemd && ls -l /host/etc/systemd/network && head -n1000 /host/etc/systemd/network/*.link' > "${ARTIFACT_DIR}/links/${NAME}.txt" || true
done </tmp/nodes
