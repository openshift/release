...
payload_version="$(env "NO_PROXY=*" "no_proxy=*" oc adm release info "${RELEASE_IMAGE_LATEST}" --output=json | jq -r '.metadata.version')" || true
payload_minor_version="$(echo "${payload_version}" | cut -f2 -d.)"
if [[ -z "${payload_minor_version}" ]] || (( ${payload_minor_version} < 21 )); then
