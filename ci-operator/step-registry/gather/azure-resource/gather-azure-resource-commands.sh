...
function gatherLBs() {
    local lbs aps name state ip_id public_ip_address private_ip_address public_ip_name temp_file backend_ids
    echo ""
    ...
    for lb in $lbs; do
        ...
        while IFS=$'\t' read -r name state public_ip_id private_ip; do
            ...
            if [ "$public_ip_id" != "null" ]; then
                ...
                [ -z "$public_ip_address" ] && public_ip_address="Not Found"
            fi
...
echo ""
run_command "az vm list-ip-addresses --resource-group $RESOURCE_GROUP --output table"
echo ""
run_command "az network public-ip list --resource-group $RESOURCE_GROUP --output table"
echo ""
gatherLBs
