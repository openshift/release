...
function create_vpc()
{
  ...
  if [ $cmd_ret -ne 0 ]; then
    infra_manager_debug "${CLUSTER_NAME}-${UPI_RESOURCE_DIR_VPC//_/-}" $cmd_ret
  fi
}
...
if [[ ! -f "metadata.json" ]]; then
  echo "$(date -u --rfc-3339=seconds) - ERROR: metadata.json not found"
  exit 1
fi
...
if [ ${#MASTER_ZONES[@]} -lt 3 ]; then
  echo "$(date -u --rfc-3339=seconds) - ERROR: Insufficient zones for master nodes. Found ${#MASTER_ZONES[@]}, need at least 3"
  exit 1
fi

ZONE_0="${MASTER_ZONES[0]}"
ZONE_1="${MASTER_ZONES[1]}"
ZONE_2="${MASTER_ZONES[2]}"
...
function create_external_lb()
{
  ...
  run_command "${CMD}" || cmd_ret=$?
  if [ $cmd_ret -ne 0 ]; then
    infra_manager_debug "${CLUSTER_NAME}-${UPI_RESOURCE_DIR_INFRA_EXTERNAL_LB//_/-}" $cmd_ret
  fi
  short_wait
}
...
function create_internal_lb()
{
  ...
  run_command "${CMD}" || cmd_ret=$?
  if [ $cmd_ret -ne 0 ]; then
    infra_manager_debug "${CLUSTER_NAME}-${UPI_RESOURCE_DIR_INFRA_INTERNAL_LB//_/-}" $cmd_ret
  fi
  short_wait
}
...
function create_dns_private_zone()
{
  ...
  run_command "${CMD}" || cmd_ret=$?
  if [ $cmd_ret -ne 0 ]; then
    infra_manager_debug "${CLUSTER_NAME}-${UPI_RESOURCE_DIR_INFRA_DNS_PRIV_ZONE//_/-}" $cmd_ret
  fi
}
...
function create_firewall_rules_and_iam_sa()
{
  ...
  run_command "${CMD}" || cmd_ret=$?
  if [ $cmd_ret -ne 0 ]; then
    infra_manager_debug "${CLUSTER_NAME}-${UPI_RESOURCE_DIR_SECURITY//_/-}" $cmd_ret
  fi
  short_wait
}
...
function create_bootstrap_resources()
{
  ...
  run_command "${CMD}" || cmd_ret=$?
  if [ $cmd_ret -ne 0 ]; then
    infra_manager_debug "${CLUSTER_NAME}-${UPI_RESOURCE_DIR_BOOTSTRAP//_/-}" $cmd_ret
  fi
  short_wait
}
...
function create_control_plane_machines()
{
  ...
  run_command "${CMD}" || cmd_ret=$?
  if [ $cmd_ret -ne 0 ]; then
    infra_manager_debug "${CLUSTER_NAME}-${UPI_RESOURCE_DIR_CONTROL_PLANE//_/-}" $cmd_ret
  fi
  short_wait
...
function create_worker_machines()
{
  ...
  run_command "${CMD}" || cmd_ret=$?
  if [ $cmd_ret -ne 0 ]; then
    infra_manager_debug "${CLUSTER_NAME}-${UPI_RESOURCE_DIR_WORKER//_/-}" $cmd_ret
  fi
  short_wait
...
if [ ${#WORKER_ZONES[@]} -lt 1 ]; then
  echo "$(date -u --rfc-3339=seconds) - ERROR: No zones available for worker nodes"
  exit 1
fi
...
if [[ -v IS_XPN ]]; then
  ...
  ROUTER_IP_TIMEOUT=600  # 10 minutes timeout
  ROUTER_IP_ELAPSED=0
  while [[ "$ROUTER_IP" == "" || "$ROUTER_IP" == "<pending>" ]]; do
    if [ ${ROUTER_IP_ELAPSED} -ge ${ROUTER_IP_TIMEOUT} ]; then
      echo "$(date -u --rfc-3339=seconds) - ERROR: Timeout waiting for router external IP after ${ROUTER_IP_TIMEOUT} seconds"
      set -e
      exit 1
    fi
    short_wait 10;
    ROUTER_IP_ELAPSED=$((ROUTER_IP_ELAPSED + 10))
    ROUTER_IP="$(oc -n openshift-ingress get service router-default --no-headers | awk '{print $4}')"
  done
  set -e
fi
