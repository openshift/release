...
openshift_install_tarball = "https://mirror.openshift.com/pub/openshift-v4/multi/clients/${OCP_STREAM}/${OPENSHIFT_VERSION}/ppc64le/openshift-install-linux.tar.gz"
openshift_client_tarball  = "https://mirror.openshift.com/pub/openshift-v4/multi/clients/${OCP_STREAM}/${OPENSHIFT_VERSION}/ppc64le/openshift-client-linux.tar.gz"
...
function create_upi_powervs_cluster() {
  ...
  /tmp/terraform apply -auto-approve -var-file var-multi-arch-upi.tfvars | \
  ...
  cp "${IBMCLOUD_HOME_FOLDER}"/ocp-install-dir/automation/terraform.tfstate "${SHARED_DIR}"/terraform-multi-arch-upi.tfstate

  /tmp/terraform output -raw -no-color bastion_private_ip | tr -d '"' > "${SHARED_DIR}"/BASTION_PRIVATE_IP
  /tmp/terraform output -raw -no-color bastion_public_ip | tr -d '"' > "${SHARED_DIR}"/BASTION_PUBLIC_IP
  cd .. || true
...
*powervs*)
  ...
  # Create a MAJOR.MINOR.* into MAJOR-MINOR, such as 4.15
  ...
  echo "PI: Create Network"
  ic pi subnet create ocp-net --cidr-block 192.168.200.0/24 --net-type private --dns-servers 9.9.9.9 --gateway 192.168.200.1 --ip-range 192.168.200.10-192.168.200.250 --jumbo
  # The CentOS-Stream-9 image is stock-image on PowerVS.
  echo "PI: Creating the Centos Stream Image"
