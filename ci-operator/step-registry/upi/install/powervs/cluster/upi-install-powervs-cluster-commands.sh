...
function create_upi_powervs_cluster() {
  ...
  OUTPUT="yes"
  cd automation/ || true
  ../terraform apply -auto-approve -var-file var-multi-arch-upi.tfvars | \
...
# so we can use scp to pull the kubeconfig
...
*powervs*)
  ...
  echo "IC: install plugins"
  ic plugin install -f power-iaas tg-cli vpc-infrastructure cis
