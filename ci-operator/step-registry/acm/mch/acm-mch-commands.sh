...
#
# Upstream PR: https://github.com/open-cluster-management-io/ocm/pull/1309
#
...
#
# Once the upstream fix is merged, this workaround becomes dead code and
# can be removed. The main wait logic will succeed on the first attempt.
###############################################################################
...

{
  ...
  # Check for OCM CA bundle race condition (PR #1309) and apply workaround
  if apply_ocm_cabundle_workaround; then
    ...
    # Workaround was not applicable - this is some other issue
    show_multiclusterhub_related_objects ;
    dump_multiclusterhub_pod_logs ;
    echo "Error: MCH failed to reach Running status in alloted time." ;
    echo "The OCM CA bundle race condition (PR #1309) was not detected." ;
