...
AssertGlobalnetDisabled() {
    ...
    case "${globalnetEnabled}" in
        false|"")
            : "Globalnet disabled on '${clusterName}' (globalnetEnabled=${globalnetEnabled:-<unset>})"
            ;;
        *)
            : "ERROR: SubmarinerConfig globalnetEnabled='${globalnetEnabled}' on '${clusterName}' — expected false or unset, incompatible with CCLM" >&2
            false
            ;;
    esac
}
