...
metadata:
  name: "${CLUSTER_IMG_SET_REF}"
spec:
  releaseImage: "${RELEASE_IMAGE_LATEST}"
EOF

  set -x
  oc get ClusterImageSet ${CLUSTER_IMG_SET_REF} -oyaml
  set +x
}
...
function checking_installation_progress {
  ...
  while true; do
    ...
    echo "------------------------------------------------------------------------------------"
    {
      ...
      echo ------------------------------------------------- ;
      #oc -n ${SPOKE_CLUSTER_NAME} get agentclusterinstalls.extensions.hive.openshift.io ${SPOKE_CLUSTER_NAME} -ocustom-columns=CLUSTER:.spec.clusterDeploymentRef.name,INSTALLING:.spec.imageSetRef.name ;
