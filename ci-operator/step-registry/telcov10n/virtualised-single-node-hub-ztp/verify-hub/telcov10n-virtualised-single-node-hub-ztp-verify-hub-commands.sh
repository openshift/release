...
hub_pool=$(cat ${cnt_fname} | jq -c)
hub_pool=$(jq -c --arg id "${hub_id}" '
  (.pool[] | select(.hub_id == $id))
  |= (.state = "installed")
  ' <<<"${hub_pool}")

echo ${hub_pool} >| ${cnt_fname}
set +x
...
function set_virtualised_sno_hub_as_installed {
  ...
  last_selected_vhub_filename="$(cat ${SHARED_DIR}/hub_pool_info.txt | cut -d' ' -f2)"
  ...
  for ((attempts = 0 ; attempts < ${max_attempts:=5} ; attempts++)); do
    ...
    if [[ "$(check_the_host_was_locked "${AUX_HOST}" "${lock_filename}" "${ts}")" == "locked" ]] ; then
      update_selected_vhub_to_use "${last_selected_vhub_filename}" "${hub_id}"
      return 0
    fi
    set -x
    sleep 1m
