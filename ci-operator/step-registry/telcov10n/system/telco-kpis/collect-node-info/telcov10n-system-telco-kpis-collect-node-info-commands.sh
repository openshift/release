...
# Override remote_tmp to a writable path.
...
main() {
    ...
    # Determine kubeconfig paths on bastion (not in SHARED_DIR)
    # These files exist on the bastion host where Ansible playbook will run
