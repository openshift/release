...
export_env_vars_from_json 'oslat' "${TEST_SETTINGS:-}" "${TEST_SETTINGS_DEFAULTS:-}"
setup_continue_on_fail
setup_debug_on_fail

main() {
    echo "Running oslat test for spoke: ${SPOKE_CLUSTER}"
    ...
    echo "Running oslat test (duration: ${DURATION})"
    local rc=0
    ...
    local artifact_subdir="${ARTIFACT_DIR}/oslat-${SPOKE_CLUSTER}"
    ...
    echo "Oslat test completed for ${SPOKE_CLUSTER} (rc=${rc})"
    return "${rc}"
}
