...
export_env_vars_from_json 'cyclictest' "${TEST_SETTINGS:-}" "${TEST_SETTINGS_DEFAULTS:-}"
setup_continue_on_fail
setup_debug_on_fail

main() {
    echo "Running cyclictest test for spoke: ${SPOKE_CLUSTER}"
    ...
    echo "Running cyclictest test (duration: ${DURATION})"
    ...
    local artifact_subdir="${ARTIFACT_DIR}/cyclictest-${SPOKE_CLUSTER}"
    ...
    echo "Cyclictest test completed for ${SPOKE_CLUSTER}"
}
