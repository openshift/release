...
# ----------------------------------------------------------------------
# download_lockdown_json
#
# Downloads a lockdown JSON file from a GitLab URL, resolving symlink
# chains. GitLab's /-/raw/ endpoint returns symlink content as plain
# text instead of following the symlink. This function detects that and
# recursively resolves until actual JSON is found (max 5 hops).
#
# Ported from eco-ci-cd lockdowns role resolve-gitlab-symlinks.sh.j2.
# Reusable across steps (deploy-sno-hub, install-hub-operators,
# mirror-spoke-operators).
#
# Parameters:
#   1 - url: GitLab raw URL to lockdown JSON (may be a symlink)
#   2 - dest: local path to save the resolved JSON file
# ----------------------------------------------------------------------
...
download_lockdown_json() {
    ...
    local temp_file="/tmp/lockdown-resolve-$$.tmp"

    echo "Downloading lockdown from: ${current_url}"
    ...
    while [ $depth -lt $max_depth ]; do
        ...
        if [ "$first_char" = "{" ] || [ "$first_char" = "[" ]; then
            mv "$temp_file" "$dest"
            echo "Resolved lockdown after $depth symlink hop(s): $(basename "$current_url")"
            return 0
        fi
