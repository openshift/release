...
# Substitute environment variables in OPERATORS JSON
echo "Processing OPERATORS variable substitution..."
# Convert VERSION to a format usable in image tags (e.g., "4.19" -> "4-19")
VERSION_TAG="${VERSION//./-}"
export VERSION_TAG

# Perform variable substitution in OPERATORS
# This allows using ${VERSION} and ${VERSION_TAG} in the OPERATORS JSON
OPERATORS=$(echo "${OPERATORS}" | envsubst)
export OPERATORS
...
ansible-playbook ./playbooks/deploy-ocp-sno.yml -i ./inventories/ocp-deployment/build-inventory.py \
    --extra-vars "release=${VERSION} cluster_name=${CLUSTER_NAME} disconnected=true mirror_only=true"
