...
NROP_BRANCH="master"
GINKGO_LABEL="tier-0 && !reboot_required"
...
ansible-playbook -vv "${SHARED_DIR}"/repos/telco-ci/playbooks/hcp_nrop_install.yml \
    -e mgmt_kubeconfig="${SHARED_DIR}"/mgmt-kubeconfig \
    -e hosted_kubeconfig="${SHARED_DIR}"/kubeconfig \
    -c local
...
if [[ "${T5CI_VERSION}" == "4.18" ]]; then
    ...
    GINKGO_LABEL="(tier-0 || tier-1) && !reboot_required"
    run_tests
fi
