...
# Load INTERFACE_LIST env variable
...
# ssh -o StrictHostKeyChecking=no $BASTION_USER@$BASTION_IP -i /tmp/temp_ssh_key "cd /tmp/cnf-features-deploy;./cnf-tests-run.sh"

echo "Gather artifacts from bastion"
scp -r -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i /tmp/temp_ssh_key $BASTION_USER@$BASTION_IP:/tmp/junit/*xml /tmp/
# rm -rf $PROJECT_DIR/temp_ssh_key

# echo "Copy junit report to artifact directory"
# ls -l /tmp/
cat /tmp/junit_test_report.xml
# cp /tmp/cnftests-junit.xml ${ARTIFACT_DIR}/junit_test-result.xml
cat /tmp/junit_test_report.xml > ${ARTIFACT_DIR}/junit.xml
ls -l ${ARTIFACT_DIR}/