...
echo "Copy tests to artifacts directory"
cp -vr /tmp/wip/tests "${ARTIFACT_DIR}"
...
if ls ${ARTIFACT_DIR}/tests/*.xml 1> /dev/null 2>&1; then
    echo "Copy junit test reports to shared directory for reporter step"
    cp -v "${ARTIFACT_DIR}"/tests/*.xml "${SHARED_DIR}/" 2>/dev/null
