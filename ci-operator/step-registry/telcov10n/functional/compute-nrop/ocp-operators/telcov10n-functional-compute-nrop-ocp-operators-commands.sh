...
if [[ "$TEST_ENV" == "stage" ]]; then
  EXTRA_VARS="${EXTRA_VARS} disconnected=true"
  EXTRA_VARS="${EXTRA_VARS} mirror_only=false" 
  EXTRA_VARS="${EXTRA_VARS} ocp_operator_mirror_skip_internal_registry_cleanup=true"
fi
