...
function check_envs()
{
    ...
    if oc -n openshift-logging get pod  -l name=cluster-logging-operator; then
    ...
    if oc -n openshift-operators-redhat get pod -l name=loki-operator-controller-manager; then
...
EOF
        ...
        minio_secret=$(oc get deployment minio -n openshift-logging-minio -o jsonpath='{.spec.template.spec.containers[0].env[?(@.name=="MINIO_SECRET_KEY")].value}')
...
function create_minio_bucket(){
   echo "Download mc binary"
    ...
    minio_url=$(oc get route minio -n openshift-logging-minio -o jsonpath='{.spec.host}')
...
function enable_clf_5x(){
    ...
    #Noe: for 5.x, we only deploy collector in openshift-logging namespace
