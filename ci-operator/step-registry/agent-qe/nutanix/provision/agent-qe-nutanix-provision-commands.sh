...
yq e '(.[] | select(has("no_log"))) |= with_entries(select(.key != "no_log"))' -i ansible-files/roles/nutanix/update_ignition/tasks/main.yml

ansible-playbook ansible-files/nutanix_provision_vm.yml
