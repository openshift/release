#!/bin/bash
# Setup mirror registry credential and IDMS when proxy registry is used.
# Enable openshift qe catalogsource when OLM and marketplace is available
# mirror operator images into local mirror registry when cluster type is c2s or sc2s
# The script exit 1 if fail to create mirror registry credential or IDMS.
...
function set_cluster_auth () {
    ...
    if [[ $ret -eq 0 ]]; then 
        ...
        jq --argjson a "{\"${MIRROR_PROXY_REGISTRY}\": {\"auth\": \"$registry_cred\"}}" '.auths |= . + $a' "/tmp/.dockerconfigjson" > /tmp/new-dockerconfigjson
...
# this func only used when the cluster not set the Proxy registry, such as C2S, SC2S clusters
...
EOF
    #OPERATORS_TO_MIRROR: comma-separated values. for example: elasticsearch-operator,cincinnati-operator,file-integrity-operator
    if [[ -z "${OPERATORS_TO_MIRROR}" ]]; then
        echo "mirror all operators"
    else 

        #only mirror images defined in Env OPERATORS_TO_MIRROR
...
function set_CA_for_nodes () {
    ...
    if [ -n "${ca_name}" ] && [ "${ca_name}" = "registry-config" ] ; then
...
function create_settled_idms () {
    echo "create IDMS for proxy registry"
    cat <<EOF  | oc create -f -
    apiVersion: config.openshift.io/v1
    kind: ImageDigestMirrorSet
    metadata:
      name: idms-additional-day2-operators
    spec:
      imageDigestMirrors:
      - mirrors:
...
EOF
    ...
    # add mirror registry for registry.redhat.io/openshift4
    INDEX=$(oc get imagedigestmirrorset idms-operator-0 -o json | jq '.spec.imageDigestMirrors | map(.source=="registry.redhat.io/openshift4") | index(true)')
    oc patch imagedigestmirrorset idms-operator-0 \
      --type='json' \
      -p="[ {\"op\": \"add\", \"path\": \"/spec/imageDigestMirrors/${INDEX}/mirrors/-\", \"value\": \"${MIRROR_PROXY_REGISTRY}/openshift4\"} ]"

    if [ $? == 0 ]; then
        echo "IDMS has been created successfully"
	return 0
    else
...
{    
    echo "create the disconnected catalog source: $CATALOGSOURCE_NAME"
            cat <<EOF | oc create -f -
...
EOF
    ...
    do
        ...
        if [[ $STATUS = "READY" ]]; then
            echo "the disconnected CatalogSource has been created successfully"
            break
        fi
    done
    if [[ $STATUS != "READY" ]]; then
...
function check_default_catalog () {
    ...
    if [ $COUNTER -ne 100 ]; then
        echo "!!! The default CatalogSource doesn't work."
...
mirror_index_image="${MIRROR_REGISTRY_HOST}/openshift-qe-optional-operators/aosqe-index:v${kube_major}.${kube_minor}"
echo "origin_index_image: ${origin_index_image}"
echo "mirror_index_image: ${mirror_index_image}"

set_cluster_auth

# Create IDMS for mirror registry. The IDMS are used for the following ci-opertor steps too. Abort the job if IDMS can not be created
create_settled_idms  || exit 1
