...
echo "=== Wiping stale Ceph signatures from OSD block devices ==="
TOOLS_IMAGE=$(oc adm release info --image-for=tools 2>/dev/null || echo "")
if [[ -z "${TOOLS_IMAGE}" ]]; then
  TOOLS_IMAGE=$(oc adm release info --image-for=cli 2>/dev/null || echo "")
fi
echo "Using image for wipe pods: ${TOOLS_IMAGE}"

OSD_PVS=$(oc get pv -o json | jq -r \
  '.items[] | select(.spec.storageClassName == "'"${OSD_STORAGE_CLASS}"'") | .metadata.name')

if [[ -n "${OSD_PVS}" && -n "${TOOLS_IMAGE}" ]]; then
  IDX=0
  while IFS= read -r PV_NAME; do
    [[ -z "${PV_NAME}" ]] && continue
    NODE=$(oc get pv "${PV_NAME}" -o jsonpath='{.metadata.labels.kubernetes\.io/hostname}' 2>/dev/null || echo "")
    ZAP_NAME="osd-zap-${IDX}"
    echo "  Wiping PV ${PV_NAME} on node ${NODE}..."
...
spec:
  ...
  - name: zap
    ...
    args:
    - |
      echo "Wiping /dev/osd-block..."
      wipefs -af /dev/osd-block 2>&1 || true
      sgdisk --zap-all /dev/osd-block 2>&1 || true
      dd if=/dev/zero of=/dev/osd-block bs=1M count=200 conv=fsync 2>/dev/null || true
      SIZE_MB=\$(( \$(blockdev --getsize64 /dev/osd-block) / 1048576 ))
      if [ \${SIZE_MB} -gt 400 ]; then
        dd if=/dev/zero of=/dev/osd-block bs=1M count=200 seek=\$(( SIZE_MB - 200 )) conv=fsync 2>/dev/null || true
      fi
      echo "Wipe complete"
    securityContext:
      privileged: true
...
ZEOF
  ...
  echo "  Waiting for ${IDX} wipe pod(s) to complete..."
  for i in $(seq 0 $((IDX - 1))); do
    ZAP_NAME="osd-zap-${i}"
    if oc wait --for=jsonpath='{.status.phase}'=Succeeded "pod/${ZAP_NAME}" -n openshift-storage --timeout=5m 2>/dev/null; then
      echo "  ${ZAP_NAME}: succeeded"
    else
      echo "  WARNING: ${ZAP_NAME} did not succeed:"
      oc logs "${ZAP_NAME}" -n openshift-storage 2>/dev/null || true
      oc get pod "${ZAP_NAME}" -n openshift-storage -o jsonpath='{.status}' 2>/dev/null || true
    fi
    oc delete pod "${ZAP_NAME}" -n openshift-storage --force --grace-period=0 2>/dev/null || true
    oc delete pvc "${ZAP_NAME}" -n openshift-storage --wait=false 2>/dev/null || true
  done
  echo "  Waiting for PVs to return to Available..."
  sleep 10
  COUNTER=0
  while [ $COUNTER -lt 120 ]; do
    AVAIL=$(oc get pv -o json | jq -r \
      '[.items[] | select(.spec.storageClassName == "'"${OSD_STORAGE_CLASS}"'" and .status.phase == "Available")] | length')
    if [[ "${AVAIL}" -ge "${IDX}" ]]; then
      echo "  All ${AVAIL} OSD PVs are Available"
      break
    fi
    sleep 5
    COUNTER=$((COUNTER + 5))
    echo "  Waiting ${COUNTER}s for PVs to become Available (${AVAIL}/${IDX})..."
  done
  echo "OSD disk wipe complete"
else
  echo "  No OSD PVs found or no image available — skipping wipe"
fi
