...
# 8. Generate and apply ITMS + IDMS for transparent image pull redirection
function generate_and_apply_mirror_sets () {
    echo "=== Generating ImageTagMirrorSet (ITMS) + ImageDigestMirrorSet (IDMS) ==="

    local INTERNAL_SVC="image-registry.openshift-image-registry.svc:5000"

    # Collect unique source prefixes (registry/namespace) from images
    declare -A SOURCE_PREFIXES
    for SRC_IMG in "${IMAGES_TO_MIRROR[@]}"; do
        local domain
        domain=$(echo "$SRC_IMG" | cut -d/ -f1)
        local ns
        ns=$(echo "$SRC_IMG" | cut -d/ -f2)
        SOURCE_PREFIXES["${domain}/${ns}"]="${INTERNAL_SVC}/${ns}"
    done

    local ITMS_ENTRIES=""
    local IDMS_ENTRIES=""
    for src in "${!SOURCE_PREFIXES[@]}"; do
        local dst="${SOURCE_PREFIXES[$src]}"
    ...
    local MIRROR_SET_FILE="/tmp/itms-idms-utility-images.yaml"
...
EOF
    ...
    echo "Generated mirror sets:"
    cat "${MIRROR_SET_FILE}"

    echo "Applying ITMS + IDMS..."
    oc apply -f "${MIRROR_SET_FILE}"

    echo "Waiting for MachineConfigPools to update after mirror set application..."
    oc wait machineconfigpool --all --for=condition=Updated --timeout=30m || {
        echo "WARNING: MachineConfigPool update timed out"
        oc get mcp
        echo "Continuing, but image pulls via mirror sets may not work until nodes are updated"
    }

    echo "=== ITMS + IDMS applied successfully ==="
}
...
mirror_images
enable_image_pulls
generate_and_apply_mirror_sets
