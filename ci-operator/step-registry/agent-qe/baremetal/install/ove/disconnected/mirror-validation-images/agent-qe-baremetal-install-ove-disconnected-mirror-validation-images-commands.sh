...
MGMT_STATE=$(oc get configs.imageregistry.operator.openshift.io/cluster \
  -o jsonpath='{.spec.managementState}' 2>/dev/null || echo "")
echo "Current registry managementState: ${MGMT_STATE}"

if [[ "${MGMT_STATE}" != "Managed" ]]; then
  echo "Registry is not Managed — enabling it with PVC storage"
  STORAGE_CLASS=""
  for sc in ocs-storagecluster-ceph-rbd-virtualization ocs-storagecluster-ceph-rbd; do
  ...
  oc patch configs.imageregistry.operator.openshift.io/cluster --type=merge \
    --patch "{\"spec\":{\"managementState\":\"Managed\",\"defaultRoute\":true,\"replicas\":1,\"storage\":{\"pvc\":{\"claim\":\"\",\"storageClassName\":\"${STORAGE_CLASS}\"}}}}"

  echo "Waiting for registry deployment to roll out..."
  COUNTER=0
  while [ $COUNTER -lt 300 ]; do
    READY=$(oc get deployment image-registry -n openshift-image-registry \
      -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
    if [[ "${READY}" -ge 1 ]]; then
      echo "Registry deployment ready (${READY} replicas)"
      break
    fi
    sleep 10
    COUNTER=$((COUNTER + 10))
    echo "Waiting ${COUNTER}s for registry deployment (ready: ${READY:-0})..."
  done
  if [[ "${READY:-0}" -lt 1 ]]; then
    echo "ERROR: Registry deployment did not become ready within 300s"
    oc get deployment image-registry -n openshift-image-registry -o yaml 2>/dev/null || true
    oc get pods -n openshift-image-registry 2>/dev/null || true
    oc get pvc -n openshift-image-registry 2>/dev/null || true
    exit 1
  fi
else
...
if [[ -z "${INTERNAL_REGISTRY}" ]]; then
  echo "ERROR: Internal registry route did not appear within 180s"
...
echo "=== Configuring registry auth from cluster pull-secret ==="
for registry in registry.redhat.io quay.io quay.io/openshift-virtualization/konflux-builds; do
  ...
  if [[ -n "${AUTH_B64}" ]]; then
    ...
    echo "  Configured auth for ${registry}"
  fi
done
