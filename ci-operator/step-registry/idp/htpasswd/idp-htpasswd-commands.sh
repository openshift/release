...
function set_common_variables () {
    if [ "$IS_HYPERSHIFT_ENV" == "yes" ]; then
        OAUTH_RESOURCE="hostedcluster/$cluster_name"
        MIDDLE_NAMESPACE="$HYPERSHIFT_NAMESPACE"
        OAUTH_NAMESPACE="clusters-$cluster_name"
        IDP_FIELD=".spec.configuration.oauth.identityProviders"
    else
        OAUTH_RESOURCE="oauth/cluster"
...
function check_idp () {
    ...
    current_idp_config=$(oc get "$OAUTH_RESOURCE" -o jsonpath='{range '$IDP_FIELD'[*]}{.name}{" "}{.type}{"\n"}{end}')
...
function set_users () {
    ...
    oc get "$OAUTH_RESOURCE" -o json > "${oauth_file_src}"
