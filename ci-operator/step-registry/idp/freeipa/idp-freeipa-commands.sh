...
function configure_freeipa() {
    ...
    oc create secret generic freeipa --from-literal=IPA_DM_PASSWORD="${IPA_PASSWORD}" --from-literal=IPA_ADMIN_PASSWORD="${IPA_PASSWORD}"  -n freeipa

    #Generate ca.crt and ca.key used by ipa server.
    temp_dir=$(mktemp -d)
    openssl genrsa -out "$temp_dir"/ca.key 2048
    openssl req -key "$temp_dir"/ca.key -nodes -new -out "$temp_dir"/ca.csr -subj '/CN=freeipa.freeipa.freeipa.svc.cluster.local/O=FREEIPA'
    openssl req -x509 -new -key "$temp_dir"/ca.key -nodes -out "$temp_dir"/ca.crt -subj '/CN=freeipa.freeipa.freeipa.svc.cluster.local/O=FREEIPA' -days 365 -set_serial 1
...
spec:
  ...
  template:
    ...
    spec:
      ...
      subdomain: freeipa
      initContainers:
      - name: freeipa-openshift-container-1-step
      ...
      - name: setup-nssdb
        ...
        args:
          - |
            ...
            openssl pkcs12 -export -in /certs/tls.crt -inkey /certs/tls.key -out "$DATA"/ca.p12 -name "/CN=freeipa.freeipa.freeipa.svc.cluster.local/O=FREEIPA" -passout file:$PASSWORD_FILE
            pki -d $NSSDB -C $PASSWORD_FILE client-init
            pki -d $NSSDB pkcs12-cert-mod --pkcs12-file "$DATA"/ca.p12 "/CN=freeipa.freeipa.freeipa.svc.cluster.local/O=FREEIPA" --pkcs12-password-file $PASSWORD_FILE --trust-flags "CTu,Cu,Cu"
            pk12util -i "$DATA"/ca.p12 -d sql:$NSSDB -w $PASSWORD_FILE -k $PASSWORD_FILE
            echo -e "$RANDOM\nn\n" | certutil -C -m 2346 -i "$DATA"/ipa.csr -o "$DATA"/ipa.crt -c '/CN=freeipa.freeipa.freeipa.svc.cluster.local/O=FREEIPA' -d $NSSDB -a --extSKID -f $PASSWORD_FILE
        volumeMounts:
          - name: data
            mountPath: /data
          - name: certs
            mountPath: /certs
      containers:
      - name: freeipa-openshift-container-2-step
        image: quay.io/freeipa/freeipa-server:fedora-39
        securityContext:
          runAsUser: 0
        tty: true
...
if [ -z "$FREEIPA_POD" ]; then
    echo "No FreeIPA pod found in the 'freeipa' namespace."
else
    FREEIPA_VERSION=$(oc exec "$FREEIPA_POD" -n freeipa -- ipa --version)
    echo "$FREEIPA_VERSION"
fi
