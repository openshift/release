    ...
    exec:
      command:
        ...
        - |
          ...
          /opt/bitnami/keycloak/setup-scripts/setup-script.sh
  ...
  - |
    ...
    data:
      setup-script.sh: |
        ...
        CLIENT_OC_CLI_TEST_ID=\$(kcadm.sh get clients -r master --config=\${KCADM_CONFIG} -q clientId=oc-cli-test -o jsonpath='{.id}')
        ...
        CLIENT_CONSOLE_TEST_ID=\$(kcadm.sh get clients -r master --config=\${KCADM_CONFIG} -q clientId=console-test -o jsonpath='{.id}')
        ...
        TEST_GROUP_ID=\$(kcadm.sh get groups -r master --config=\${KCADM_CONFIG} -q name=keycloak-testgroup-1 -o jsonpath='{.id}')
        ...
        for USER in "\${USER_LIST[@]}"; do
          ...
          if ! kcadm.sh get users -r master --config=\${KCADM_CONFIG} -q username=\${TEST_USER_NAME} &> /dev/null; then
            kcadm.sh create users -r master \
              ...
              --config=\${KCADM_CONFIG}
            TEST_USER_ID=\$(kcadm.sh get users -r master --config=\${KCADM_CONFIG} -q username=\${TEST_USER_NAME} -o jsonpath='{.id}')
