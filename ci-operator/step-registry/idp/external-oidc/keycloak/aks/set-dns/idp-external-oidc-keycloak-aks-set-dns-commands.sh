...
if [ ! -f ${SHARED_DIR}/keycloak-prefix ] ; then
    echo "The keycloak-prefix fiel must be provided by a previous step!"
    exit 1
fi
...
sleep 60
curl -vSI --cacert /tmp/router-ca/tls.crt https://${KEYCLOAK_HOST}/realms/master/.well-known/openid-configuration | head -n 1 | grep -q 'HTTP/2 200'
...
if [ ! -f "${SHARED_DIR}"/oidcProviders-secret-configmap.yaml ] ; then
    echo "The oidcProviders-secret-configmap.yaml fiels must be provided by a previous step!"
    exit 1
fi
