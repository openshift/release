...
if [ ! -z ${VERSION} ]; then
  Y_VERSION=$(echo ${VERSION} | cut -d'.' -f2)
  echo "$(date -u --rfc-3339=seconds) - determined version is 4.${Y_VERSION}"
else
...
echo "Overriding Machine Pool to SNO, setting master's & worker's replicas to 0"
