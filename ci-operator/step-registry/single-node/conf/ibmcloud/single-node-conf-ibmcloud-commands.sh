...
echo "Updating install-config.yaml to a single ${SINGLE_NODE_IBMCLOUD_INSTANCE_TYPE} control plane node and 0 workers"
...
if "ibmcloud" in platform:
    platform["ibmcloud"]["type"] = "'${SINGLE_NODE_IBMCLOUD_INSTANCE_TYPE}'"
else:
    raise ValueError("This step only applies to IBM Cloud, please use the correct step for your platform")
