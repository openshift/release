...
_junit_emit() {
  # Emit a JUnit XML result for the Quay upgrade step and propagate
...
function CollectDiagnostics () {
    # Dump Quay subscription, CSV, QuayRegistry, and pod state to artifacts for debugging.
...
function ValidateQuayHealth () {
    # Check QuayRegistry availability and key Quay deployment statuses post-upgrade.
...
function Main () {
    # Orchestrate the Quay operator upgrade: resolve channel, patch subscription, wait, validate.
