...
SCAN_DIR="${JUNIT_DIR:-$ARTIFACT_DIR}"

echo "=== Skip-Ratio Health Gate ==="
echo "Scan directory : ${SCAN_DIR}"
echo "Max skip ratio : ${MAX_SKIP_RATIO}"
echo ""

# Collect junit XML files
mapfile -t JUNIT_FILES < <(find "${SCAN_DIR}" -name 'junit*.xml' -type f 2>/dev/null | sort)

if [[ ${#JUNIT_FILES[@]} -eq 0 ]]; then
    echo "ERROR: No junit*.xml files found in ${SCAN_DIR}"
    exit 1
fi

echo "Found ${#JUNIT_FILES[@]} junit file(s)."
echo ""

# Use python3 to parse JUnit XML and compute skip ratio
python3 - "${MAX_SKIP_RATIO}" "${ARTIFACT_DIR}" "${JUNIT_FILES[@]}" <<'PYEOF'
import sys
import os
