...
function CheckThanosHealth () {
    ...
    typeset -a criticalComponents=("thanos-receive" "thanos-compact" "thanos-store")
    ...
    for idx in "${!componentNames[@]}"; do
        ...
        if [[ "${podCount}" -eq 0 ]]; then
            if printf '%s\n' "${criticalComponents[@]}" | grep -qx "${component}"; then
                missingCritical+=("${component}")
            else
                missingOptional+=("${component}")
            fi
            continue
        fi
    ...
    if (( foundCount == 0 && discoveryErrors > 0 )); then
        ...
        if (( ${#missingCritical[@]} > 0 )); then
            typeset critMsg="Missing S3-critical: ${missingCritical[*]}"
            failMsg="${failMsg:+${failMsg}; }${critMsg}"
        fi
        if [[ -n "${failMsg}" ]]; then
