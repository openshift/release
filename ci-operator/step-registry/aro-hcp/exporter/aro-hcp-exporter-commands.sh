...
git checkout upstream/vars_for_prow_job -- tooling/aro-hcp-exporter/Makefile

# Resolve image variables from Makefile (uses templatize internally)
IMAGE_VARS_FILE="/tmp/image-vars.env"
make -C tooling/aro-hcp-exporter export-image-vars DEPLOY_ENV="${DEPLOY_ENV}" IMAGE_VARS_FILE="${IMAGE_VARS_FILE}"
...
TARGET_IMAGE="${IMAGE_REGISTRY}/test-imani-aro-hcp-exporter:${IMAGE_TAG}"
