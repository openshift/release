...
echo "Pushing aro-hcp-sessiongate image to ACR..."
...
echo "CI-built image: ${ARO_HCP_SESSIONGATE}"
...
git checkout upstream/vars_for_prow_job -- sessiongate/Makefile

# Resolve image variables from Makefile (uses templatize internally)
IMAGE_VARS_FILE="/tmp/image-vars.env"
make -C sessiongate export-image-vars DEPLOY_ENV="${DEPLOY_ENV}" IMAGE_VARS_FILE="${IMAGE_VARS_FILE}"
...
TARGET_IMAGE="${IMAGE_REGISTRY}/test-imani-aro-hcp-sessiongate:${IMAGE_TAG}"
...
oc image mirror "${ARO_HCP_SESSIONGATE}" "${TARGET_IMAGE}"
