...
echo "Pushing aro-hcp-frontend image to ACR..."
...
echo "CI-built image: ${ARO_HCP_FRONTEND}"
...
git checkout upstream/vars_for_prow_job -- frontend/Makefile

# Resolve image variables from Makefile (uses templatize internally)
IMAGE_VARS_FILE="/tmp/image-vars.env"
make -C frontend export-image-vars DEPLOY_ENV="${DEPLOY_ENV}" IMAGE_VARS_FILE="${IMAGE_VARS_FILE}"
...
TARGET_IMAGE="${IMAGE_REGISTRY}/test-imani-aro-hcp-frontend:${IMAGE_TAG}"
...
oc image mirror "${ARO_HCP_FRONTEND}" "${TARGET_IMAGE}"
