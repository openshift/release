...
echo "Pushing aro-hcp-admin-api image to ACR..."
...
echo "CI-built image: ${ARO_HCP_ADMIN_API}"
...
git checkout upstream/vars_for_prow_job -- admin/Makefile

# Resolve image variables from Makefile (uses templatize internally)
IMAGE_VARS_FILE="/tmp/image-vars.env"
make -C admin export-image-vars DEPLOY_ENV="${DEPLOY_ENV}" IMAGE_VARS_FILE="${IMAGE_VARS_FILE}"
...
TARGET_IMAGE="${IMAGE_REGISTRY}/test-imani-aro-hcp-admin-api:${IMAGE_TAG}"
...
oc image mirror "${ARO_HCP_ADMIN_API}" "${TARGET_IMAGE}"
