...
echo "Pushing aro-hcp-oc-mirror image to ACR..."
...
echo "CI-built image: ${ARO_HCP_OC_MIRROR}"
...
git checkout upstream/vars_for_prow_job -- image-sync/oc-mirror/Makefile

# Resolve image variables from Makefile (uses templatize internally)
IMAGE_VARS_FILE="/tmp/image-vars.env"
make -C image-sync/oc-mirror export-image-vars DEPLOY_ENV="${DEPLOY_ENV}" IMAGE_VARS_FILE="${IMAGE_VARS_FILE}"
...
TARGET_IMAGE="${IMAGE_REGISTRY}/test-imani-aro-hcp-oc-mirror:${IMAGE_TAG}"
...
TARGET_IMAGE_LATEST="${IMAGE_REGISTRY}/test-imani-aro-hcp-oc-mirror:latest"

oc image mirror "${ARO_HCP_OC_MIRROR}" "${TARGET_IMAGE}"
echo "Image pushed successfully to ${TARGET_IMAGE}"

oc image mirror "${ARO_HCP_OC_MIRROR}" "${TARGET_IMAGE_LATEST}"
echo "Image pushed successfully to ${TARGET_IMAGE_LATEST}"
