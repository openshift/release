...
mkdir "${ARTIFACT_DIR}/${CUSTOMER_RG_NAME}"
az deployment group list --resource-group "${CUSTOMER_RG_NAME}" -o yaml > "${ARTIFACT_DIR}/${CUSTOMER_RG_NAME}/deployment-group-list.yaml"

# List all deployments in the specified resource group and extract their names
deployment_names=$(az deployment group list --resource-group "${CUSTOMER_RG_NAME}" --query "[].name" -o tsv)

# Check if any deployments were found
if [ -z "$deployment_names" ]; then
    echo "No deployments found in resource group: ${CUSTOMER_RG_NAME}"
    exit 0
fi

echo "Deployment operations for resource group: ${CUSTOMER_RG_NAME}"
echo "--------------------------------------------------------"

# Loop through each deployment name and list its operations
for deployment_name in $deployment_names; do
    az deployment operation group list --resource-group "${CUSTOMER_RG_NAME}" --name "$deployment_name" -o yaml > "${ARTIFACT_DIR}/${CUSTOMER_RG_NAME}/deployment-operation-${deployment_name}.yaml"

    echo "Deployment: $deployment_name"
    az deployment operation group list --resource-group "${CUSTOMER_RG_NAME}" --name "$deployment_name" -o table
    echo "--------------------------------------------------------"
done