...
# Variant of opendatahub-io-ci-image-mirror for use inside a generic-claim
# workflow. In a generic-claim job, $KUBECONFIG points to the claimed cluster
# (which uses client-cert auth with no OAuth token), so `oc registry login`
# fails. This ref clears $KUBECONFIG before that call so `oc` falls back to the
# build cluster's service account token for CI registry authentication.
#
# Use this ref instead of opendatahub-io-ci-image-mirror when the image mirror
# step runs as part of a job that claims a cluster via generic-claim (or any
# workflow that sets $KUBECONFIG to a non-build cluster).
...
# Clear $KUBECONFIG so oc uses the build cluster's service account token
# instead of the claimed cluster's client-cert kubeconfig.
