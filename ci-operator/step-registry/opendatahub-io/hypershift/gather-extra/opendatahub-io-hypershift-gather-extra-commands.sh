...
# HyperShift Hosted Cluster Gather-Extra
# This script runs the standard gather-extra collection against the hosted cluster
# Based on ci-operator/step-registry/gather/extra/gather-extra-commands.sh

set -euo pipefail

echo "Switching to hosted cluster kubeconfig for gather-extra collection"
export KUBECONFIG="${SHARED_DIR}/nested_kubeconfig"

# Verify we can reach the hosted cluster
if ! oc whoami --show-server; then
  echo "WARNING: Hosted cluster not reachable; skipping gather-extra collection"
  exit 0
fi

echo "Running standard gather-extra collection against hosted cluster..."

# === BEGIN STANDARD GATHER-EXTRA LOGIC ===
# The following is copied from gather/extra/gather-extra-commands.sh
...
# Explicitly inspect operator namespace since operator-sdk installation doesn't link to cluster operators
if [[ -n "${OO_INSTALL_NAMESPACE:-}" ]]; then
  echo "Explicitly inspecting operator namespace: ${OO_INSTALL_NAMESPACE}"
  oc --insecure-skip-tls-verify --request-timeout=5s adm inspect ns/${OO_INSTALL_NAMESPACE} --dest-dir ${ARTIFACT_DIR}/inspect || true
fi
...
# NOTE: Machine configs not available in HyperShift hosted clusters (managed by management cluster)
...
# NOTE: Machine sets and control plane machines not available in HyperShift hosted clusters
...
# NOTE: Machine resources not available in HyperShift hosted clusters
...
# NOTE: AWS/Infrastructure CAPI resources not available in HyperShift hosted clusters
...
queue ${ARTIFACT_DIR}/customresourcedefinitions.json oc --insecure-skip-tls-verify --request-timeout=5s get customresourcedefinitions -o json
queue ${ARTIFACT_DIR}/oc_cmds/customresourcedefinitions oc --insecure-skip-tls-verify --request-timeout=5s get customresourcedefinitions
queue ${ARTIFACT_DIR}/validatingwebhookconfigurations.json oc --insecure-skip-tls-verify --request-timeout=5s get validatingwebhookconfigurations -o json
queue ${ARTIFACT_DIR}/oc_cmds/validatingwebhookconfigurations oc --insecure-skip-tls-verify --request-timeout=5s get validatingwebhookconfigurations
queue ${ARTIFACT_DIR}/mutatingwebhookconfigurations.json oc --insecure-skip-tls-verify --request-timeout=5s get mutatingwebhookconfigurations -o json
queue ${ARTIFACT_DIR}/oc_cmds/mutatingwebhookconfigurations oc --insecure-skip-tls-verify --request-timeout=5s get mutatingwebhookconfigurations
...
# NOTE: No master nodes in HyperShift hosted clusters - control plane runs in management cluster
...
# NOTE: Multus logs collection not available in HyperShift hosted clusters
...
# NOTE: tcpdump/conntrackdump collection not available in HyperShift hosted clusters
...
# NOTE: etcd runs in management cluster, not accessible from hosted cluster
