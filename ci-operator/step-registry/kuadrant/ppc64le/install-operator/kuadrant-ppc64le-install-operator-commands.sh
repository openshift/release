...
# otherwise create the CoreDNS provider Secret used on ppc64le reference clusters.
for ns in kuadrant kuadrant2; do
  if [[ -f "${DNS_CREDS_DIR}/secret.yaml" ]]; then
    oc apply -n "${ns}" -f "${DNS_CREDS_DIR}/secret.yaml"
  else
    echo "Creating CoreDNS provider Secret ${DNS_PROVIDER_SECRET_NAME} in ${ns} (zone ${COREDNS_ZONE}, ppc64le pattern)"
    cat <<EOF | oc apply -f -
apiVersion: v1
kind: Secret
metadata:
