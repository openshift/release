...
# Runs the Kuadrant testsuite as an in-cluster Job on the ppc64le cluster.
#
# Bastion-proven ppc64le workarounds applied in-Job before make:
#   - Velocity MockServer echo (header-mirroring; Kuadrant/testsuite#952)
#   - Authorino/OIDC dataplane-ready soft-wait patch
#   - protobuf==6.32.1 pin (broken ppc64le upb >=6.33.0)
#   - CFSSL ensure (baked in Dockerfile.ppc64le; fallback download if missing)
#   - CoreDNS getaddrinfo plugin + UI ignore
#
# Why: the ci-operator step pod lives on the amd64 build farm, on a different
# network than the leased ppc64le cluster. It cannot resolve the private CoreDNS
# zone (*.k.example.com) nor reach the Gateway MetalLB IPs (192.168.x.240).
#
# The reference ppc64le LPAR works because the client runs *on* the cluster
# network. We reproduce that here by running the testsuite as a Job on a ppc64le
# worker node using a ppc64le-native testsuite image. From inside the cluster:
...
TESTSUITE_PPC64LE_IMAGE="${TESTSUITE_PPC64LE_IMAGE:-quay.io/vray_rh/rhcl-testsuite:stablev1-ppc64le}"
...
# ---------------------------------------------------------------------------
# 2a. Velocity MockServer expectations (ppc64le workaround for testsuite#952)
#     Mounted into the Job and copied over the upstream JAVASCRIPT file before
#     make smoke. Proven on ppc64le with mockserver-ppc64le-fixed-v10.
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 2a2. Authorino/OIDC dataplane-ready soft-wait (ppc64le race after AuthPolicy Enforced)
...
# ---------------------------------------------------------------------------
DATAPLANE_PATCH_FILE="${WORK_DIR}/kuadrant-ppc64le-run-testsuite-dataplane-ready.patch"
...
# ---------------------------------------------------------------------------
# protobuf >=6.33.0 ships broken ppc64le upb wheels (protocolbuffers/protobuf#24103).
...
# Ignore UI: the ppc64le testsuite image omits Playwright; collecting
...
rc=0
echo '=== Pinning protobuf==${PROTOBUF_PIN} for ppc64le (avoid broken upb >=6.33.0) ==='
...
# ppc64le workaround for Kuadrant/testsuite#952 (JAVASCRIPT/GraalJS -> Velocity).
# File is mounted from ConfigMap kuadrant-testrunner-hook (see step prep below).
echo '=== ppc64le workaround: install Velocity MockServer echo expectations ==='
...
echo '=== Applying ppc64le dataplane-ready testsuite patch ==='
if grep -q 'AUTH_DATAPLANE_READY_TIMEOUT' testsuite/utils/constants.py 2>/dev/null; then
  ...
  if command -v git >/dev/null 2>&1; then
    git apply --verbose /kuadrant-hook/kuadrant-ppc64le-run-testsuite-dataplane-ready.patch
  elif command -v patch >/dev/null 2>&1; then
    patch -p1 < /kuadrant-hook/kuadrant-ppc64le-run-testsuite-dataplane-ready.patch
  else
...
# CFSSL: Dockerfile.ppc64le installs /usr/bin/cfssl; re-install if the baked image is older.
echo '=== Ensuring cfssl is on PATH ==='
if ! command -v cfssl >/dev/null 2>&1; then
  echo 'cfssl missing; downloading ppc64le binary from cloudflare/cfssl v1.6.5'
  mkdir -p /tmp/bin
  curl -fsSL -o /tmp/bin/cfssl \\
    https://github.com/cloudflare/cfssl/releases/download/v1.6.5/cfssl_1.6.5_linux_ppc64le
...
# (ppc64le workaround for Kuadrant/testsuite#952).
oc -n "${TEST_RUNNER_NAMESPACE}" create configmap kuadrant-testrunner-hook \
  ...
  --from-file=kuadrant-ppc64le-run-testsuite-dataplane-ready.patch="${DATAPLANE_PATCH_FILE}" \
  --dry-run=client -o yaml | oc apply -f -
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
      - name: testsuite
        image: ${TESTSUITE_PPC64LE_IMAGE}
        # Always: same tag may be rebuilt; avoid stale node cache.
...
if [[ -n "${RUNNER_POD}" ]]; then
  ...
  # Follow with restarts: a single `oc logs -f` often dies on long ppc64le runs
  # and previously left us blind.
  follow_rounds=0
  while true; do
