...
- name: Load key and instance type from environment
  set_fact:
    ssh_key_name: "{{ lookup('env', 'SSH_KEY_NAME') | default('', true) }}"
    instance_type: "{{ lookup('env', 'RHEL_VM_TYPE') | default('m6a.xlarge', true) }}"
