...
if [[ ! ("$branch" =~ ^release-[0-9]+\.[0-9]+$ || "$branch" =~ ^backplane-[0-9]+\.[0-9]+$ || "$branch" =~ ^global-hub-[0-9]+\.[0-9]+$) ]]; then
    log "ERROR Branch $branch is not a release, backplane or global-hub branch."
    log "      Base branch of PR must match release-x.y, backplane-x.y or global-hub-x.y"
    exit 1
fi
