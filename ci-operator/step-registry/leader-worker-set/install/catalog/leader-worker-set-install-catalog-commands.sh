...
function install_oc_mirror () {
    ...
    run_command "curl -k -fL -o oc-mirror.tar.gz ${CGWURL}/oc-mirror/latest/oc-mirror-rhel9-linux-amd64.tar.gz"
...
function check_catalog_readiness () {
    ...
    echo "Storing the catalog source name to '${SHARED_DIR}/lws_catsrc_name'..."
    echo "${CS_CATSRC_NAME}" > "${SHARED_DIR}"/lws_catsrc_name
}
