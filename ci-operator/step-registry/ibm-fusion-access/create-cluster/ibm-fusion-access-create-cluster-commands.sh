...
jq -cn \
  ...
  '
  {
    ...
    spec: {
      ...
      pmcollector: { nodeSelector: { "scale.spectrum.ibm.com/role": "storage" } },
      daemon: {
        nodeSelector: { "scale.spectrum.ibm.com/role": "storage" },
        nsdDevicesConfig: { localDevicePaths: [{ devicePath: "/dev/disk/by-id/*", deviceType: "generic" }] },
        clusterProfile: {
