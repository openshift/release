...
if [[ -f "$FA__PULL_SECRET_EXTRA_PATH" ]]; then
  echo "✅ Additional pull secret credentials found at: $FA__PULL_SECRET_EXTRA_PATH"
  FA__PULL_SECRET_EXTRA="$(cat "$FA__PULL_SECRET_EXTRA_PATH")"
  echo "✅ FA__PULL_SECRET_EXTRA environment variable set from mounted secret"
else
  echo "❌ Additional pull secret credentials not found at: $FA__PULL_SECRET_EXTRA_PATH"
fi
...
if [[ "$IBM_ENTITLEMENT_AVAILABLE" == "false" ]]; then
  ...
  echo ""
  echo "IBM entitlement credentials are required for accessing IBM registry images."
  echo "This will cause test failures later - failing fast as per fail-fast philosophy."
  exit 1
fi
...
if oc get secret fusion-pullsecret -n "${FUSION_ACCESS_NAMESPACE}" >/dev/null; then
  ...
  echo "  - Target registry: ${FA__IBM_REGISTRY}"
...
")
  ...
  create_entitlement_secret_in_namespace() {
    ...
    if oc get secret ibm-entitlement-key -n "${target_namespace}" >/dev/null; then
...
EOF
    then
      oc wait --for=jsonpath='{.metadata.name}'=ibm-entitlement-key secret/ibm-entitlement-key -n ${target_namespace} --timeout=60s >/dev/null
  ...
  for ns in "${FA__SCALE__NAMESPACE}" "${STORAGE_SCALE_DNS_NAMESPACE}" "${STORAGE_SCALE_CSI_NAMESPACE}" "${STORAGE_SCALE_OPERATOR_NAMESPACE}"; do
  ...
  echo "2. The credentials should provide access to ${FA__IBM_REGISTRY} registry"
fi
...
if oc get secret fusion-pullsecret-extra -n "${FUSION_ACCESS_NAMESPACE}" >/dev/null; then
  ...
  if [[ -n "${FA__PULL_SECRET_EXTRA:-}" ]]; then
    ...
    echo "  - Key length: ${#FA__PULL_SECRET_EXTRA} characters"
    echo "  - Key format: ${FA__PULL_SECRET_EXTRA:0:10}... (first 10 chars)"
...
EOF
    ...
    echo "❌ ERROR: Additional pull secret credentials not found"
    echo "Expected location: $FA__PULL_SECRET_EXTRA_PATH"
    echo ""
    echo "Additional pull secret is required for accessing quay.io/openshift-storage-scale images."
    echo "This will cause test failures later - failing fast as per fail-fast philosophy."
    exit 1
  fi
fi
...
if oc get secret fusion-pullsecret -n "${FUSION_ACCESS_NAMESPACE}" >/dev/null; then
  ...
  echo "✅ fusion-pullsecret: Created (supports ${FA__IBM_REGISTRY})"
