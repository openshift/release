...
elif [ -z "$MULTISTAGE_PARAM_OVERRIDE_INDEX_IMAGE" ]; then
  echo "Installing from custom catalog source $QUAY_OPERATOR_SOURCE, but not provoide index image: $MULTISTAGE_PARAM_OVERRIDE_INDEX_IMAGE"
  exit 1
else #Install Quay operator with fbc image
  echo "Installing Quay from unreleased fbc image: $MULTISTAGE_PARAM_OVERRIDE_INDEX_IMAGE"
  update_pull_secret
  create_icsp
