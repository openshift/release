...
ARTIFACT_DIR_NEW_UI="${ARTIFACT_DIR}/new_ui/"
mkdir -p ${ARTIFACT_DIR_NEW_UI}
...
if [ -d "./cypress/results/" ] && [ "$(ls -A "./cypress/results/")" ]; then
    cp -r ./cypress/results/* ${ARTIFACT_DIR_NEW_UI}
    for file in "${ARTIFACT_DIR_NEW_UI}"/*; do
        if [[ ! "$(basename "$file")" =~ ^"$JUNIT_PREFIX" ]]; then
            mv "$file" "${ARTIFACT_DIR_NEW_UI}"/"$JUNIT_PREFIX""$(basename "$file")"
        fi
    done
fi

if [ -d "./cypress/videos/" ] && [ "$(ls -A "./cypress/videos/")" ]; then
    cp -r ./cypress/videos/* ${ARTIFACT_DIR_NEW_UI}
fi

if [ -d  "./cypress/logs/" ] && [ "$(ls -A "./cypress/logs/")" ]; then
    cp -r ./cypress/logs/* ${ARTIFACT_DIR_NEW_UI}
fi

if [[ -e "./stage_quay_io_testing_report.xml" ]]; then
    cp -r "./stage_quay_io_testing_report.xml" ${ARTIFACT_DIR_NEW_UI}
fi
