...
resource "aws_instance" "quaybuilder" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo podman login -u '${QUAY_BREW_USERNAME}' -p ${QUAY_BREW_PASSWORD} brew.registry.redhat.io && sudo trivy image brew.registry.redhat.io/rh-osbs/quay-quay-rhel8:v3.12.1-8"
    ]
  }
