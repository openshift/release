...
set -x
cd new-ui-tests 
for path in $(which -a node); do echo "$path: $($path -v)"; done || true
for path in $(which -a npm); do echo "$path: $($path -v)"; done || true
for path in $(which -a npx); do echo "$path: $($path -v)"; done || true
node -v
which node
rpm -qf "$(which node)"
which npm
