...
NO_COLOR=1 npm run cypress run -b chrome --reporter cypress-multi-reporters --reporter-options configFile=reporter-config.json --env grepTags="${NEW_UI_TESTING_COVERAGE}",grepFilterSpecs=true || true 

npm run jrm ./quay_new_ui_testing_report.xml ./cypress/results/quay_new_ui_testing_report-* || true
