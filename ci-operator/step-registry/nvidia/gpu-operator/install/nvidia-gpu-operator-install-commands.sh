...
for i in $(seq 1 40); do
  ...
  if [ "${CSV_PHASE}" == "Succeeded" ]; then
    CSV_VERSION=$(oc get csv -n nvidia-gpu-operator "${CSV_NAME}" -o jsonpath='{.spec.version}' 2>/dev/null || echo "unknown")
    echo "  CSV is ready: ${CSV_NAME}"
    echo "  GPU Operator version: ${CSV_VERSION}"
    break
  fi
...
# Step 9: Create ClusterPolicy from CSV defaults
echo "Step 9: Creating ClusterPolicy from CSV alm-examples..."

# Get the example ClusterPolicy from CSV
oc get csv -n nvidia-gpu-operator "${CSV_NAME}" -o jsonpath='{.metadata.annotations.alm-examples}' | \
  jq '.[0]' > /tmp/clusterpolicy.json

# Configure ClusterPolicy based on use case
# The CSV alm-examples already contains correct defaults for:
# - use_ocp_driver_toolkit: true (for OpenShift DTK)
# - CDI configuration
# - Driver configuration
# - defaultRuntime auto-detection (field is deprecated)

if [ "${NVIDIA_DRA_DYNAMIC_MIG:-false}" == "true" ]; then
  # Dynamic MIG mode for DRA partitionable devices
  # Reference: https://github.com/empovit/nvidia-dra-driver-verification/blob/main/dynamic-mig.md
  echo "Configuring for Dynamic MIG mode (DRA manages MIG dynamically)"
  cat /tmp/clusterpolicy.json | \
    jq '.spec.devicePlugin.enabled = false |
        .spec.mig.strategy = "none" |
        .spec.migManager.enabled = false' > /tmp/clusterpolicy-modified.json
  oc apply -f /tmp/clusterpolicy-modified.json
elif [ -n "${MIG_STRATEGY}" ]; then
  # Static MIG mode with device plugin
  echo "Configuring for static MIG strategy: ${MIG_STRATEGY}"
  cat /tmp/clusterpolicy.json | \
    jq --arg mig_strategy "${MIG_STRATEGY}" \
      '.spec.mig.strategy = $mig_strategy' > /tmp/clusterpolicy-modified.json
  oc apply -f /tmp/clusterpolicy-modified.json
else
  # Use CSV defaults (no MIG)
  echo "Using CSV defaults without modifications"
...
if ! oc wait clusterpolicy --all --for=condition=Ready --timeout="${TIMEOUT}"; then
...
# Step 11: Verify CDI is enabled
echo "Step 11: Verifying ClusterPolicy configuration..."
CDI_ENABLED=$(oc get clusterpolicy gpu-cluster-policy -o jsonpath='{.spec.cdi.enabled}')
DRIVER_ENABLED=$(oc get clusterpolicy gpu-cluster-policy -o jsonpath='{.spec.driver.enabled}')
...
if [ "${DRIVER_ENABLED}" == "true" ]; then
  ...
  echo "✗ ERROR: Driver is not enabled: ${DRIVER_ENABLED}"
  exit 1
fi

echo "✓ Default runtime: ${DEFAULT_RUNTIME}"

# Step 12: Verify GPU nodes are labeled
echo ""
echo "Step 12: Verifying GPU nodes are labeled..."
...
echo "  - CSV: ${CSV_NAME}"
echo "  - Version: ${CSV_VERSION:-unknown}"
