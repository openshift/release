...
append_report_link_to_pr() {
  ...
  if [ -n "${BUILD_ID:-}" ] && [ -n "${JOB_NAME:-}" ]; then
    if [ "${JOB_TYPE:-}" = "periodic" ]; then
      report_url="https://gcs.ci.openshift.org/gcs/test-platform-results-public/logs/${JOB_NAME}/${BUILD_ID}/artifacts/periodic-jira-agent/jira-agent-report/artifacts/jira-agent-report.html"
    else
      report_url="https://gcs.ci.openshift.org/gcs/test-platform-results-public/pr-logs/pull/openshift_release/${PULL_NUMBER:-0}/${JOB_NAME}/${BUILD_ID}/artifacts/periodic-jira-agent/jira-agent-report/artifacts/jira-agent-report.html"
    fi
  fi
