...
if [[ "${TEST_TARGET}" == "snapshots-test" ]]; then
    GITHUB_APP_ID=$(cat "/var/run/vault/capi-versioning-automation/github_app_id")
    GITHUB_APP_INSTALLATION_ID=$(cat "/var/run/vault/capi-versioning-automation/github_app_installation_id")
    GITHUB_APP_PRIVATE_KEY_PATH="/var/run/vault/capi-versioning-automation/github_app_private_key.pem"
