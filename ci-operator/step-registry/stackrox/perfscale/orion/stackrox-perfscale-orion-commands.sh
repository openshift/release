...
# Extract UUID from the cluster-density-v2 index_data.json if it exists
if [ -f "${SHARED_DIR}/cluster-density-v2-index_data.json" ]; then
    export UUID=$(jq -r '.uuid' "${SHARED_DIR}/cluster-density-v2-index_data.json")
    echo "Extracted UUID from cluster-density-v2: ${UUID}"
else
    echo "Warning: cluster-density-v2-index_data.json not found in SHARED_DIR"
fi
