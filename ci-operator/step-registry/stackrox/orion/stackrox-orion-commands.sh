...
# Baseline isolation: for presubmit jobs with a workload UUID, query OpenSearch
# for periodic-only UUIDs and use --uuid/--baseline to compare the current run
# exclusively against nightly runs (no other PR runs in the baseline).
WORKLOAD_UUID_FILE="${SHARED_DIR}/workload-uuid"
if [[ -f "${WORKLOAD_UUID_FILE}" ]] && [[ "${JOB_TYPE}" == "presubmit" ]]; then
    WORKLOAD_UUID=$(<"${WORKLOAD_UUID_FILE}")
    echo "Baseline isolation: workload UUID is ${WORKLOAD_UUID}"

    LOOKBACK_DAYS="${LOOKBACK:-15}"

    BASELINE_QUERY=$(python3 - "${ORION_CONFIG}" "${LOOKBACK_DAYS}" <<'PYEOF'
import yaml, json, sys, os, re

config_path = sys.argv[1]
lookback_days = sys.argv[2]
...
def resolve_template(match):
    ...
    var_name = expr.split('|')[0].strip() if '|' in expr else expr
    dm = re.search(r"default\(['\"](.+?)['\"]\)", expr) if '|' in expr else None
    default_val = dm.group(1) if dm else ''
    return os.environ.get(var_name,
        os.environ.get(var_name.upper(),
        os.environ.get(var_name.lower(), default_val)))

content = re.sub(r'\{\{(.+?)\}\}', resolve_template, content)
...
must = [
    {'term': {'jobType': 'periodic'}},
    {'range': {'timestamp': {'gte': f'now-{lookback_days}d/d'}}}
]
for key, value in metadata.items():
    if key in ('not', 'jobType'):
        continue
    if value is None or value == '':
        continue
    if key == 'ocpVersion':
        must.append({'prefix': {key: str(value)}})
    else:
        must.append({'term': {key: value}})

must_not = [{'term': {k: v}} for k, v in metadata.get('not', {}).items()]
...
PYEOF
    ...
    if [[ -n "${BASELINE_QUERY}" ]]; then
        echo "Querying OpenSearch for periodic baseline UUIDs (last ${LOOKBACK_DAYS} days)..."

        [[ $- == *x* ]] && _was_tracing=true || _was_tracing=false
        set +x
        BASELINE_RESPONSE=$(curl -s --fail -k \
            -H "Content-Type: application/json" \
            "${ES_SERVER}/${ES_METADATA_INDEX}/_search" \
            -d "${BASELINE_QUERY}") || true
        $_was_tracing && set -x

        if [[ -n "${BASELINE_RESPONSE}" ]]; then
            BASELINE_UUIDS=$(echo "${BASELINE_RESPONSE}" | python3 -c "
import json, sys
data = json.loads(sys.stdin.read())
current_uuid = sys.argv[1]
hits = data.get('hits', {}).get('hits', [])
uuids = sorted(set(
    h['_source']['uuid'] for h in hits
    if 'uuid' in h.get('_source', {})))
uuids = [u for u in uuids if u != current_uuid]
if uuids:
    print(','.join(uuids))
else:
    sys.exit(1)
" "${WORKLOAD_UUID}") || true
            ...
            if [[ -n "${BASELINE_UUIDS}" ]]; then
                BASELINE_COUNT=$(echo "${BASELINE_UUIDS}" | tr ',' '\n' | wc -l)
                echo "Baseline isolation active: comparing current run against ${BASELINE_COUNT} periodic runs."
                EXTRA_FLAGS+=" --uuid ${WORKLOAD_UUID} --baseline ${BASELINE_UUIDS}"
            else
                echo "WARNING: No periodic baseline UUIDs found. Falling back to default Orion behavior."
            fi
        else
            echo "WARNING: OpenSearch query returned no response. Falling back to default Orion behavior."
        fi
    else
        echo "WARNING: Failed to build baseline query from Orion config. Falling back to default Orion behavior."
    fi
fi
