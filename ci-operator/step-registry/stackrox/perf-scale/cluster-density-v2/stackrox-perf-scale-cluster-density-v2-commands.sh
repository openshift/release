...
REPO_URL="https://github.com/davdhacs/e2e-benchmarking";
LATEST_TAG=$(curl -s "https://api.github.com/repos/davdhacs/e2e-benchmarking/releases/latest" | jq -r '.tag_name');
TAG_OPTION="--branch $(if [ "$E2E_VERSION" == "default" ]; then echo "$LATEST_TAG"; else echo "$E2E_VERSION"; fi)";
git clone $REPO_URL $TAG_OPTION --depth 1 \
  || git clone $REPO_URL --branch rox-26061 --depth 1
...
ES_SERVER="" ITERATIONS=${current_worker_count} CHURN=false bash -x ./run.sh

# The measurable run
iteration_multiplier=$(($ITERATION_MULTIPLIER_ENV))
export ITERATIONS=$(($iteration_multiplier*$current_worker_count))

echo skip export ES_SERVER="https://$ES_USERNAME:$ES_PASSWORD@$ES_HOST"
