...
gcloud compute images create crio-setup-fedora-test-${currentDate} \
    --source-disk="${disk_name}" \
    --family="crio-setup-fedora-test" \
    --source-disk-zone=${ZONE} \
    --project="openshift-node-devel"
# Delete images older than 2 weeks
images=$(gcloud compute images list --project="openshift-node-devel" --filter="family:crio-setup-fedora-test AND creationTimestamp<$(date -d '2 weeks ago' +%Y-%m-%dT%H:%M:%SZ)" --format="value(name)")
if [ -n "$images" ]; then
