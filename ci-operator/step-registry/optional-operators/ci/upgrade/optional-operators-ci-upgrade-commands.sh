...
INSTALL_NAMESPACE=$(cat "${SHARED_DIR}"/oo-install-namespace)
...
oc -n "$INSTALL_NAMESPACE" patch subscription "${SUB}" --type merge --patch '{"spec":{"channel":"'"${CHANNEL}"'","installPlanApproval":"Automatic"}}'
