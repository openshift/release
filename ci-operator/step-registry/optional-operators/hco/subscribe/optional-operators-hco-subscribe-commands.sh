...
OO_INSTALL_NAMESPACE="${OO_INSTALL_NAMESPACE:-kubevirt-hyperconverged}"
...
OO_TARGET_NAMESPACES="${OO_TARGET_NAMESPACES:-kubevirt-hyperconverged}"
...
OPERATOR_GROUP=$(oc -n "${OO_INSTALL_NAMESPACE}" get operatorgroup -o jsonpath="{.items[*].metadata.name}" || true)

if [[ $(echo "${OPERATOR_GROUP}" | wc -w) -gt 1 ]]; then
    echo "Error: multiple OperatorGroups in namespace \"${OO_INSTALL_NAMESPACE}\": ${OPERATOR_GROUP}" 1>&2
    oc -n "${OO_INSTALL_NAMESPACE}" get operatorgroup -o yaml >"$ARTIFACT_DIR/operatorgroups-${OO_INSTALL_NAMESPACE}.yaml"
    exit 1
elif [[ -n "${OPERATOR_GROUP}" ]]; then
    echo "OperatorGroup \"${OPERATOR_GROUP}\" exists: modifying it"
    oc -n "${OO_INSTALL_NAMESPACE}" get operatorgroup "${OPERATOR_GROUP}" -o yaml >"${ARTIFACT_DIR}/og-${OPERATOR_GROUP}-orig.yaml"
    OG_OPERATION=apply
    OG_NAMESTANZA="name: ${OPERATOR_GROUP}"
else
    echo "OperatorGroup does not exist: creating it"
    OG_OPERATION=create
    OG_NAMESTANZA="name: ${OO_INSTALL_NAMESPACE}-group"
fi
...
echo "OperatorGroup name is \"$OPERATOR_GROUP\""
...
OG_ART="$ARTIFACT_DIR/og-${OPERATOR_GROUP}.yaml"
echo "Dumping OperatorGroup ${OPERATOR_GROUP} as $OG_ART"
oc get -n "${OO_INSTALL_NAMESPACE}" operatorgroup "${OPERATOR_GROUP}" -o yaml >"$OG_ART"
