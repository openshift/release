...
if [[ $IF_SERIAL_ORDER == "True" ]];then
   #mark application outages scenario done
   while [ "$(oc get ns | grep -c 'serial-start')" -lt 1 ]; do
       echo "One chaos testing scenario is running, wait for next round"
       sleep 30
   done
fi
...
echo "Finished running pod network chaos scenarios"
echo "Return code: $rc"
