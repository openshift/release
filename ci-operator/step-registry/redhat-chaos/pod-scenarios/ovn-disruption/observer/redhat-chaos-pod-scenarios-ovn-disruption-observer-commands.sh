...
echo "starting ovn pod disruption scenario observer for ovnkube-node pods on worker nodes"

# Get worker node names
worker_nodes=$(oc get nodes -l node-role.kubernetes.io/worker= -o jsonpath='{.items[*].metadata.name}')

# Check if worker nodes were found
...
# Get ovnkube-node pods running on worker nodes and label them
echo "Finding and labeling ovnkube-node pods on worker nodes..."
...
for node in $worker_nodes; do
  oc get pods -A -l app=ovnkube-node -o wide | grep "$node" | awk '{print $1, $2}' | while read namespace name; do
    ...
    oc label pod $name -n $namespace ovnkube-node-worker --overwrite
  done
done

echo "Finished labeling ovnkube-node pods on worker nodes"

# Select the first worker node for region detection
node_name=$(echo $worker_nodes | awk '{print $1}')

# Get the region label for the selected worker node
...
echo "Finished running ovn pod disruption observer"
echo "Return code: $rc"
exit $rc 