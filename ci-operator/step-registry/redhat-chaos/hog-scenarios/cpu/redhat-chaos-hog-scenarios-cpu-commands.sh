...
echo ""
echo "=========================================="
echo "Starting cpu-hog scenario"
echo "=========================================="
...
if [[ $rc -ne 0 ]]; then
    ...
    echo ""
    echo "Looking for cpu-hog pods in namespace ${TARGET_NAMESPACE}:"
    oc get pods -n ${TARGET_NAMESPACE} -l scenario=cpu-hog -o wide || echo "No cpu-hog pods found with label"
    oc get pods -n ${TARGET_NAMESPACE} | grep -i "cpu-hog" || echo "No cpu-hog pods found by name"
    
    echo ""
    echo "Getting all pods in ${TARGET_NAMESPACE} namespace:"
    oc get pods -n ${TARGET_NAMESPACE} -o wide
    
    # Try to find and describe any cpu-hog related pods
    for pod in $(oc get pods -n ${TARGET_NAMESPACE} -o name | grep -i "cpu-hog" || true); do
        echo ""
        echo "=========================================="
