...
if [[ $rc -ne 0 ]]; then
    ...
    echo ""
    echo "Looking for io-hog pods in namespace ${TARGET_NAMESPACE}:"
    oc get pods -n ${TARGET_NAMESPACE} -l scenario=io-hog -o wide || echo "No io-hog pods found with label"
    oc get pods -n ${TARGET_NAMESPACE} | grep -i "io-hog" || echo "No io-hog pods found by name"
    
    echo ""
    echo "Getting all pods in ${TARGET_NAMESPACE} namespace:"
    oc get pods -n ${TARGET_NAMESPACE} -o wide
    
    # Try to find and describe any io-hog related pods
    for pod in $(oc get pods -n ${TARGET_NAMESPACE} -o name | grep -i "io-hog" || true); do
        echo ""
        echo "=========================================="
