...
function check_private() {
  ...
  cmd="gcloud --project openshift-qe dns record-sets list --zone ${pub_zone_name} --format=\"table(type,name,rrdatas)\" --filter=\"(type=A OR type=SRV) AND NOT name~rh-api\" 2>/dev/null | tee ${tmp_output}"
  ...
  if [ ${num_priv_ip} -lt 2 ]; then
    logger "ERROR" "One or more api/*.apps dns record-sets in public zone '${pub_zone_name}' are with non-private IP address."
    return 1
  fi
  cmd="gcloud --project openshift-qe dns record-sets list --zone ${priv_zone_name} --format=\"table(type,name,rrdatas)\" --filter=\"(type=A OR type=SRV) AND NOT name~rh-api\" 2>/dev/null | tee ${tmp_output}"
  logger "INFO" "Running Command '${cmd}'"
  eval "${cmd}"
  num_priv_ip=$(grep -P "\s+10\.[0-9\.]+$" "${tmp_output}" | wc -l)
...
if ! check_private; then
    logger "ERROR" "Failed on post-installation check whether it is a private cluster."
    exit 1
fi