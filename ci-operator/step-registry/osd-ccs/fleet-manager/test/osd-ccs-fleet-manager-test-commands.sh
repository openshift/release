...
if [ "$NO_OF_AVAILABLE_OVERPROVISIONING_DEPL" -ne "$DESIRED_OVERPROV_REPLICAS" ] || [ "$DESIRED_OVERPROV_REPLICAS" -ne "$NO_OF_AVAILABLE_OVERPROVISIONING_DEPL_CONFIG" ]; then
  echo "Expected number of overprovisioning replicas: $NO_OF_AVAILABLE_OVERPROVISIONING_DEPL to match deployed replicas: $DESIRED_OVERPROV_REPLICAS and deployment config value: $NO_OF_AVAILABLE_OVERPROVISIONING_DEPL_CONFIG. Exiting..."
  exit 1
fi
...
if [ "$NO_OF_AVAILABLE_OVERPROVISIONING_AUTOSCALER_DEPL_CONFIG" -ne "$NO_OF_AVAILABLE_OVERPROVISIONING_AUTOSCALER_DEPL" ]; then
  echo "Expected number of overprovisioning replicas in the config: $NO_OF_AVAILABLE_OVERPROVISIONING_AUTOSCALER_DEPL_CONFIG to match available replicas: $NO_OF_AVAILABLE_OVERPROVISIONING_AUTOSCALER_DEPL. Exiting..."
  exit 1
fi
...
if [ "$NO_OF_RUNNING_OVERPROVISIONING_PODS" -ne "$NO_OF_AVAILABLE_OVERPROVISIONING_DEPL_CONFIG" ]; then
  echo "Expected number of overprovisioning running pods: $NO_OF_RUNNING_OVERPROVISIONING_PODS to match deployment config value: $NO_OF_AVAILABLE_OVERPROVISIONING_DEPL_CONFIG. Exiting..."
  exit 1
fi
...
if [ "$NO_OF_RUNNING_OVERPROVISIONING_AUTOSCALER_PODS" -ne "$NO_OF_AVAILABLE_OVERPROVISIONING_AUTOSCALER_DEPL_CONFIG" ]; then
  echo "Expected number of overprovisioning-autoscaler running pods: $NO_OF_RUNNING_OVERPROVISIONING_AUTOSCALER_PODS to match deployment config value: $NO_OF_AVAILABLE_OVERPROVISIONING_AUTOSCALER_DEPL_CONFIG. Exiting..."
  exit 1
fi
...
if [ "$CL_PROP_AUTOSCALER_CRB" -ne 1 ]; then
  echo "cluster-proportional-autoscaler ClusterRoleBinding not found. Exiting..."
  exit 1
fi
...
if [ "$CL_PROP_AUTOSCALER_CR" -ne 1 ]; then
  echo "cluster-proportional-autoscaler ClusterRole not found. Exiting..."
  exit 1
fi
...
if [ "$CL_PROP_AUTOSCALER_SA" -lt 1 ]; then
  echo "cluster-proportional-autoscaler Service Accounts not found. Exiting..."
  exit 1
fi
...
if [[ "$DEFAULT_PRIORITY_CLASS" != *"true"* ]] || [[ "$DEFAULT_PRIORITY_CLASS" != *"0"* ]];then
  echo "'default' PriorityClass should have value 0 and GLOBAL-DEFAULT set to true. Got the value and GLOBAL_DEFAULT: $DEFAULT_PRIORITY_CLASS. Exiting..."
  exit 1
fi
...
if [[ "$OVERPROVISIONING_PRIORITY_CLASS" != *"false"* ]] || [[ "$OVERPROVISIONING_PRIORITY_CLASS" != *"-1"* ]];then
  echo "'overprovisioning' PriorityClass should have value -1 and GLOBAL-DEFAULT set to false. Got the value and GLOBAL_DEFAULT: $OVERPROVISIONING_PRIORITY_CLASS. Exiting..."
  exit 1
fi
...
function check_monitoring_disabled () 
{
  ...
  if [ "$DISABLED_MONITORING_CONFIG_COUNT" -lt 1 ]; then
    echo "Workload monitoring should be disabled by default. Exiting..."
    exit 1
  fi
}
...
function confirm_sectors () {
  ...
  if [[ "$MC_SECTOR" != "$sector" ]]; then
    echo "Management cluster sector should be: '$sector'. Got: '$MC_SECTOR'. Exiting..."
    exit 1
  fi
  if [[ "$SC_SECTOR" != "$sector" ]]; then
    echo "Service cluster sector should be: '$sector'. Got: '$SC_SECTOR'. Exiting..."
    exit 1
  fi
}
...
function confirm_mc_count () {
  ...
  if [[ "$ACTUAL_COUNT" != "$INITIAL_MC_COUNT" ]]; then
    echo "Mamangement cluster cound should be: $INITIAL_MC_COUNT. Got: $ACTUAL_COUNT. Exiting..."
    exit 1
  fi
}
...
function confirm_labels () {
  ...
  if [[ "$LABELS_COUNT" -gt "$count" ]]; then
    echo "Expected labels count for $cluster_type with $cluster_id to be $count. Got: $LABELS_COUNT. Exiting..."
    exit 1
  fi
  if [ "$LABELS_COUNT" -gt 0 ]; then
    ...
    if [[ "$KEY_MATCH" -lt 1 ]]; then
      echo "Expected previously added label key: '$key' to be returned in labels, but none was found. Exiting..."
      exit 1
    fi
    VALUE_MATCH=$(ocm get /api/osd_fleet_mgmt/v1/"$cluster_type"/"$cluster_id"/labels | grep -c "$value")
    if [[ "$VALUE_MATCH" -lt 1 ]]; then
      echo "Expected previously added label value: '$value' to be returned in labels, but none was found. Exiting..."
      exit 1
    fi
  fi
}
