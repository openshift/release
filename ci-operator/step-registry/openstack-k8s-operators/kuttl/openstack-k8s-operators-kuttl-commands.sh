...
if [ -f "/go/src/github.com/${ORG}/${BASE_OP}/kuttl-test.yaml" ]; then
  ...
  if [ ${SERVICE_NAME} == "openstack" ]; then
    ...
    export OPENSTACK_IMG=${OPENSTACK_IMG_OLD_RELEASE:="quay.io/openstack-k8s-operators/openstack-operator-index:87ab1f1fa16743cad640f994f459ef14c5d2b9ca"}
    export TIMEOUT=600s
    ...
    oc wait openstackcontrolplane -n ${NAMESPACE} --for=condition=Ready --timeout=${TIMEOUT} -l core.openstack.org/openstackcontrolplane || exit 1
    oc get -n ${NAMESPACE} openstackversion

    # cleanup to run kuttl
    make openstack_deploy_cleanup && \
    oc wait -n ${NAMESPACE} --for=delete pod/swift-storage-0 --timeout=${TIMEOUT}
    storage_cleanup && storage_create

    # run kuttl
    make ${SERVICE_NAME}_kuttl_run
  else
