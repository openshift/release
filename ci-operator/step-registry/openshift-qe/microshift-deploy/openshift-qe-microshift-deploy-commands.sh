...
if [[ "${BUILD_FROM_PR}" == "true" ]]; then
  automation_checkout_sha=$(ssh ${SSH_ARGS} root@${bastion} "git -C '${microshift_repo}' rev-parse HEAD")
  fetched_ref_sha=$(ssh ${SSH_ARGS} root@${bastion} "git -C '${microshift_repo}' rev-parse 'refs/remotes/origin/pr-${PULL_NUMBER}'")
  ...
  if ! pr_microshift_version=$(ssh ${SSH_ARGS} root@${bastion} "
    set -o pipefail
