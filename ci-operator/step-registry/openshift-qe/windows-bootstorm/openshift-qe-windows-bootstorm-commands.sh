...
EXTRA_FLAGS="${BOOTSTORM_EXTRA_FLAGS} --metrics-profile metrics.yml,cnv-metrics.yml --gc-metrics=false --windows-image-url=${WINDOWS_IMAGE_URL} --vms-per-node=${VMS_PER_NODE} --storage-class=${STORAGE_CLASS} --access-mode=${ACCESS_MODE} --eviction-strategy=${EVICTION_STRATEGY} --profile-type=${PROFILE_TYPE}"
...
[[ -n "${https_proxy:-}" ]] && https_proxy="${https_proxy/socks5:/socks5h:}"
[[ -n "${http_proxy:-}" ]] && http_proxy="${http_proxy/socks5:/socks5h:}"
...
curl -skL -o /tmp/virtctl.tar.gz "${VIRTCTL_URL}"
gzip -t /tmp/virtctl.tar.gz 2>/dev/null || { echo "ERROR: virtctl download failed"; exit 1; }
mkdir -p /tmp/bin
tar -xzf /tmp/virtctl.tar.gz -C /tmp/bin/
