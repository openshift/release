#!/bin/bash
set -o errexit
set -o nounset
set -o pipefail
set -x

LAB_CLOUD="${LAB_CLOUD:-}"

SSH_ARGS="-i /bm/jh_priv_ssh_key -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null"
if [ -z "${RUNLOCAL:-}" ]; then
  bastion=$(cat "/bm/address")
else
  bastion=localhost
fi

REPO_NAME=${REPO_NAME:-}
PULL_NUMBER=${PULL_NUMBER:-}

regulus_repo="/root/regulus-${LAB_CLOUD}-$(date "+%Y-%m-%d-%H-%M-%S")"

function install-regulus {
  # Setup Bastion
  ssh ${SSH_ARGS} root@${bastion} "
    set -e
    set -o pipefail
    if [ -d  ${regulus_repo} ] ; then
        rm -fr  ${regulus_repo}
    fi
    git clone https://github.com/HughNhan/regulus.git --depth=1 --branch=${REG_BRANCH:-main} ${regulus_repo}
    cd ${regulus_repo}
    # REG_PR or PULL_NUMBER can't be set at the same time
    if [[ -n '${REG_PR}' ]]; then
     git pull origin pull/${REG_PR}/head:${REG_PR} --rebase
     git switch ${REG_PR}
    elif [[ -n '${PULL_NUMBER}' ]] && [[ '${REPO_NAME}' == 'regulus' ]]; then
     git pull origin pull/${PULL_NUMBER}/head:${PULL_NUMBER} --rebase
     git switch ${PULL_NUMBER}
    fi
    git branch
  "
}

install-regulus

# ─────────────────────────────────────────────────────────────────────────────
# Generate Regulus lab.config 
# ─────────────────────────────────────────────────────────────────────────────
vars=(
  KUBECONFIG
  REG_KNI_USER
  REG_OCPHOST
  REG_DP
  OCP_WORKER_0
  OCP_WORKER_1
  OCP_WORKER_2
  BM_HOSTS
  REG_SRIOV_NIC
  REG_SRIOV_MTU
  REG_SRIOV_NIC_MODEL
  REG_MACVLAN_NIC
  REG_MACVLAN_MTU
  REG_DPDK_NIC_1
  REG_DPDK_NIC_2
  REG_DPDK_NIC_MODEL
  TREX_HOSTS
  TREX_SRIOV_INTERFACE_1
  TREX_SRIOV_INTERFACE_2
  TREX_DPDK_NIC_MODEL
  REM_DPDK_CONFIG
)

if [ -e  /tmp/lab.config ]; then
    rm /tmp/lab.config
fi

echo KUBECONFIG_PATH=$KUBECONFIG_PATH LAB=$LAB LAB_CLOUD=$LAB_CLOUD TYPE=$TYPE


cat > /tmp/lab.config <<EOF
# This file was generated by openshift-qe-installer-bm-day2-regulus-commands.sh
# It can be sourced by other scripts to import environment values.
EOF
for v in "${vars[@]}"; do
  # "${!v}" expands the variable whose name is in $v
  if [[ "$v" == "KUBECONFIG" ]]; then
    # see openshift-qe-installer-bm-load-kubeconfig-commands.sh
    if [ -z "${KUBECONFIG_PATH}" ]; then
        val=/root/$LAB/$LAB_CLOUD/$TYPE/kubeconfig 
    else
        val=/$KUBECONFIG_PATH/kubeconfig 
    fi

  elif [[ "$v" == "REG_OCPHOST" ]]; then
    val=$bastion
  else
    val="${!v:-}"
  fi

  # Reformat vars in the form of ~/something to become /root/something
  if [[ "$val" == ~* ]]; then
    val="${val/#\~\//\/root/}"
  fi

  # Escape any embedded double-quotes
  safe_val=${val//\"/\\\"}
  printf '%s="%s"\n' "$v" "$safe_val" >> /tmp/lab.config
done

# ───────────────────────────────────────────────────────────────────────────
# Init Regulus and execute test suite. See jobs.config of $REG_BRANCH
# ───────────────────────────────────────────────────────────────────────────

function run-regulus {
  echo scp -q ${SSH_ARGS} /tmp/lab.config  root@${bastion}:${regulus_repo}/ 
  scp -q ${SSH_ARGS} /tmp/lab.config  root@${bastion}:${regulus_repo}/ 
  ssh ${SSH_ARGS} root@${bastion} "
    set -e
    set -o pipefail
    cd ${regulus_repo}
    source bootstrap.sh
    make init-lab 
    make init-jobs 
    #make jobs
  "
}

run-regulus

# EOF

