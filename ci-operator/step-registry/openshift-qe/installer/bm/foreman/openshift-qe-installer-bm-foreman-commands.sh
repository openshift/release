...
sshpass -p "$(cat /secret/login)" ssh -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null root@${bastion} """
  ...
  for i in $(curl -sS $QUADS_INSTANCE/cloud/$LAB_CLOUD\_ocpinventory.json | grep name | tail -n $((NUM_NODES_CLOUD-STARTING_NODE)) | head -n $NUM_NODES | awk '{print $2}' | tr -d ,\"); do
    echo $i
