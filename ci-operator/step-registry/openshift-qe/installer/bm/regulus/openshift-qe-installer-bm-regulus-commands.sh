...
TRUE_BASTION_HOST=$(cat ${CLUSTER_PROFILE_DIR}/true_bastion_host 2>/dev/null || echo "")      # second level such as cloud48 bastion
LAB_CLOUD="${LAB_CLOUD:-}"
LAB="${LAB:-}"
...
# Discover jetlag_repo and TRUE_BASTION_HOST if not already set
if [ -z "${TRUE_BASTION_HOST}" ]; then
  ...
  DISCOVER_JETLAG=true
else
  echo "TRUE_BASTION_HOST already set to: ${TRUE_BASTION_HOST}"
fi
...
if [ -n "${TEST_NAME:-}" ]; then
  ...
  if [ -e  /tmp/jobs.config ]; then
      rm /tmp/jobs.config
  fi

  echo "Updating jobs.config for mode: $TEST_NAME"
...
run-regulus() {
  ...
  if [ -n "${TEST_NAME:-}" ]; then
    do_jscp "root@${bastion}" "/tmp/jobs.config" "${regulus_repo}/jobs.config"
  fi
  # Kick off Regulus.
  do_jssh "root@${bastion}" "
    set -e
