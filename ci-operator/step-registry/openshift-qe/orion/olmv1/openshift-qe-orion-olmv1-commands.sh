...
send_slack_notification() {
  ...
  export PYTHONPATH="/Users/jiazha/goproject/openshift-tests-private/hack/:${PYTHONPATH:-}"
...
notify_slack_if_failure() {
  ...
  xxd junit_olmv1-GCP.xml | head -n1
  cleaned=$(mktemp)
  iconv -f utf-8 -t utf-8 "$xml_file" -o "$cleaned" 2>/dev/null
  failures=$(xmllint --xpath "string(//testsuite/@failures)" "$cleaned" 2>/dev/null | tr -d '[:space:]')
  rm -f "$cleaned"

  if [[ -z "$failures" || ! "$failures" =~ ^[0-9]+$ ]]; then
  echo "[WARN] xmllint failed or returned invalid output. Falling back to grep." >&2
  failures=$(grep -oP 'failures="\K[0-9]+' "$xml_file" | head -n1)
  fi
  ...
  if [[ "$failures" != "0" ]]; then
    ...
    # Construct prow job link if JOB_NAME and BUILD_ID are available
