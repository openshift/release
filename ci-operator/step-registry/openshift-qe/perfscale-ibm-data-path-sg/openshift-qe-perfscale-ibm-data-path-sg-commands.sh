...
ibmcloud is sgs --resource-group-name $resource_group_name --output json >/tmp/ibmcloud-is-sgs-rgname.txt
ibmcloud is sgs --output json>/tmp/ibmcloud-is-sgs.txt
sg=$(ibmcloud is sgs --output json | jq -r '.[] | select(.name | contains("cluster-wide"))|.id')
echo $sg
...
echo "${IBMCLOUD_CLI}" is security-group-rule-add $sg inbound tcp --port-min=10000 --port-max=61000
echo "${IBMCLOUD_CLI}" is security-group-rule-add $sg inbound udp --port-min=10000 --port-max=61000
sleep 14400
