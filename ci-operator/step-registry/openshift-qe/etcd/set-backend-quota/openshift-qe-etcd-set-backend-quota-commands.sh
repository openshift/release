...
# supported 8-16 GiB range before we touch the cluster with `oc`. The bounds
# mirror the Minimum/Maximum kubebuilder validation on
# etcd.spec.backendQuotaGiB in openshift/api, so we fail with a clear message
# instead of an opaque admission rejection from `oc patch`.
if ! [[ "${ETCD_BACKEND_QUOTA_GIB}" =~ ^[0-9]+$ ]]; then
    echo "ERROR: ETCD_BACKEND_QUOTA_GIB must be an integer, got '${ETCD_BACKEND_QUOTA_GIB}'." >&2
    exit 1
fi
if (( ETCD_BACKEND_QUOTA_GIB < 8 || ETCD_BACKEND_QUOTA_GIB > 16 )); then
    echo "ERROR: ETCD_BACKEND_QUOTA_GIB must be within 8-16 GiB, got '${ETCD_BACKEND_QUOTA_GIB}'." >&2
    exit 1
fi
