...
function rosa_upgrade()
{
  ...
  RECOMMEND_VERSION=`! rosa list upgrade -c $CLUSTER_ID --region $REGION| grep $MAJOR_CURRENT_VERSION || rosa list upgrade -c $CLUSTER_ID --region $REGION| grep $MAJOR_CURRENT_VERSION | grep recommended | awk '{print $1}'`
