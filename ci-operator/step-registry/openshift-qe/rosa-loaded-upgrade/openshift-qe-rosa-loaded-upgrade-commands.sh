...
function classic_rosa_upgrade()
{
  ...
  RECOMMEND_VERSION=`rosa list upgrade -c $CLUSTER_NAME | grep recommended | awk '{print $1}'`

  #Prior choose RECOMMEND_VERSION, if No RECOMMEND_VERSION, used TARGET_RELEASES
  if [[ -z $RECOMMEND_VERSION ]];then
	  UPGEADE_TO_VERSION=$RECOMMEND_VERSION
  else
	  UPGEADE_TO_VERSION=$TARGET_RELEASES
  fi
  echo  "-------------------------------------------------------------------------------------------"
  echo  Loaded Upgrade from [ $CURRENT_VERSION ] to [ $UPGEADE_TO_VERSION ] for $CLUSTER_NAME on $REGION
  echo  "-------------------------------------------------------------------------------------------"
  ...
  echo rosa upgrade cluster -c $CLUSTER_NAME --mode=auto --region $REGION --version $UPGEADE_TO_VERSION  --schedule-date $SCHEDULE_DATE --schedule-time $SCHEDULE_TIME -y
  echo "###############################`date`#######################################"
  rosa upgrade cluster -c $CLUSTER_NAME --mode=auto --region $REGION --version $UPGEADE_TO_VERSION --schedule-date $SCHEDULE_DATE --schedule-time $SCHEDULE_TIME -y
  ...
  do
	  ...
	  if [[ $INIT -gt $MAXRETRY ]];then
		  ...
		  rosa describe upgrade --region $REGION -c $CLUSTER_NAME
                  echo "######################################################################"
