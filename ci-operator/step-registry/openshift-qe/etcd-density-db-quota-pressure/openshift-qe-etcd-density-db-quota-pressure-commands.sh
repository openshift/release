...
# Fill etcd as fast as the API server allows: this step only exists to put the
# backend under storage pressure, so there is no reason to throttle it. At the
# wrapper default of 20 the 8GiB variant takes ~53m and the 16GiB variant ~1h46m.
export QPS=100
export BURST=100

# Variant-specific flags
EXTRA_FLAGS="--iterations=${DB_QUOTA_ITERATIONS} --iterations-per-namespace=${DB_QUOTA_ITERATIONS_PER_NAMESPACE} --kb-chunks=${KB_CHUNKS} --kb-size=${KB_SIZE}"
