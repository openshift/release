...
EXTRA_FLAGS="${ND_EXTRA_FLAGS} ${KB_FLAGS} --gc-metrics=false --pods-per-node=$PODS_PER_NODE --pod-ready-threshold=$POD_READY_THRESHOLD --profile-type=${PROFILE_TYPE} --pprof=${PPROF} --pprof-interval=1m"
