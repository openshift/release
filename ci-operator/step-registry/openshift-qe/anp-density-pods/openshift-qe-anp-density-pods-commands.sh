...
if [[ $cluster_infra == "BareMetal" && $hypershift_pods -ge 1 ]];then
        echo "Executing anp-density-pods in hypershift cluster"
...
export WORKLOAD=anp-density-pods
...
EXTRA_FLAGS+="  --iterations=$ITERATIONS --gc-metrics=true --profile-type=${PROFILE_TYPE} --pprof=${PPROF}"
