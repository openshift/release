...
ssh -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null -o ServerAliveInterval=60 -o ServerAliveCountMax=240 zuul@${bastion} "
  ...
  make e2e-tune SCENARIO=${SCENARIO} LAB_ENV=~/nfv-e2e/lab-init.env
"
EOF

scp -q ${SSH_ARGS} /tmp/nfv-e2e-tune.sh root@${jumphost}:/tmp/
ssh ${SSH_ARGS} root@${jumphost} 'bash /tmp/nfv-e2e-tune.sh'
