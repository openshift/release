...
ssh -oStrictHostKeyChecking=no -oUserKnownHostsFile=/dev/null -o ServerAliveInterval=60 -o ServerAliveCountMax=240 zuul@${bastion} "
  ...
  make e2e SCENARIO=${SCENARIO} LAB_ENV=~/nfv-e2e/lab-init.env \
    E2E_EXTRA='--skip-tune --skip-crucible --edpm-host ${EDPM_HOST}'
"
EOF

scp -q ${SSH_ARGS} /tmp/nfv-e2e-deploy.sh root@${jumphost}:/tmp/
ssh ${SSH_ARGS} root@${jumphost} 'bash /tmp/nfv-e2e-deploy.sh'
