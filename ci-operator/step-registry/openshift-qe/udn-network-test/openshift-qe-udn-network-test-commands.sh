...
LATEST_TAG=$(curl -s "https://api.github.com/repos/SachinNinganure/e2e-benchmarking/releases/latest" | jq -r '.tag_name');
TAG_OPTION="--branch $(if [ "$E2E_VERSION" == "default" ]; then echo "$LATEST_TAG"; else echo "$E2E_VERSION"; fi)";
git clone $REPO_URL $TAG_OPTION --depth 1
#git clone $REPO_URL $TAG_OPTION --depth 1 --b cnv-udn
...
WORKLOAD=full-run.yaml ./run.sh --udnl2
