...
# 1. get vpc id nad public subnet
...
PROXY_CREDENTIAL="${BASTION_NAME}:$(openssl passwd -apr1 ${PROXY_PASSWORD})"
PROXY_CREDENTIAL_CONTENT="$(echo -e ${PROXY_CREDENTIAL} | base64 -w0)"

PROXY_CONFIG_CONTENT=$(cat ${workdir}/squid.conf | base64 -w0)


REGISTRY_PASSWORD_CONTENT=$(cat "${CLUSTER_PROFILE_DIR}/registry_credential" | base64 -w0)
REGISTRY_CRT_CONTENT=$(cat "${CLUSTER_PROFILE_DIR}/server_domain_crt" | base64 -w0)
REGISTRY_KEY_CONTENT=$(cat "${CLUSTER_PROFILE_DIR}/server_domain_key" | base64 -w0)
...
cp "${SHARED_DIR}/proxy_url" "${ARTIFACT_DIR}/"
