...
DEFAULT_ROUTE=$(oc get ingresscontroller -n openshift-ingress-operator default -o jsonpath='{.status.domain}')
...
sed -i 's/user_id_claim: sub/user_id_claim: client_id/g' lightspeed-stack.template.yaml
sed -i 's/username_claim: preferred_username/username_claim: clientHost/g' lightspeed-stack.template.yaml

oc process -p IMAGE=$IMAGE -p IMAGE_TAG=$TAG -p GEMINI_API_SECRET_NAME=gemini-api-key -p LLAMA_STACK_DB_SECRET_NAME=llama-stack-db -f template.yaml --local > template.json
