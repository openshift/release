...
# Forked from
# ci-operator/step-registry/optional-operators/operator-sdk/non-ci-bundle-image
# (as of openshift/release#83049) into an OADP-owned copy so KDM-specific
# changes here don't need approval from that step's own OWNERS. Upstream
# fixes/improvements to the original ref may be worth porting back manually.
...
if [[ "${OO_MIRROR_TO_CLUSTER_REGISTRY}" == "true" ]]; then
    ...
    # resolve step.
    #
    # Opt-in (default false): this ref is currently only consumed by the KDM
    # e2e jobs. Assumes an ephemeral, single-use test cluster; none of these
    ...
    # Read-modify-write, not replace: other insecureRegistries entries may
    # already be present and a blind --type=merge replace on the array
    # would discard them.
    #
