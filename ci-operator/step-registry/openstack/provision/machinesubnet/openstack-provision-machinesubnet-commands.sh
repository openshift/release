...
if [ ${ADDITIONAL_SUBNET} = "true" ]; then
    subnet_params=" --network ${MACHINES_NET_ID} --subnet-range ${ADDITIONAL_SUBNET_RANGE} \
      --allocation-pool start=${ADDITIONAL_ALLOCATION_POOL_START},end=${ADDITIONAL_ALLOCATION_POOL_END}"
    
    ADDITIONAL_MACHINES_SUBNET_ID="$(openstack subnet create "${CLUSTER_NAME}-${CONFIG_TYPE}-additional-machines-subnet" $subnet_params \
      --description "Machines additional subnet for ${CLUSTER_NAME}-${CONFIG_TYPE}" \
      --format value --column id)"
    echo "Created additional subnet for OpenShift machines: ${ADDITIONAL_MACHINES_SUBNET_ID}"
    echo ${ADDITIONAL_MACHINES_SUBNET_ID}>${SHARED_DIR}/ADDITIONAL_MACHINES_SUBNET_ID
    echo ${ADDITIONAL_SUBNET_RANGE}>${SHARED_DIR}/ADDITIONAL_MACHINES_SUBNET_RANGE
fi
