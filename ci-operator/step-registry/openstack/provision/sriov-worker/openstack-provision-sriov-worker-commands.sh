...
function wait_for_no_worker_machines() {
    INTERVAL=10
    CNT=60

    while [ $((CNT)) -gt 0 ]; do
        GONE=false
        WORKERS=$(oc get node --no-headers)
        if [[ ${WORKERS} != *"worker"* ]]; then
            echo "All workers are gone"
            GONE=true
            break
        fi
        echo "Waiting for all workers to be gone"
        sleep $INTERVAL
        CNT=$((CNT - 1))
    done

    if [[ ${GONE} == "false" ]]; then
        echo "Timed out waiting for all workers to be gone"
        echo "${WORKERS}"
        exit 1
    fi
}
...
oc scale --replicas=0 machineset "${WORKER_MACHINESET}" -n openshift-machine-api
wait_for_no_worker_machines
