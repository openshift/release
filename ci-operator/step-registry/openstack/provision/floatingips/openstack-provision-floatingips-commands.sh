...
if [[ "${HIVE_OSP_RESOURCE}" == "true" ]]; then
	echo "Creating API floating IP for Hive"
	...
	jq -r '.id'                  <<<"$HIVE_API_FIP" >> "${SHARED_DIR}/DELETE_FIPS"

	echo "Creating Ingress floating IP for Hive"
	...
	jq -r '.id'                  <<<"$HIVE_INGRESS_FIP" >> "${SHARED_DIR}/DELETE_FIPS"
fi
