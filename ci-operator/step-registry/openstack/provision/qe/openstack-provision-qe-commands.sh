...
job_number=`curl --insecure "${OCP_TESTING_URL}/api/json?tree=builds\[number,status,timestamp,id,duration,result,url,actions\[parameters\[name,value\]\]\]" | jq "${jq_query}"`
...
then
	...
	info "INFO: The ${TEST_CONFIG_FILE} job has already run and it's ready for collect its results"
