...
job_number=`curl --insecure "${OCP_TESTING_URL}/api/json?tree=builds\[number,status,timestamp,id,duration,result,url,actions\[parameters\[name,value\]\]\]" | jq "${jq_query}" | head -1`
