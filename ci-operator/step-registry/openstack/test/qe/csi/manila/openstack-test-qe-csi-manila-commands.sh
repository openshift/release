...
info "INFO: Get openshift-test CSI Manila test results e2e.log"

test_url="${OCP_TESTING_URL}/${JOB_NUMBER}/artifact/manilacsi-test-results"
http_code=`curl --insecure -o /dev/null -s -w "%{http_code}\n" "${test_url}/manilacsi-test.log"`

if [ $http_code -ne 200 ]
then
	info "ERROR: Log Not Found with HTTP status code: $http_code in build ${JOB_NUMBER}"
	exit 1
fi

# Insecure option for testing purpose until https://github.com/openshift/release/pull/61614 is merged and tested
curl --insecure "${test_url}/manilacsi-test.log" > "${ARTIFACT_DIR}/e2e.log"

info "INFO: Get openshift-test CSI Manila test results junit"

# Insecure option for testing purpose until https://github.com/openshift/release/pull/61614 is merged and tested
curl --insecure -o "${ARTIFACT_DIR}/junit.zip" "${test_url}/*zip*/manilacsi-test-results.zip"
