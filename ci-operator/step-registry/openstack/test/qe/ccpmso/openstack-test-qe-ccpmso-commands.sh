...
curl --insecure "${test_url}/cluster-control-plane-machine-set-operator_e2e-periodic.log" > "${ARTIFACT_DIR}/e2e.log"

info "INFO: Get openstack-test-cpms test results junit"

# Insecure option for testing purpose until https://github.com/openshift/release/pull/61614 is merged and tested 
curl --insecure -o "${ARTIFACT_DIR}/junit.zip" "${test_url}/*zip*/e2e-periodic.zip"
