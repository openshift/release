...
install_vault() {
  ...
  helm upgrade --install "${release_name}" "${VAULT_CHART_ARCHIVE}" \
    ...
    --set 'server.extraEnvironmentVars.VAULT_SKIP_VERIFY=true' \
    ...
    --set "server.service.annotations.service\.beta\.openshift\.io/serving-cert-secret-name=${serving_cert}" \
  ...
  local init_json
  init_json="$(oc exec "${pod_name}" -n "${namespace}" -- vault operator init -key-shares=1 -key-threshold=1 -format=json)"
  oc exec "${pod_name}" -n "${namespace}" -- vault operator unseal "$(jq -r '.unseal_keys_b64[0]' <<<"${init_json}")" >/dev/null
