...
# Generated by Cursor
function vm_host_groups() {
  echo "$(date -u --rfc-3339=seconds) - Collecting vSphere cluster group information"
  
  # Get the list of cluster groups
  echo "Getting list of cluster groups..."
  set +e  # Temporarily disable errexit to handle potential failures gracefully
  cluster_groups=$(govc cluster.group.ls 2>&1)
  govc_exit_code=$?
  set -e  # Re-enable errexit
  
  if [ $govc_exit_code -ne 0 ]; then
    echo "Warning: Failed to get cluster groups: $cluster_groups"
    return 0
  fi
  
  if [ -z "$cluster_groups" ]; then
    echo "No cluster groups found"
    return 0
  fi
  
  echo "Found cluster groups:"
  echo "$cluster_groups"
  echo ""
  ...
  while IFS= read -r group; do
    if [ -n "$group" ]; then
      echo "Group: $group"
      echo "Details:"
      set +e  # Temporarily disable errexit for individual group queries
      group_details=$(govc cluster.group.ls -name "$group" 2>&1)
      group_exit_code=$?
      set -e  # Re-enable errexit
      
      if [ $group_exit_code -eq 0 ]; then
        echo "$group_details"
      else
        echo "Failed to get details for group '$group': $group_details"
      fi
      echo ""
    fi
  done <<< "$cluster_groups"
...
# Call the vm_host_groups function to collect cluster group information
vm_host_groups
