...
function collect_diagnostic_data {
  ...
  while [[ $v_idx -lt $VCENTER_COUNT ]]; do
    ...
    jq -r -c --arg vcenter "${GOVC_URL}" '[.failureDomains[] | select(.server == $vcenter) | .topology.networks[]] | unique | .[]' < "$SHARED_DIR"/platform.json

    PORTGROUPS=$(jq -r -c --arg vcenter "${VCENTER}" '[.failureDomains[] | select(.server == $vcenter) | .topology.networks[]] | unique | .[]' < "$SHARED_DIR"/platform.json)
    ...
    echo "${VCENTER}; port groups in failure domain: ${#PORTGROUPS[@]}"
    ...
    for PORTGROUP in "${PORTGROUPS[@]}"; do
      ...
      echo "${VCENTER}; ${PORTGROUP}; found networks: ${#networks[@]}"
