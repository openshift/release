...
function install_on_bastionhost() {
  local ret
  ...
  if [[ -n "${bastion_ssh_user}" ]] && [[ -n "${bastion_public_address}" ]]; then
    ...
    echo "Uploading installer artifacts and installer bin to the bastion host..."
    installer_bin="$(which openshift-install)"
    scp -o StrictHostKeyChecking=no -i "${CLUSTER_PROFILE_DIR}/ssh-privatekey" -r ${dir} ${installer_bin} ${bastion_ssh_user}@${bastion_public_address}:/home/${bastion_ssh_user}

    echo "Checking the artifacts on the bastion host (${bastion_ssh_user}@${bastion_public_address})..."
    ssh -o StrictHostKeyChecking=no -i "${CLUSTER_PROFILE_DIR}/ssh-privatekey" "${bastion_ssh_user}@${bastion_public_address}" "whoami; ls -la . $(basename ${dir}) $(basename ${dir})/manifests; ./openshift-install version"
    echo "Creating cluster on the bastion host (${bastion_ssh_user}@${bastion_public_address})..."
    ssh -o StrictHostKeyChecking=no -i "${CLUSTER_PROFILE_DIR}/ssh-privatekey" "${bastion_ssh_user}@${bastion_public_address}" "./openshift-install create cluster --dir installer"
    ret=$?

    echo "Copy back installer artifacts from the bastion host..."
    scp -o StrictHostKeyChecking=no -i "${CLUSTER_PROFILE_DIR}/ssh-privatekey" -r ${bastion_ssh_user}@${bastion_public_address}:/home/${bastion_ssh_user}/"$(basename ${dir})"/* ${dir}
  else
