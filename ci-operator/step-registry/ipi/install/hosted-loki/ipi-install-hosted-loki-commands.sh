...
if [[ -f "/usr/bin/python3" ]]; then
  ...
  <p>
      <a target="_blank" href="https://grafana-loki.ci.openshift.org/explore?orgId=1&left=%5B%22now-24h%22,%22now%22,%22Grafana%20Cloud%22,%7B%22expr%22:%22%7Binvoker%3D%5C%22${ENCODED_INVOKER}%5C%22%7D%20%7C%20unpack%22%7D%5D">Loki</a> - Loki provides searchable log information for all containers that existed on a cluster, not just those present at the end of the run like must-gather.
  </p>
  <p>
      <a target="_blank" href="https://hackmd.io/zx5Av4mTQKqdL7EF0JyYBQ?view">Loki cheat sheet</a> - Common expressions used to search Loki log results.
  </p>
EOF
fi
