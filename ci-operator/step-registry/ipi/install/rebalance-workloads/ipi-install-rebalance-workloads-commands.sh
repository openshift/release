...
function rebalancePods() {
  ...
  if [ "${NODE_COUNT}" -ne 1 ]; then
    echo "pods matching ${LABEL_SELECTOR} in namespace ${NS} are correctly spread"
    return
  fi

  echo "need to rebalance pods matching ${LABEL_SELECTOR} in namespace ${NS}..."
  ...
  echo "pods matching ${LABEL_SELECTOR} in namespace ${NS} have been rebalanced"
}
