...
function wait_for_co_ready() {
  echo "$(date -u --rfc-3339=seconds) - Waiting for cluster operators to become Ready"
  oc wait --all --for=condition=Available=True --for=condition=Progressing=False --for=condition=Degraded=False clusteroperators.config.openshift.io --timeout=10m
  if [[ $? -ne 0 ]]; then
    echo "$(date -u --rfc-3339=seconds) - All operators did not become ready"
    exit 1
  fi

  echo "$(date -u --rfc-3339=seconds) - Waiting for cluster operators to not be Progressing"
  oc wait --all --for=condition=Available=True --for=condition=Progressing=False --for=condition=Degraded=False clusteroperators.config.openshift.io --timeout=10m
  if [[ $? -ne 0 ]]; then
    echo "$(date -u --rfc-3339=seconds) - All operators did not finish progressing"
    exit 1
  fi
}
