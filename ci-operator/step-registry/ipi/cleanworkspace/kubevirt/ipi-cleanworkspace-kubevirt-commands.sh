...
echo "GREP: "
oc --kubeconfig=${KUBECONFIG_KUBEVIRT_IPI} -n ${KUBEVIRT_NAMESPACE} get vms | grep "${TENANT_NAME}"
echo "AWK:"
oc --kubeconfig=${KUBECONFIG_KUBEVIRT_IPI} -n ${KUBEVIRT_NAMESPACE} get vms | grep "${TENANT_NAME}" | awk '{print $1}'
echo "HEAD:"
oc --kubeconfig=${KUBECONFIG_KUBEVIRT_IPI} -n ${KUBEVIRT_NAMESPACE} get vms | grep "${TENANT_NAME}" | awk '{print $1}'|head -n 1
echo "ALL:"
