...
cp ${SHARED_DIR}/install-config.yaml ${SHARED_DIR}/install-config.old.yaml
echo "8<--------8<--------8<--------8<--------8<--------8<--------8<--------8<--------"
...
if [ ! -f /tmp/yq ]; then
  ...
  do
    echo "Try #${I}"
    curl -L "https://github.com/mikefarah/yq/releases/download/v4.44.1/yq_linux_${ARCH}" -o /tmp/yq && chmod +x /tmp/yq
...
CFG_COMPUTE_TYPE=$(mktemp --suffix=.yaml)
cat << '___EOF___' > ${CFG_COMPUTE_TYPE}
compute:
 platform: {}
___EOF___

# shellcheck disable=SC2046
CFG_CONTROL_TYPE=$(mktemp --suffix=.yaml)
cat << '___EOF___' > ${CFG_CONTROL_TYPE}
controlPlane:
 platform: {}
___EOF___
...
# Are we performing a Single Node OpenShift cluster deploy?
if [[ "${CONTROL_PLANE_REPLICAS}" == "1" && "${WORKER_REPLICAS}" == "0" ]]; then
  CFG_COMPUTE2_TYPE=$(mktemp --suffix=.yaml)
  cat << '___EOF___' > ${CFG_COMPUTE2_TYPE}
compute:
  platform:
    powervs:
      processors: 6
...
if [ -n "${FEATURE_SET}" ]; then
  echo "Adding 'featureSet: ...' to install-config.yaml"
  CFG_FEATURE_SET=$(mktemp --suffix=.yaml)
  cat << ___EOF___ > ${CFG_FEATURE_SET}
featureSet: ${FEATURE_SET}
___EOF___
fi
...
if [ -n "${FEATURE_GATES}" ]; then
  echo "Adding 'featureGates: ...' to install-config.yaml"
  CFG_FEATURE_GATES=$(mktemp --suffix=.yaml)
  cat << ___EOF___ > ${CFG_FEATURE_GATES}
featureGates: ${FEATURE_GATES}
___EOF___
fi

echo "CFG_COMPUTE_TYPE=${CFG_COMPUTE_TYPE}"
echo "CFG_COMPUTE2_TYPE=${CFG_COMPUTE2_TYPE}"
echo "CFG_CONTROL_TYPE=${CFG_CONTROL_TYPE}"
echo "CFG_FEATURE_SET=${CFG_FEATURE_SET}"
echo "CFG_FEATURE_GATES=${CFG_FEATURE_GATES}"
# Append/modify the generated sections into the main install-config.yaml
for CFG_FILE in ${CFG_COMPUTE_TYPE} ${CFG_COMPUTE2_TYPE} ${CFG_CONTROL_TYPE} ${CFG_FEATURE_SET} ${CFG_FEATURE_GATES}
do
  echo "Adding ${CFG_FILE}"
  /tmp/yq --inplace '. *d load("'${CFG_FILE}'")' ${CONFIG}
done
...
# Clean up
/bin/rm -f ${CFG_COMPUTE_TYPE} ${CFG_COMPUTE2_TYPE} ${CFG_CONTROL_TYPE} ${CFG_FEATURE_SET} ${CFG_FEATURE_GATES}
