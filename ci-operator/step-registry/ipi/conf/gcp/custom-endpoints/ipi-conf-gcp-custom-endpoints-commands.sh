...
if [[ "${ENABLE_CUSTOM_ENDPOINT_FOR_COMPUTE}" == "yes" ]]; then
  cat >> "${PATCH}" << EOF
    - name: Compute
      url: compute-${gcp_custom_endpoint}.p.googleapis.com
EOF
fi
...
if [[ "${ENABLE_CUSTOM_ENDPOINT_FOR_CONTAINER}" == "yes" ]]; then
  cat >> "${PATCH}" << EOF
    - name: Container
      url: container-${gcp_custom_endpoint}.p.googleapis.com
EOF
fi
...
if [[ "${ENABLE_CUSTOM_ENDPOINT_FOR_DNS}" == "yes" ]]; then
  cat >> "${PATCH}" << EOF
    - name: DNS
      url: dns-${gcp_custom_endpoint}.p.googleapis.com
EOF
fi
...
if [[ "${ENABLE_CUSTOM_ENDPOINT_FOR_FILE}" == "yes" ]]; then
  cat >> "${PATCH}" << EOF
    - name: File
      url: file-${gcp_custom_endpoint}.p.googleapis.com
EOF
fi
...
if [[ "${ENABLE_CUSTOM_ENDPOINT_FOR_IAM}" == "yes" ]]; then
  cat >> "${PATCH}" << EOF
    - name: IAM
      url: iam-${gcp_custom_endpoint}.p.googleapis.com
EOF
fi
...
if [[ "${ENABLE_CUSTOM_ENDPOINT_FOR_SERVICEUSAGE}" == "yes" ]]; then
  cat >> "${PATCH}" << EOF
    - name: ServiceUsage
      url: serviceusage-${gcp_custom_endpoint}.p.googleapis.com
EOF
fi
...
if [[ "${ENABLE_CUSTOM_ENDPOINT_FOR_CLOUDRESOURCEMANAGER}" == "yes" ]]; then
  cat >> "${PATCH}" << EOF
    - name: CloudResourceManager
      url: cloudresourcemanager-${gcp_custom_endpoint}.p.googleapis.com
EOF
fi
...
if [[ "${ENABLE_CUSTOM_ENDPOINT_FOR_STORAGE}" == "yes" ]]; then
  cat >> "${PATCH}" << EOF
    - name: Storage
      url: storage-${gcp_custom_endpoint}.p.googleapis.com
EOF
fi
