...
# Check if install-config.yaml has controlPlane.platform.gcp.serviceAccount set
...
if [ -n "${CUSTOM_CONTROL_PLANE_SA}" ] && [ "${CUSTOM_CONTROL_PLANE_SA}" != "null" ]; then
	...
	logger "INFO" "Using default master node service account: ${MASTER_NODE_SA}"
fi

# Determine worker node service account
# Check if install-config.yaml has compute[0].platform.gcp.serviceAccount set
CUSTOM_COMPUTE_SA=""
if [ -f "${CONFIG}" ]; then
...
if [ -n "${CUSTOM_COMPUTE_SA}" ] && [ "${CUSTOM_COMPUTE_SA}" != "null" ]; then
	...
	logger "INFO" "Using default worker node service account: ${WORKER_NODE_SA}"
fi
