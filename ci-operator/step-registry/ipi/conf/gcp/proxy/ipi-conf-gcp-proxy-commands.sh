...
if [[ -z "${NETWORK}" || -z "${CONTROL_PLANE_SUBNET}" ]]; then
  ...
  if [[ -z "${NETWORK}" && -s "${SHARED_DIR}/vpc-info.yaml" ]]; then
    ...
    CONTROL_PLANE_SUBNET="$(basename "${HOST_PROJECT_CONTROL_PLANE_SUBNET}")"
  fi
fi
if [[ -z "${NETWORK}" || -z "${CONTROL_PLANE_SUBNET}" ]]; then
...
EOF
  cat > "${SHARED_DIR}/client_proxy_setting.json" << EOF
{
  "http_proxy": "${CLIENT_PROXY_URL}",
  "https_proxy": "${CLIENT_PROXY_URL}"
}
EOF
fi
