...
spec:
  ...
  echo "$(date -u --rfc-3339=seconds) - Waiting for lease to be fulfilled..."
  oc wait leases.vspherecapacitymanager.splat.io --kubeconfig /var/run/vault/vsphere-config/vsphere-capacity-manager-kubeconfig --for=jsonpath='{.status.phase}'=Fulfilled -l boskos-lease-id="${LEASED_RESOURCE}"
fi
done

echo "$(date -u --rfc-3339=seconds) - Storing leases in SHARED_DIR..."
oc get leases.vspherecapacitymanager.splat.io --kubeconfig /var/run/vault/vsphere-config/vsphere-capacity-manager-kubeconfig -l boskos-lease-id="${LEASED_RESOURCE} > ${SHARED_DIR}/leases.yaml
