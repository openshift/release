...
# Enable journal forwarding to log-gather so deprovision diags can retrieve journals (same as VCM path).
if [[ -f /var/run/vault/vsphere-ibmcloud-config/journal-logging-enabled ]]; then
