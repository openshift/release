...
echo "export CLUSTER_NAME=$VSPHERE_CLUSTER_NAME" > "$env_file"
echo "export CLUSTER_DOMAIN=$cluster_domain" > "$env_file"
echo "export NETWORK_NAME=$network_name" > "$env_file"
echo "export GOVC_USERNAME=$GOVC_USERNAME" > "$env_file"
echo "export GOVC_PASSWORKD=$GOVC_PASSWORD" > "$env_file"
...
if [ "${JOB_NAME_SAFE}" = "launch" ]; then
  ...
  api_dns_target_spoke='"TTL": 60, "ResourceRecords": [{"Value": "'${vips[2]}'"}]'
  apps_dns_target_spoke='"TTL": 60, "ResourceRecords": [{"Value": "'${vips[3]}'"}]'
fi
...
"Changes": [{
    ...
    "Action": "UPSERT",
    "ResourceRecordSet": {
      "Name": "api.$cluster_domain_spoke.",
      "Type": "$RECORD_TYPE",
      $api_dns_target_spoke
      }
    "Action": "UPSERT",
    "ResourceRecordSet": {
    ...
    "Action": "UPSERT",
    "ResourceRecordSet": {
      "Name": "*.apps.$cluster_domain_spoke.",
      "Type": "$RECORD_TYPE",
      $apps_dns_target_spoke
      }
}]}
EOF
