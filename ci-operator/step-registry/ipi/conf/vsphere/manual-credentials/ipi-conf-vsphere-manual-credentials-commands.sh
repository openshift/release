...
vsphere_url=$(jq -r '.server // empty' <<<"${base_vcenter}")
if [[ -z "${vsphere_url}" ]]; then
  echo "platform.vsphere.vcenters[0].server is missing"
  exit 1
fi

SA_KUBECONFIG=${SA_KUBECONFIG:-/var/run/vault/vsphere-ibmcloud-ci/vsphere-capacity-manager-kubeconfig}
pool_json=$(mktemp)
trap 'rm -f "${pool_json}" "${patch:-}"' EXIT
oc get pools.vspherecapacitymanager.splat.io \
  --kubeconfig "${SA_KUBECONFIG}" \
  -n vsphere-infra-helpers -o json >"${pool_json}"
VCENTER_AUTH_PATH=$(jq -r --arg VCENTER "${vsphere_url}" \
  'first(.items[] | select(.spec.server == $VCENTER) | .metadata.annotations["ci-auth-path"] // empty)' \
  <"${pool_json}")
if [[ -z "${VCENTER_AUTH_PATH}" || ! -f "${VCENTER_AUTH_PATH}" ]]; then
  echo "vCenter authentication configuration was not found for ${vsphere_url}"
  exit 1
fi
