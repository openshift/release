...
if [[ "${OPENSHIFT_REQUIRED_CORES}" -eq "" ]]; then
  log "OPENSHIFT_REQUIRED_CORES is empty, calculating via vm-specs.json in vault"
  ...
  # jcallen notes: do we really need this since bootstrap is not a long term virtual machine?
...
if [[ "${OPENSHIFT_REQUIRED_MEMORY}" -eq "" ]]; then
  log "OPENSHIFT_REQUIRED_MEMORY is empty, calculating via vm-specs.json in vault"
...
log "OPENSHIFT_REQUIRED_MEMORY: ${OPENSHIFT_REQUIRED_MEMORY}"
log "OPENSHIFT_REQUIRED_CORES: ${OPENSHIFT_REQUIRED_CORES}"
