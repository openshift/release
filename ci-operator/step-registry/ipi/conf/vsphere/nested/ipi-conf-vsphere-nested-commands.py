...
r = ansible_runner.run_command(
    ...
    cmdline_args=['main.yml', '-i', 'hosts', '--extra-var', 'version=VC8.0.2.00100-22617221-ESXi8.0u2c','-vvv', '-k'],
    input_fd=sys.stdin,
...
install_config["platform"]["apiVIP"] = vips[0].strip()
if len(vips) == 1:
    install_config["platform"]["ingressVIP"] = vips[0].strip()
else:
    install_config["platform"]["ingressVIP"] = vips[1].strip()
