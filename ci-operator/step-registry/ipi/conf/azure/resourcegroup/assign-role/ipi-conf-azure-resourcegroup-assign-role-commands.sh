...
if [[ -z "${cluster_rg}" ]] && [[ -z "${vnet_rg}" ]]; then
    echo "This step used to grant proper permissions on scope of cluster rg or vnet rg, but both rg are empty, abort..."
    exit 1
fi
...
if [[ -n "${cluster_rg}" ]]; then
    ...
    echo "Assigin role 'Contributor' to resource group ${cluster_rg}"
...
if [[ -n "${vnet_rg}" ]]; then
    ...
    echo "Assigin role 'Network Contributor' to resource group ${cluster_rg}"
