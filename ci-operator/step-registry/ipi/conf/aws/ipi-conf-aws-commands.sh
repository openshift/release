...
imageContentSources:
- mirrors:
  - quayio-pull-through-cache-us-east-1-ci.apps.master.ci.devcluster.openshift.com
  source: quay.io
EOF
  ...
  pull_secret_aws=$(jq --arg auth ${mirror_auth} --arg repo "quayio-pull-through-cache-us-east-1-ci.apps.master.ci.devcluster.openshift.com" '.["auths"] += {($repo): {$auth}}' <<<  $pull_secret)
