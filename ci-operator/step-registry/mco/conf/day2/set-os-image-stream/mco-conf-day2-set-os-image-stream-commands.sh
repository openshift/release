...
# Main execution
if [[ -z "${MCO_CONF_DAY2_MCP_OS_IMAGE_STREAM_NAME}" ]]; then
  echo "MCO_CONF_DAY2_MCP_OS_IMAGE_STREAM_NAME is empty. No MachineConfigPools will be configured."
  exit 0
fi
...
declare -a mcp_list
if [[ -z "${MCO_CONF_DAY2_MCP_OS_IMAGE_STREAM_POOLS}" ]]; then
  echo "MCO_CONF_DAY2_MCP_OS_IMAGE_STREAM_POOLS is empty. Configuring ALL MachineConfigPools in the cluster."
  ...
  echo "Configuring specified MachineConfigPools: ${MCO_CONF_DAY2_MCP_OS_IMAGE_STREAM_POOLS}"
  # Convert space-separated list to array
  read -ra mcp_list <<< "${MCO_CONF_DAY2_MCP_OS_IMAGE_STREAM_POOLS}"
fi

echo "Configuring osImageStream '${MCO_CONF_DAY2_MCP_OS_IMAGE_STREAM_NAME}' on ${#mcp_list[@]} MachineConfigPool(s)"

# Configure each MCP
for mcp_name in "${mcp_list[@]}"; do
  set_os_image_stream "${mcp_name}" "${MCO_CONF_DAY2_MCP_OS_IMAGE_STREAM_NAME}"
done

# Wait for each MCP to be updated
for mcp_name in "${mcp_list[@]}"; do
  wait_for_mcp_to_update "${mcp_name}" "${MCO_CONF_DAY2_MCP_OS_IMAGE_STREAM_TIMEOUT}"
done
