...
<testsuite time="${BURNER_RUN_DURATION}" name="openshift-tests" tests="1" failures="1" skipped="0" errors="0">
  <testcase name="[sig-olmv1][Jira:OLM] OLMv1 stress PolarionID:${CASEID}-${CASETITLE}" time="${BURNER_RUN_DURATION}">
...
function download_github_directory {
    ...
    # Arguments:
    #   $1 - GitHub user/org (e.g., "kuiwang02")
    #   $2 - Repository name (e.g., "operator-framework-operator-controller")
    #   $3 - Branch name (e.g., "migratestress")
    #   $4 - Directory path in repo (e.g., "openshift/tests-extension/test/qe/util/stress")
    #   $5 - Local target directory (e.g., "/tmp/olmv1stress")
...
openshift-extended-olm-test-burner-pkg-ins-v1-ote:
  total: ${results[tests]}
...
export STRESS_GITHUB_ORG="${STRESS_GITHUB_ORG_V1}" STRESS_GITHUB_REPO="${STRESS_GITHUB_REPO_V1}" STRESS_GITHUB_BRANCH="${STRESS_GITHUB_BRANCH_V1}" STRESS_DIR_PATH="${STRESS_DIR_PATH_V1}" STRESS_DOWNLOAD_DIR="${STRESS_DOWNLOAD_DIR_V1}"
