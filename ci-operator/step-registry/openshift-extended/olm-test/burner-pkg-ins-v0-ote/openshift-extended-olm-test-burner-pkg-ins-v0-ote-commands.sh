...
function generate_junit_xml {
    ...
    if [ "W${ret_code}W" != "W0W" ]; then
        cat >"${junit_dir}/junit_OLM.xml" <<- EOF
<testsuite time="${BURNER_RUN_DURATION}" name="openshift-tests" tests="1" failures="1" skipped="0" errors="0">
  <testcase name="[sig-operator][Jira:OLM] OLM v0 stress PolarionID:${CASEID}-${CASETITLE}" time="${BURNER_RUN_DURATION}">
...
EOF
        ...
        cat >"${junit_dir}/junit_OLM.xml" <<- EOF
<testsuite time="${BURNER_RUN_DURATION}" name="openshift-tests" tests="1" failures="0" skipped="0" errors="0">
  <testcase name="[sig-operator][Jira:OLM] OLM v0 stress PolarionID:${CASEID}-${CASETITLE}" time="${BURNER_RUN_DURATION}"/>
</testsuite>
EOF
...
function download_github_directory {
    # Download a directory from GitHub repository using GitHub API and curl
    # Arguments:
    #   $1 - GitHub user/org (e.g., "openshift")
    #   $2 - Repository name (e.g., "operator-framework-olm")
    #   $3 - Branch name (e.g., "main")
    #   $4 - Directory path in repo (e.g., "tests-extension/test/qe/util/stress")
    #   $5 - Local target directory (e.g., "/tmp/olmv0stress")

    local github_user="$1"
    local repo_name="$2"
    local branch_name="$3"
    local repo_dir_path="$4"
    local local_target_dir="$5"

    # GitHub API URL to list directory contents
    local api_url="https://api.github.com/repos/${github_user}/${repo_name}/contents/${repo_dir_path}?ref=${branch_name}"

    echo "Downloading stress directory from GitHub: ${github_user}/${github_repo}@${branch_name}:${repo_dir_path}"

    # Get directory listing as JSON
    local dir_listing
    dir_listing=$(curl --fail --retry 5 -sS -L "${api_url}") || {
        ret_code=$?;
        FAIL_MESSAGE="cant fetch directory listing from GitHub API: ${api_url}";
        exit_burner $ret_code yes;
    }

    # Check if response is valid JSON array
    if ! echo "${dir_listing}" | jq -e '. | type == "array"' >/dev/null 2>&1; then
        FAIL_MESSAGE="GitHub API response is not a valid array, possibly invalid path or branch"
        echo "API Response: ${dir_listing}"
        exit_burner 1 yes
    fi

    # Parse JSON and process each item
    local items_count
    items_count=$(echo "${dir_listing}" | jq '. | length')
    ...
    for i in $(seq 0 $((items_count - 1))); do
        ...
        item_type=$(echo "${dir_listing}" | jq -r ".[$i].type")
        item_path=$(echo "${dir_listing}" | jq -r ".[$i].path")
        item_name=$(echo "${dir_listing}" | jq -r ".[$i].name")
        download_url=$(echo "${dir_listing}" | jq -r ".[$i].download_url")

        if [ "${item_type}" == "file" ]; then
            # Download file
            local local_file_path="${local_target_dir}/${item_name}"

            curl --fail --retry 5 -sS -L "${download_url}" -o "${local_file_path}" || {
                ret_code=$?;
                FAIL_MESSAGE="cant download file from GitHub: ${item_name}";
                exit_burner $ret_code yes;
            }

        elif [ "${item_type}" == "dir" ]; then
            # Recursively download subdirectory
            local local_subdir_path="${local_target_dir}/${item_name}"

            # Create subdirectory
            mkdir -p "${local_subdir_path}" || {
                ret_code=$?;
                FAIL_MESSAGE="cant create subdirectory: ${local_subdir_path}";
                exit_burner $ret_code yes;
            }

            # Recursive call for subdirectory
            download_github_directory "${github_user}" "${repo_name}" "${branch_name}" "${item_path}" "${local_subdir_path}"
        fi
    done
}

function get_config_files {
    # Download stress tools from GitHub repository using configurable parameters
    local github_user="${STRESS_GITHUB_ORG}"
    local github_repo="${STRESS_GITHUB_REPO}"
    local github_branch="${STRESS_GITHUB_BRANCH}"
    local stress_dir_path="${STRESS_DIR_PATH}"
    local download_dir="${STRESS_DOWNLOAD_DIR}"

    echo "Downloading stress tools from ${github_user}/${github_repo}@${github_branch} to ${download_dir}"

    # Create download directory
    mkdir -p "${download_dir}" || { ret_code=$?; FAIL_MESSAGE="cant create dir ${download_dir}"; exit_burner $ret_code yes; }

    # Disable command tracing during download to reduce log verbosity
    set +x
    # Download the stress directory using our custom function
    download_github_directory "${github_user}" "${github_repo}" "${github_branch}" "${stress_dir_path}" "${download_dir}"
    set -x

    # Verify stress directory structure exists
    local util_parent_dir="${download_dir}/util"
    local config_parent_dir="${download_dir}/manifests/config"

    if [ ! -d "${util_parent_dir}" ]; then
        FAIL_MESSAGE="util directory not found in downloaded stress tools: ${util_parent_dir}"
        exit_burner 1 yes
    fi

    if [ ! -d "${config_parent_dir}" ]; then
        FAIL_MESSAGE="config directory not found in downloaded stress tools: ${config_parent_dir}"
        exit_burner 1 yes
    fi

    # Copy stress tools
    cp -fr "${util_parent_dir}/"* . || { ret_code=$?; FAIL_MESSAGE="cant copy burner util from downloaded stress tools"; exit_burner $ret_code yes; }
    cp -fr "${config_parent_dir}/${OPERATION}/"* . || { ret_code=$?; FAIL_MESSAGE="cant copy burner-config from downloaded stress tools"; exit_burner $ret_code yes; }

    echo "Successfully prepared stress tools"
}
...
function summarize_test_result {
    ...
    grep -r -E -h -o 'testsuite.*tests="[0-9]+"[^>]*' ${ARTIFACT_DIR}/junit/junit*.xml > /tmp/result.log || { echo "can not grep test result";exit_burner 1 "no"; }
    while read row ; do
        echo "row: ${row}"
...
openshift-extended-olm-test-burner-pkg-ins-v0-ote:
  total: ${results[tests]}
...
export STRESS_GITHUB_ORG STRESS_GITHUB_REPO STRESS_GITHUB_BRANCH STRESS_DIR_PATH STRESS_DOWNLOAD_DIR
