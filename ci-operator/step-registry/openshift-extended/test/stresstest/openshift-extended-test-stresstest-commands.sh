...
function run {
    ...
    echo "TEST_SCENARIOS_STRESSTEST: \"${TEST_SCENARIOS_STRESSTEST:-}\""
    echo "TEST_ADDITIONAL_STRESSTEST: \"${TEST_ADDITIONAL_STRESSTEST:-}\""
    echo "TEST_IMPORTANCE: \"${TEST_IMPORTANCE}\""
    echo "TEST_FILTERS_STRESSTEST: \"${hardcoded_filters};${TEST_FILTERS_STRESSTEST:-}\""
    echo "FILTERS_ADDITIONAL_STRESSTEST: \"${FILTERS_ADDITIONAL_STRESSTEST:-}\""
    echo "TEST_TIMEOUT_STRESSTEST: \"${TEST_TIMEOUT_STRESSTEST}\""
    if [[ -n "${TEST_SCENARIOS_STRESSTEST:-}" ]]; then
        readarray -t scenarios <<< "${TEST_SCENARIOS_STRESSTEST}"
    ...
    if [ "W${test_scenarios}W" == "WW" ]; then
        echo "fail to parse ${TEST_SCENARIOS_STRESSTEST}"
        exit 1
    fi
    ...
    test_additional=""
    if [[ -n "${TEST_ADDITIONAL_STRESSTEST:-}" ]]; then
        readarray -t additionals <<< "${TEST_ADDITIONAL_STRESSTEST}"
    ...
    test_filters="${hardcoded_filters};${TEST_FILTERS_STRESSTEST}"
    if [[ -n "${FILTERS_ADDITIONAL_STRESSTEST:-}" ]]; then
        echo "add FILTERS_ADDITIONAL_STRESSTEST into test_filters"
        test_filters="${hardcoded_filters};${TEST_FILTERS_STRESSTEST};${FILTERS_ADDITIONAL_STRESSTEST}"
    fi
    ...
    echo "MODULE_FILTERS_STRESSTEST: \"${MODULE_FILTERS_STRESSTEST:-}\""
    handle_module_filter "${MODULE_FILTERS_STRESSTEST}"
    ...
    if [ "W${TEST_PROVIDER}W" == "WnoneW" ]; then
        ...
        --timeout "${TEST_TIMEOUT_STRESSTEST}m" --junit-dir="${ARTIFACT_DIR}/junit" -f ./case_selected || ret_value=$?
    else
        extended-platform-tests run --max-parallel-tests ${TEST_PARALLEL} \
