...
function run {
    ...
    echo "TEST_SCENARIOS_DISASTERRECOVERY: \"${TEST_SCENARIOS_DISASTERRECOVERY:-}\""
    echo "TEST_ADDITIONAL_DISASTERRECOVERY: \"${TEST_ADDITIONAL_DISASTERRECOVERY:-}\""
    echo "TEST_IMPORTANCE: \"${TEST_IMPORTANCE}\""
    echo "TEST_FILTERS_DISASTERRECOVERY: \"${hardcoded_filters};${TEST_FILTERS_DISASTERRECOVERY:-}\""
    echo "FILTERS_ADDITIONAL_DISASTERRECOVERY: \"${FILTERS_ADDITIONAL_DISASTERRECOVERY:-}\""
    echo "TEST_TIMEOUT_DISASTERRECOVERY: \"${TEST_TIMEOUT_DISASTERRECOVERY}\""
    if [[ -n "${TEST_SCENARIOS_DISASTERRECOVERY:-}" ]]; then
        readarray -t scenarios <<< "${TEST_SCENARIOS_DISASTERRECOVERY}"
    ...
    if [ "W${test_scenarios}W" == "WW" ]; then
        echo "fail to parse ${TEST_SCENARIOS_DISASTERRECOVERY}"
        exit 1
    fi
    ...
    test_additional=""
    if [[ -n "${TEST_ADDITIONAL_DISASTERRECOVERY:-}" ]]; then
        readarray -t additionals <<< "${TEST_ADDITIONAL_DISASTERRECOVERY}"
    ...
    test_filters="${hardcoded_filters};${TEST_FILTERS_DISASTERRECOVERY}"
    if [[ -n "${FILTERS_ADDITIONAL_DISASTERRECOVERY:-}" ]]; then
        echo "add FILTERS_ADDITIONAL_DISASTERRECOVERY into test_filters"
        test_filters="${hardcoded_filters};${TEST_FILTERS_DISASTERRECOVERY};${FILTERS_ADDITIONAL_DISASTERRECOVERY}"
    fi
    ...
    echo "MODULE_FILTERS_DISASTERRECOVERY: \"${MODULE_FILTERS_DISASTERRECOVERY:-}\""
    handle_module_filter "${MODULE_FILTERS_DISASTERRECOVERY}"
    ...
    if [ "W${TEST_PROVIDER}W" == "WnoneW" ]; then
        ...
        --timeout "${TEST_TIMEOUT_DISASTERRECOVERY}m" --junit-dir="${ARTIFACT_DIR}/junit" -f ./case_selected || ret_value=$?
    else
        extended-platform-tests run --max-parallel-tests ${TEST_PARALLEL} \
