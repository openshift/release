...
if [[ ${log} =~ "No problems detected" ]]; then
    ...
    <testsuite name="OLM-Catalog-Validation" tests="1" failures="0">
        <testcase name="scanning cluster by k8sgpt"/>
    </testsuite>
EOF
else
    cat >"${ARTIFACT_DIR}/junit/k8sgpt-result.xml" <<EOF
    <testsuite name="OLM-Catalog-Validation" tests="1" failures="1">
      <testcase name="scanning cluster by k8sgpt">
