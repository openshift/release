...
KUBECTL_WRAPPER_DIR="${SHARED_DIR}/quay-sts-bin"
mkdir -p "${KUBECTL_WRAPPER_DIR}"
cat >"${KUBECTL_WRAPPER_DIR}/kubectl" <<'EOF'
#!/bin/sh
exec oc "$@"
EOF
chmod +x "${KUBECTL_WRAPPER_DIR}/kubectl"
export PATH="${KUBECTL_WRAPPER_DIR}:/tmp/quay-sts-bin:${PATH}"
