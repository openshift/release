...
# (registry.access.redhat.com/ubi9/ubi-minimal:9.8) exits 0 immediately
...
ssh_bastion "sudo skopeo copy --retry-times 3 \
    ...
    || { log "ERROR: failed to mirror ubi9/ubi-minimal:9.8"; exit 1; }
...
# Snapshot MCP rendered configs before the ITMS so the rollout can be detected.
...
# Wait for MCO to roll out the ITMS change to all nodes. The shared helper snapshots
# MCP rendered-config names, waits for them to change (proving MCO started a rollout),
# then waits for condition=Updated — avoiding the race where a bare
# `oc wait --for=condition=Updated` returns immediately on the pre-ITMS state.
