...
export CUCUMBER_DIR="${TEST_RUN_RESULTS_DIR}/ui-tests/target/cucumber"
...
while read -r FILE; do
  mkdir -p "${TEST_RUN_RESULTS_DIR}"/"$(dirname "$FILE")"
  cp "$FILE" "${TEST_RUN_RESULTS_DIR}"/"$(dirname "$FILE")"
done <<<"$(find * -type f -name "*.log")"

while read -r DIR; do
  mkdir -p "${TEST_RUN_RESULTS_DIR}"/"$DIR"
  cp -r "$DIR"/* "${TEST_RUN_RESULTS_DIR}"/"$DIR"
done <<<"$(find * -maxdepth 2 -type d -wholename "*target/cucumber*")"
