...
# Validate release branch. We can only run on global-hub-x.y branches.
if [[ ! ("$branch" =~ ^global-hub-[0-9]+\.[0-9]+$) ]]; then
    log "ERROR Branch $branch is not a global-hub branch."
    log "      Base branch of PR must match global-hub-x.y"
    exit 1
fi
