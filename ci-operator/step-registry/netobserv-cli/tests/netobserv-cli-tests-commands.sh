...
# (see core-services/prow/02_config/_config.yaml required_files pattern
...
write_cli_spyglass_report() {
  local exit_code="${1:-0}"
  local step_name="netobserv-cli-tests"
  ...
  if [[ -z "${ARTIFACT_DIR:-}" ]]; then
    echo "====> ARTIFACT_DIR unset; skipping CLI Spyglass report"
    return 0
  fi

  report="${ARTIFACT_DIR}/custom-link-cli.html"
...
<html>
<head>
  <title>NetObserv CLI debug</title>
  <meta name="description" content="Links to netobserv-cli ginkgo e2e logs, JUnit failures, and report artifacts for netobserv-cli-tests.">
  <style>
    body {
...
<body>
  ...
  <a class="btn" href="$(html_escape "${artifacts_base}/junit/report.xml")" target="_blank">JUnit report (XML)</a>
...
EOF
    ...
    cat <<EOF
  <p class="muted">Tip: full ginkgo console output is in the step build log linked above.</p>
</body>
</html>
EOF
  } > "${report}" || write_rc=$?
  ...
  if [[ "${write_rc}" -eq 0 ]]; then
    echo "====> Wrote CLI Spyglass report: ${report}"
