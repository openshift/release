...
while IFS=$'\t' read -r node_name expected_ip expected_iface _pod_name; do
  ...
  if ! container_name=$(get_ovnkube_container "${pod_name}"); then
    echo "unable to determine OVN container for node ${node_name} pod ${pod_name}" >&2
    validation_failed=1
    continue
  fi
  ...
  if ! oc exec -n "${namespace}" "${pod_name}" -c "${container_name}" -- ovs-vsctl show | tee "${show_file}"; then
    echo "unable to capture ovs-vsctl show for node ${node_name}" >&2
    validation_failed=1
    continue
  fi

  if ! oc exec -n "${namespace}" "${pod_name}" -c "${container_name}" -- ovs-vsctl get Open_vSwitch . external_ids | tee "${extids_file}"; then
    echo "unable to capture OVS external_ids for node ${node_name}" >&2
    validation_failed=1
    continue
  fi
  ...
  if ! grep -Fq 'type: geneve' "${show_file}"; then
    echo "no geneve ports were found in ovs-vsctl show for node ${node_name}" >&2
    validation_failed=1
    continue
  fi

  expected_geneve_peers=0
  while IFS=$'\t' read -r peer_node peer_ip _peer_iface _peer_pod; do
    if [[ "${peer_node}" == "${node_name}" ]]; then
      continue
    fi

    expected_geneve_peers=$((expected_geneve_peers + 1))
    if ! grep -Fq "remote_ip=\"${peer_ip}\"" "${show_file}"; then
      echo "missing geneve remote_ip ${peer_ip} in ovs-vsctl show for node ${node_name}" >&2
      validation_failed=1
    fi
  done < "${mapping_file}"

  local_geneve_ports=$(grep -F "local_ip=\"${expected_ip}\"" "${show_file}" | wc -l | tr -d ' ')
  if [[ "${local_geneve_ports}" -lt "${expected_geneve_peers}" ]]; then
    echo "expected at least ${expected_geneve_peers} geneve ports with local_ip ${expected_ip} on ${node_name}, found ${local_geneve_ports}" >&2
    validation_failed=1
  fi
