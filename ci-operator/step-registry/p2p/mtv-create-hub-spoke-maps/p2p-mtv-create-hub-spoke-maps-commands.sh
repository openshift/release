...
WaitMapReady storagemap "${_revStorMap}"
    ...
    set +x
    MgmtOc get networkmap,storagemap -n "${MTV_NAMESPACE}" -o json \
        | jq '[.items[]? | {name: .metadata.name, kind: .kind, phase: (.status.phase // null), conditions: [.status.conditions[]? | {type, status, reason, message: (.message | gsub("\\b[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+\\b"; "<REDACTED-IP>") | gsub("https?://[^\\s\\"]+"; "<REDACTED-URL>"))}], ownerReferences: [.metadata.ownerReferences[]? | {kind, name}], created: .metadata.creationTimestamp}]' \
        > "${ARTIFACT_DIR}/mtv-hub-spoke-maps-status.json" 2>&1 || true
    set -x
