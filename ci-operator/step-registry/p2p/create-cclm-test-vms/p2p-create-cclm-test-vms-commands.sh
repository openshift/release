...
typeset P2P_HS_SPOKE_VM_PREFIX="${P2P_HS_SPOKE_VM_PREFIX:-test-vm}"
...
DumpDiagnostics() {
    ...
    if [[ "${P2P_HS_CREATE_HUB_VMS}" != "false" ]]; then
        for ((i = 1; i <= vmCount; i++)); do
            {
                ...
                oc --kubeconfig="${KUBECONFIG}" get \
                    ...
                    -n "${P2P_HS_VM_NAMESPACE}" -o json 2>/dev/null \
                ...
                oc --kubeconfig="${KUBECONFIG}" get \
                    ...
                    | jq '{name: .metadata.name, kind: .kind,
                           ...
                           storageClassName: .spec.storageClassName}' || true
                set -x
        ...
        oc --kubeconfig="${KUBECONFIG}" get events -n "${P2P_HS_VM_NAMESPACE}" \
            ...
            > "${diagDir}/hub-events.json" 2>&1 || true
        set -x
    fi
    for ((i = 1; i <= vmCount; i++)); do
            {
        ...
        oc --kubeconfig="${spokeKubeconfig}" get events -n "${P2P_HS_VM_NAMESPACE}" \
            --sort-by='.lastTimestamp' -o json \
            | jq '[.items[]? | {type, reason, regarding: (.regarding.name // .involvedObject.name), message: (.message | gsub("\\b[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+\\b"; "<REDACTED-IP>") | gsub("https?://[^\\s\\"]+"; "<REDACTED-URL>") | gsub("token=[^&\\s]+"; "token=<REDACTED>"))}]' \
            > "${diagDir}/spoke-events.json" 2>&1 || true
        set -x
}
...
if (( cclmStepRc == 0 )) && [[ "${P2P_HS_CREATE_HUB_VMS}" != "false" ]]; then
    CreateClusterVms "${KUBECONFIG}" "${P2P_HS_HUB_VM_PREFIX}" \
        "${P2P_HS_VM_NAMESPACE}" "${P2P_HS_HUB_STORAGE_CLASS}" \
        "${P2P_HS_VM_DATA_SOURCE_NAME}" "${P2P_HS_VM_DATA_SOURCE_NAMESPACE}" \
        "hub" || cclmStepRc=$?
fi

if (( cclmStepRc == 0 )); then
    CreateClusterVms "${spokeKubeconfig}" "${P2P_HS_SPOKE_VM_PREFIX}" \
        "${P2P_HS_VM_NAMESPACE}" "${P2P_HS_SPOKE_STORAGE_CLASS}" \
        "${P2P_HS_VM_DATA_SOURCE_NAME}" "${P2P_HS_VM_DATA_SOURCE_NAMESPACE}" \
        "spoke" || cclmStepRc=$?
fi

# Status dump (non-fatal, always runs regardless of cclmStepRc)
if [[ -n "${ARTIFACT_DIR}" ]]; then
    ...
    set +x
    {
        ...
        if [[ "${P2P_HS_CREATE_HUB_VMS}" != "false" ]]; then
            for ((i = 1; i <= vmCount; i++)); do
                typeset vmName="${P2P_HS_HUB_VM_PREFIX}-${i}"
                oc --kubeconfig="${KUBECONFIG}" get "virtualmachine/${vmName}" \
                    -n "${P2P_HS_VM_NAMESPACE}" -o json 2>/dev/null \
                    | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null), ready: (.status.ready // null), created: .metadata.creationTimestamp}' || true
                oc --kubeconfig="${KUBECONFIG}" get "virtualmachineinstance/${vmName}" \
                    -n "${P2P_HS_VM_NAMESPACE}" -o json 2>/dev/null \
                    | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null), nodeName: "<REDACTED>", created: .metadata.creationTimestamp}' || true
            done
        fi
        if [[ -n "${spokeKubeconfig:-}" ]]; then
            for ((i = 1; i <= vmCount; i++)); do
                typeset vmName="${P2P_HS_SPOKE_VM_PREFIX}-${i}"
                oc --kubeconfig="${spokeKubeconfig}" get "virtualmachine/${vmName}" \
                    -n "${P2P_HS_VM_NAMESPACE}" -o json 2>/dev/null \
                    | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null), ready: (.status.ready // null), created: .metadata.creationTimestamp}' || true
                oc --kubeconfig="${spokeKubeconfig}" get "virtualmachineinstance/${vmName}" \
                    -n "${P2P_HS_VM_NAMESPACE}" -o json 2>/dev/null \
                    | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null), nodeName: "<REDACTED>", created: .metadata.creationTimestamp}' || true
            done
        fi
    } > "${ARTIFACT_DIR}/hub-spoke-vm-create-status.txt" 2>&1 || true
    set -x
fi
