...
if [[ -n "${SHARED_DIR}" && -s "${SHARED_DIR}/proxy-conf.sh" ]]; then
    [[ $- == *x* ]] && isTracing=true || isTracing=false
    set +x
    # shellcheck disable=SC1090
    source "${SHARED_DIR}/proxy-conf.sh"
    [[ "${isTracing}" == 'true' ]] && set -x
fi
...
WaitProviderReady() {
    ...
    MgmtOc wait "provider/${providerName}" -n "${MTV_NAMESPACE}" \
        --for=condition=Ready --timeout="${MTV_HS_PLAN_READY_TIMEOUT}" 1>/dev/null
    true
}
...
WaitForSyncControllerReady() {
    ...
    oc --kubeconfig="${kc}" wait deployment/virt-synchronization-controller \
        ...
        --timeout="${MTV_HS_SYNC_CONTROLLER_WAIT}" 1>/dev/null
    true
}
...
PreflightCclm() {
    ...
    [[ "${fcGate}" == 'true' ]] || {
        : "ERROR: ForkliftController feature_ocp_live_migration is not enabled (got '${fcGate}')"
        return 1
    }
    ...
    envVal="$(MgmtOc get "deployment/${MTV_HS_FORKLIFT_CONTROLLER_NAME}" \
        ...
        || true)"
    [[ "${envVal}" == 'true' ]] || {
        : "ERROR: ForkliftController deployment FEATURE_OCP_LIVE_MIGRATION env is not true (got '${envVal}')"
        return 1
    }
...
RefreshProvidersForLivePlan() {
    ...
    MgmtOc wait "provider/${srcProvider}" -n "${MTV_NAMESPACE}" \
        --for=condition=Ready --timeout="${MTV_HS_PROVIDER_INVENTORY_REFRESH_WAIT}" 1>/dev/null
...
WaitMigrationSucceeded() {
    ...
    typeset dstKc="${5:?}"
    typeset dstNs="${6:?}"
    # shellcheck disable=SC2034  # syncStartedAt passed by name to CheckSyncStuck (nameref at line 661)
...
VerifyAllVmsMigrated() {
    ...
    for ((i = 1; i <= vmCount; i++)); do
        ...
        phase="$(oc --kubeconfig="${dstKc}" get "virtualmachineinstance/${vmName}" -n "${vmNs}" \
            -o jsonpath='{.status.phase}' || true)"
        [[ "${phase}" == 'Running' ]] || {
            : "ERROR: Migrated VM ${vmName} not Running on destination (phase='${phase}')"
            return 1
        }
    done
}
...
DumpDirectionDiagnostics() {
    ...
    set +x
    MgmtOc get plan,migration,networkmap,storagemap,provider -n "${MTV_NAMESPACE}" -o json \
        | jq '[.items[]? | {name: .metadata.name, kind: .kind, namespace: .metadata.namespace, phase: (.status.phase // null), conditions: [.status.conditions[]? | {type, status, reason}], created: .metadata.creationTimestamp}]' \
        > "${dirDiag}/hub-mtv-resources.json" 2>&1 || true
    MgmtOc get "plan/${planName}" -n "${MTV_NAMESPACE}" -o json \
        | jq '{name: .metadata.name, phase: (.status.phase // null), conditions: [.status.conditions[]? | {type, status, reason, message: (.message | gsub("\\b[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+\\b"; "<REDACTED-IP>") | gsub("https?://[^\\s\"]+"; "<REDACTED-URL>"))}], vms: [.spec.vms[]? | {name: .name}], targetNamespace: .spec.targetNamespace}' \
        > "${dirDiag}/plan-status.json" 2>&1 || true
    MgmtOc get "migration/${migName}" -n "${MTV_NAMESPACE}" -o json \
        | jq '{name: .metadata.name, phase: (.status.phase // null), conditions: [.status.conditions[]? | {type, status, reason, message: (.message | gsub("\\b[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+\\b"; "<REDACTED-IP>") | gsub("https?://[^\\s\"]+"; "<REDACTED-URL>"))}], started: .metadata.creationTimestamp}' \
        > "${dirDiag}/migration-status.json" 2>&1 || true
    MgmtOc get events -n "${MTV_NAMESPACE}" --sort-by='.lastTimestamp' -o json \
        | jq '[.items[]? | {type, reason, regarding: (.regarding.name // .involvedObject.name), message: (.message | gsub("\\b[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+\\b"; "<REDACTED-IP>") | gsub("https?://[^\\s\"]+"; "<REDACTED-URL>") | gsub("token=[^&\\s]+"; "token=<REDACTED>"))}]' \
        > "${dirDiag}/hub-mtv-events.json" 2>&1 || true
    set -x
    ...
    for ((i = 1; i <= vmCount; i++)); do
        ...
        set +x
        {
            oc --kubeconfig="${srcKc}" get "virtualmachine/${vmName}" \
                -n "${srcNs}" -o json 2>/dev/null \
                | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null), ready: (.status.ready // null), created: .metadata.creationTimestamp}' || true
            oc --kubeconfig="${srcKc}" get "virtualmachineinstance/${vmName}" \
                -n "${srcNs}" -o json 2>/dev/null \
                | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null), nodeName: "<REDACTED>", created: .metadata.creationTimestamp}' || true
            oc --kubeconfig="${dstKc}" get "virtualmachine/${vmName}" \
                -n "${dstNs}" -o json 2>/dev/null \
                | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null), ready: (.status.ready // null), created: .metadata.creationTimestamp}' || true
            oc --kubeconfig="${dstKc}" get "virtualmachineinstance/${vmName}" \
                -n "${dstNs}" -o json 2>/dev/null \
                | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null), nodeName: "<REDACTED>", created: .metadata.creationTimestamp}' || true
        } > "${dirDiag}/vm-${i}-status.json" 2>&1
        set -x
    done

    # VMIM status: allowlist only name/phase/conditions — no podIPs, node names, or annotations.
    oc --kubeconfig="${srcKc}" get vmim -n "${srcNs}" -o json 2>/dev/null \
        | jq '[.items[] | {name: .metadata.name, phase: .status.phase, conditions: (.status.conditions // [])}]' \
        > "${dirDiag}/source-vmim-status.json" 2>&1 || true
    oc --kubeconfig="${dstKc}" get vmim -n "${dstNs}" -o json 2>/dev/null \
        | jq '[.items[] | {name: .metadata.name, phase: .status.phase, conditions: (.status.conditions // [])}]' \
        > "${dirDiag}/dest-vmim-status.json" 2>&1 || true
    # virt-controller: errors and warnings only — strip lines containing IPs/hostnames.
    oc --kubeconfig="${srcKc}" logs -n "${MTV_HS_CNV_NAMESPACE}" \
        -l kubevirt.io=virt-controller --tail=200 2>/dev/null \
        | grep -E '^[EW][0-9]' \
        > "${dirDiag}/source-virt-controller-errors.log" 2>&1 || true
    oc --kubeconfig="${dstKc}" logs -n "${MTV_HS_CNV_NAMESPACE}" \
        -l kubevirt.io=virt-controller --tail=200 2>/dev/null \
        | grep -E '^[EW][0-9]' \
        > "${dirDiag}/dest-virt-controller-errors.log" 2>&1 || true
}
...
DumpDiagnostics() {
    ...
    (both)
        # Leg 1 spoke→hub: same spoke-VM set, source on spoke.
        ...
        # Leg 2 hub→spoke return: same spoke VMs now on hub after leg 1.
        # Use _revVmPrefix (spoke-vm prefix) and revTargetNs (where VMs landed on hub)
        # not _fwdVmPrefix (hub-vm) which only exists when P2P_HS_CREATE_HUB_VMS=true.
...
RunOneMigrationDirection() {
    ...
    if [[ -n "${ARTIFACT_DIR}" ]]; then
        ...
        {
            MgmtOc get "plan/${planName}" -n "${MTV_NAMESPACE}" -o json \
                | jq '{name: .metadata.name, kind: .kind,
                       phase: (.status.phase // null),
                       conditions: [.status.conditions[]? |
                         {type, status, reason,
                          message: (.message
                            | gsub("\\b[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+\\b"; "<REDACTED-IP>")
                            | gsub("https?://[^\\s\\"]+"; "<REDACTED-URL>"))}],
                       vms: [.spec.vms[]? | {name: .name}],
                       targetNamespace: .spec.targetNamespace}' || true
            MgmtOc get "migration/${migName}" -n "${MTV_NAMESPACE}" -o json \
                | jq '{name: .metadata.name, kind: .kind,
                       phase: (.status.phase // null),
                       conditions: [.status.conditions[]? |
                         {type, status, reason,
                          message: (.message
                            | gsub("\\b[0-9]+\\.[0-9]+\\.[0-9]+\\.[0-9]+\\b"; "<REDACTED-IP>")
                            | gsub("https?://[^\\s\\"]+"; "<REDACTED-URL>"))}],
                       created: .metadata.creationTimestamp}' || true
            PrintMigrationPipeline "${migName}"
            typeset -i i
            for ((i = 1; i <= vmCount; i++)); do
                typeset vmName="${vmPrefix}-${i}"
                oc --kubeconfig="${srcKc}" get "virtualmachine/${vmName}" \
                    -n "${vmNs}" -o json 2>/dev/null \
                    | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null),
                           ready: (.status.ready // null), created: .metadata.creationTimestamp}' || true
                oc --kubeconfig="${srcKc}" get "virtualmachineinstance/${vmName}" \
                    -n "${vmNs}" -o json 2>/dev/null \
                    | jq '{name: .metadata.name, kind: .kind, phase: (.status.phase // null),
                           nodeName: "<REDACTED>", created: .metadata.creationTimestamp}' || true
                oc --kubeconfig="${dstKc}" get "virtualmachine/${vmName}" \
        ...
        } > "${ARTIFACT_DIR}/hub-spoke-${direction}-migration-status.txt" 2>&1 || true
        set -x
    fi
    true
}
...
#   both              " spoke to hub then hub to spoke return (default; same VM set round-trips when P2P_HS_CREATE_HUB_VMS=false , no hub VMs pre-created)
...
(
    ...
    || "${cclmDirection}" == 'spoke-round-trip-ss' ]] || {
        : "ERROR: P2P_MIGRATION_DIRECTION must be one of: both, hub-to-spoke, spoke-to-hub, spoke-to-spoke, spoke-round-trip-ss (got '${cclmDirection}')"
        exit 1
    }
    ...
    if [[ "${cclmDirection}" == 'spoke-to-hub' || "${cclmDirection}" == 'both' ]]; then
        ...
        if [[ "${cclmDirection}" == 'both' ]]; then
            JStep "[both] Post-leg-1: Cleanup source spoke VM resources" \
                CleanupDestinationStaleResources \
