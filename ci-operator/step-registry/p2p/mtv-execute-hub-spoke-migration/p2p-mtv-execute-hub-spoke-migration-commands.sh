...
# especially in the hub-spoke ACM-managed topology after a fresh cluster provisioning.
ProbeCclmSyncPortFromPod() {
    typeset kc="${1:?}"
    typeset ns="${2:?}"
    typeset podName="${3:?}"
    typeset destIp="${4:?}"
    typeset -i attempt=0 maxAttempts="${MTV_HS_CCLM_SYNC_PROBE_RETRIES}" retrySecs=20
...
# suppresses set -e; bare [[ ... ]] guards are silently swallowed in that context.
PreflightCclmSyncConnectivity() {
    ...
    [[ -n "${destSyncIp}" ]] || { : "ERROR: no Running virt-synchronization-controller found on dst cluster" >&2; return 1; }
    ...
    [[ -n "${srcLauncherPod}" ]] || { : "ERROR: no Running virt-launcher pod for ${vmPrefix}-1 on src cluster" >&2; return 1; }
