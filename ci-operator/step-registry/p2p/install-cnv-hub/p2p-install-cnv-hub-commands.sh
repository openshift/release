...
DumpDiagnostics() {
    ...
    # Get CNV resources without -o wide (omits NODE/IP columns)
    oc get subscription.operators.coreos.com,csv,hyperconverged,kubevirt \
        -n openshift-cnv -o custom-columns=NAME:.metadata.name,NAMESPACE:.metadata.namespace,KIND:.kind 2>&1 | tee "${diagDir}/cnv-resources.txt" || true
    ...
    oc get events -n openshift-cnv --sort-by='.lastTimestamp' \
      -o custom-columns='LAST:.lastTimestamp,TYPE:.type,REASON:.reason,OBJECT:.involvedObject.kind/.involvedObject.name' \
        > "${diagDir}/cnv-events.txt" 2>>"${diagDir}/cnv-events.stderr" || true
    
    # Get pod info using jq to omit NODE and NOMINATED_NODE columns
    oc get pods -n openshift-cnv -o json | \
        jq -r '[.items[] | {NAME: .metadata.name, NAMESPACE: .metadata.namespace, READY: (.status.containerStatuses | map(.ready) | map(tostring) | join(",")), STATUS: .status.phase, RESTARTS: (.status.containerStatuses[0].restartCount // 0)}] | .[] | "\(.NAME)\t\(.NAMESPACE)\t\(.READY)\t\(.STATUS)\t\(.RESTARTS)"' \
