...
COCL_OPERATOR_PATCH_URL="https://patch-diff.githubusercontent.com/raw/confidential-clusters/cocl-operator/pull/78.patch"
...
while [ $SECONDS -lt $TIMEOUT ]; do
    ...
    echo "Still waiting for pods to be ready: $NOT_RUNNING_PODS"
    sleep 10
    SECONDS=$((SECONDS + 10))
done
...
if [ $SECONDS -ge $TIMEOUT ]; then
    ...
    if [ -n "$FAILED_PODS" ]; then
        echo "The following pods are not in a 'Running' or 'Succeeded' state:"
