...
echo "$(date -u --rfc-3339=seconds) - Fetching ImageRegistry CR..."
...
# Get the registry bucket name from the CR (format: <infra-id>-image-registry-<region>-<random>)
if [[ -n "${REGISTRY_CONFIG}" ]]; then
  REGISTRY_BUCKET=$(echo "${REGISTRY_CONFIG}" | jq -r '.spec.storage.gcs.bucket // empty')
fi
if [[ -n "${REGISTRY_BUCKET:-}" ]]; then
  ...
  echo "$(date -u --rfc-3339=seconds) - WARNING: Could not determine registry bucket name from CR, skipping bucket check"
fi
