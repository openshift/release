...
# rather than to the Compute Engine Service Agent above.
echo "$(date -u --rfc-3339=seconds) - Granting roles/cloudkms.cryptoKeyEncrypterDecrypter to CI provisioner service account..."
gcloud kms keys add-iam-policy-binding "${KEY_NAME}" \
  --keyring="${KEY_RING_NAME}" \
  --location="${LOCATION}" \
  --project="${GOOGLE_PROJECT_ID}" \
  ...
  --role="roles/cloudkms.cryptoKeyEncrypterDecrypter"
echo "IAM binding granted to ${sa_email}."
