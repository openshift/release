...
##########Create Bastion#############
#####################################
# Attach the bastion to the control-plane subnet. GCP VPC firewall rules are
# tag-scoped, so the ingress rule created below applies only to this VM.
# Azure NSGs are subnet-scoped and therefore use a dedicated bastion subnet;
# AWS attaches the bastion to a public subnet with its own security group.
...
# Include icmp so cluster nodes can reach the bastion. Installer ICMP rules
# target master/worker tags only and do not apply to this instance.
