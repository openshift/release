...
for yaml_filename in $(ls -p "${creds_requests_dir}"/*.yaml | awk -F'/' '{print $NF}'); do
  ...
  eval "$cmd"
  #We do not need to record SA name for post-cleanup action because 
  # installer will delete it when launch "openshift-install destroy" command
  #Installer filters service accounts with a display name prefixed 
  # with the cluster's infra ID when process destroy
  #https://github.com/openshift/installer/blob/422e17192462f2b5d5572d8600096949d3c14634/pkg/destroy/gcp/serviceaccount.go#L13-L14
