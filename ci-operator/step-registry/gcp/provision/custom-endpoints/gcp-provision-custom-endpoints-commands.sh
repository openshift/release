...
if [[ -n "${PRIVATE_SERVICE_CONNECT_ADDRESS}" ]]; then
  ip_address="${PRIVATE_SERVICE_CONNECT_ADDRESS}"
fi
CMD="gcloud compute addresses create ${CLUSTER_NAME}-psc-address --global --purpose=PRIVATE_SERVICE_CONNECT --addresses=${ip_address} --network=${NETWORK}"
run_command "${CMD}"
