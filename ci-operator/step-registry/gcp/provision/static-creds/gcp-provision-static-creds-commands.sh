...
function create_credentials_manifests()
{
  ...
  local target_dir="${4}"
  cat << EOF > ${target_dir}/99_${namespace}_${name}-secret.yaml
apiVersion: v1
kind: Secret
metadata:
  namespace: ${namespace}
...
if [ X"${FEATURE_SET}" != X"TechPreviewNoUpgrade" ]; then
  # workaround
  ...
  echo "WARN: Workaround for https://bugzilla.redhat.com/show_bug.cgi?id=2035903, removing TechPreviewNoUpgrade CredentialsRequests"
...
for yaml_filename in $(ls -p "${creds_requests_dir}"/*.yaml | awk -F'/' '{print $NF}'); do
  echo "Processing ${yaml_filename}"
  roles=$(yq-go r "${creds_requests_dir}/${yaml_filename}" "spec.providerSpec.predefinedRoles" | sed 's/-//g')
  ...
  if [ -z "$roles" ] || [ -z "$secret_name" ] || [ -z "$secret_namespace" ] || [ -z "$metadata_name" ]; then
    echo "$(date -u --rfc-3339=seconds) - ERROR: spec.providerSpec.predefinedRoles, spec.secretRef.name, spec.secretRef.namespace or metadata.name is empty"
  ...
  sa_name="${infra_name:0:12}-${metadata_name:0:11}"-`echo $RANDOM`
  sa_display_name="${infra_name}-${metadata_name}"
