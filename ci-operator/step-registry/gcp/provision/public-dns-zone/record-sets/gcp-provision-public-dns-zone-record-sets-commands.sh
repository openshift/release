...
if [ ! -e ${public_custom_dns_json} ]; then
...
count=$(jq '.|length' ${public_custom_dns_json})
for i in $(seq 0 $((count-1)));
do
    name=$(jq --argjson i $i '.[$i].name' ${public_custom_dns_json})
    target=$(jq --argjson i $i '.[$i].target' ${public_custom_dns_json})
    record_type=$(jq --argjson i $i '.[$i].record_type' ${public_custom_dns_json})
