...
if [ X"${CREATE_SSH_BASTION}" == X"no" ]; then
  echo "CREATE_SSH_BASTION is set to 'no', so nothing to do." && exit 0
fi
...
if [[ -s "${SHARED_DIR}/xpn.json" ]]; then
  ...
  CLUSTER_NETWORK="$(jq -r '.clusterNetwork' "${SHARED_DIR}/xpn.json")"
...
cat > "/tmp/bastion_firewall.yaml" << EOF
imports:
- path: /tmp/bastion_firewall.py

resources:
- name: bastion-firewall
  type: /tmp/bastion_firewall.py
  properties:
...
if [ X"${DISCONNECTED_NETWORK}" == X"yes" ]; then
  cat > "/tmp/bastion_firewall.py" << EOF
def GenerateConfig(context):
  resources = [
		{
		  'name': context.properties['infra_id'] + '-bastion-ingress-allow',
          ...
          {
		        'IPProtocol': 'tcp',
		        'ports': ['22', '3128', '3129', '5000', '8080']
		      }
        ],
		    'sourceRanges': ['0.0.0.0/0'],
		    'targetTags': [context.properties['infra_id'] + '-bastion']
      }
	  },
		{
		  'name': context.properties['infra_id'] + '-bastion-egress-allow',
		  'type': 'compute.v1.firewall',
		  'properties': {
        'direction': 'EGRESS',
		    'network': context.properties['cluster_network'],
		    'allowed': [
          {
		    ...
		    'sourceTags': [context.properties['infra_id'] + '-bastion'],
...
gcloud ${project_option} deployment-manager deployments create "${CLUSTER_NAME}-bastion-firewall" --config "/tmp/bastion_firewall.yaml"

cat > "${SHARED_DIR}/ssh-bastion-destroy.sh" << EOF
gcloud compute instances delete -q "${CLUSTER_NAME}-bastion" --zone=${ZONE_0}
gcloud ${project_option} deployment-manager deployments delete -q "${CLUSTER_NAME}-bastion-firewall"
EOF
