...
#!/bin/bash
set -xeuo pipefail
sudo sh -c 'echo "GRUB_CMDLINE_LINUX=\"$(sed "s/\"/\\\\\"/g" /etc/default/grub | grep -E "^GRUB_CMDLINE_LINUX=" | sed "s/^GRUB_CMDLINE_LINUX=\"//;s/\"$//") cgroup_enable=memory systemd.unified_cgroup_hierarchy=0\"" > /etc/default/grub'
