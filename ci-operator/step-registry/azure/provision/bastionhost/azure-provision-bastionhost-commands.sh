...
if [[ -z "${bastion_subnet}" ]] && [[ -z "${bastion_nsg}" ]]; then
    ...
    if [[ "${IPSTACK}" == "dualstack" ]]; then
        vnet_subnet_address_parameter="${vnet_subnet_address_parameter} ${AZURE_VNET_IPV6_ADDRESS_PREFIXES}"
    fi
...
EOF
    ...
    az network public-ip create --resource-group ${bastion_rg} --name ${bastion_name}PublicIPv4 --sku Standard --version IPv4
    az network nic create --resource-group ${bastion_rg} --name ${bastion_name}NIC --vnet-name ${bastion_vnet_name} --subnet ${bastion_subnet} --network-security-group '' --public-ip-address ${bastion_name}PublicIPv4
    if [[ "${IPSTACK}" == "dualstack" ]]; then
        az network public-ip create --resource-group ${bastion_rg} --name ${bastion_name}PublicIPv6 --sku Standard --version IPv6
        az network nic ip-config create --resource-group ${bastion_rg} --name ${bastion_name}IPv6config --nic-name ${bastion_name}NIC --private-ip-address-version IPv6 --vnet-name ${bastion_vnet_name} --subnet ${bastion_subnet} --public-ip-address ${bastion_name}PublicIPv6
    fi
