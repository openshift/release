...
NGS_ID="$(az network nsg list --query "[?name=='${NSG_NAME}'].id" -o tsv)"
...
az network vnet subnet create --name "$SUBNET_NAME" --resource-group "$VNET_RESOURCE_GROUP" --vnet-name "$VNET_NAME" \
    --address-prefix "$SUBNET_ADDRESS_PREFIX" --network-security-group "$NGS_ID"
...
echo "$NGS_ID" > "${SHARED_DIR}/azure_nsg_id"
