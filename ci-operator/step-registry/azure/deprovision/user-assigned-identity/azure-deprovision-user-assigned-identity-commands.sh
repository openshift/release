...
function check_master_ready() {
    ...
    while (( try < max_try )); do
        ...
        if (( node_num == total_nodes_count )) && oc wait --for condition=Progressing=false controlplanemachineset/cluster -n openshift-machine-api; then
            echo "$(date -u --rfc-3339=seconds) - all master nodes are recreated, get ready"
            break
        fi
...
# * update cpms to to remove field managedIdentity, wait for all master nodes recreated
...
# Remove role related permission attached to cluster sp on subscription level
if [[ -f "${SHARED_DIR}/azure_role_assignment_destroy" ]]; then
    echo "Remove role related permissions from cluster sp on sbuscription level"
    cmd=$(< "${SHARED_DIR}/azure_role_assignment_destroy")
    run_command "${cmd}"
else
    echo "Unable to find file ${SHARED_DIR}/azure_role_assignment_destroy, exit..."
    exit 1
fi
