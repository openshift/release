...
# update subscription to use the custome catalog
#oc patch subscription rhods-operator -n redhat-ods-operator \
#--type='merge' -p '{"spec":{"source":"rhoai-catalog-dev"}}'

# apply a new image that points to Fast Channel
oc patch catalogsource rhoai-catalog-dev -n openshift-marketplace --type='merge' -p \
'{"spec": {"image": "quay.io/rhoai/rhoai-fbc-fragment:rhoai-${UPGRADE_VERSION}"}}'
