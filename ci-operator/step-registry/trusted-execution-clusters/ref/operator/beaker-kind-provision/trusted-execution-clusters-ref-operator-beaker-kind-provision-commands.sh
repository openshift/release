...
# Deploy Kind Cluster using operator Setup
progress "Deploying Kind cluster using operator setup"
...
if ! timeout "${KIND_CREATE_TIMEOUT}" ssh "${SSHOPTS[@]}" "${BEAKER_USER}@${BEAKER_IP}" \
  "export PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:\${PATH:-}; \
   ...
   bash /tmp/beaker-setup.sh '${KIND_CLUSTER_NAME}' '${CONTAINER_RUNTIME}' '${BEAKER_IP}' '${OPERATOR_REPO}' '${OPERATOR_BRANCH}'"; then
