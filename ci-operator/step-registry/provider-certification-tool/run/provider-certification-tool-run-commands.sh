...
ls "${ARTIFACT_DIR}/certification-results/*.tar"
echo "s3://openshift-provider-certification/baseline-results/${VERSION}-${DATE}.tar.gz"
echo '{"platform-type":"'"${CLUSTER_TYPE}"'"}'

aws s3 cp "${ARTIFACT_DIR}/certification-results/*.tar" "s3://openshift-provider-certification/baseline-results/${VERSION}-${DATE}.tar.gz" \
