...
do
    ...
    if [ "$(oc get pods -l deployment='hello-openshift' -o=jsonpath='{.items[*].status.phase}')" == "Running" ]; then
    ...
    if [[ $COUNTER -eq $MAX_RETRY ]]; then
        ...
        oc get pods --l deployment='hello-openshift'
        exit 1
    fi
    sleep $INTERVAL
done
oc expose service/hello-openshift
TEST_ROUTE=$(oc get route hello-openshift -o=jsonpath='{.status.ingress[?(@.routerName=="default")].host}')

echo "Validating the cert-manager customized default ingress certificate"
# The CA_FILE will also be used later to update KUBECONFIG
