...
function configure_cloud_credentials() {
    ...
    run_command "oc apply -f - <<< ${manifest}"
    ...
    echo "Configuring the DNS nameservers for DNS01 recursive self-check, to avoid overlaps between the target hosted zone and the cluster's default private hosted zone..."
...
function create_aws_route53_clusterissuer() {
    local aws_credential_request=$(cat <<EOF
...
EOF
    ...
    local base_domain=$(oc get dns cluster -o=jsonpath='{.spec.baseDomain}')
    local target_dns_domain=$(cut -d '.' -f 1 --complement <<< "$base_domain")
    local public_zone_id=$(oc get dns cluster -o=jsonpath='{.spec.publicZone.id}')
...
EOF
    ...
    echo "Retrieving configs to be used in the ClusterIssuer spec..."
    local project_id=$(oc get infrastructure cluster -o=jsonpath='{.status.platformStatus.gcp.projectID}')
