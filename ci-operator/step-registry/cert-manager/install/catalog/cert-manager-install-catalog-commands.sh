...
mirror:
  ...
  - name: ghcr.io/letsencrypt/pebble:2.7.0
EOF
...
# (TEMP) set ISCP to map 'registry.connect.redhat.com/hashicorp/vault' to 'docker.io/hashicorp/vault'
# it should be removed once we onboard external registry dependencies to 'quay.io/openshifttest'
cat << EOF | oc apply -f -
