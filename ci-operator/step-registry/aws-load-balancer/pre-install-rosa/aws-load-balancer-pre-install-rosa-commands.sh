...
if [ -n "${SSO_CLIENT_ID}" ] && [ -n "${SSO_CLIENT_SECRET}" ]; then
    echo "Logging into staging with SSO credentials using rosa cli $(rosa version)"
    rosa login --env "staging" --client-id "${SSO_CLIENT_ID}" --client-secret "${SSO_CLIENT_SECRET}" || { echo "Login failed"; exit 1; }
