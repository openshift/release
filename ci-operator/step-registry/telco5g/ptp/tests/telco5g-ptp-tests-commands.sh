...
spec:
    ...
    - name: priv
      ...
      command:
        ...
        - |
          ...
          if [[ !-z "${USE_UPSTREAM}" && "${USE_UPSTREAM}" == "True" ]]; then
  ...
  jobdefinition=$(sed "s#USE_UPSTREAM_VAL#${T5CI_DEPLOY_UPSTREAM}#" <<<"$jobdefinition")
...
# run latest release on upstream main branch
if [[ -z "${T5CI_DEPLOY_UPSTREAM}" ]]; then
...
# deploy ptp-operator
if [[ -z "${T5CI_DEPLOY_UPSTREAM}" || "${T5CI_DEPLOY_UPSTREAM}" == "False" ]]; then
