...
if [[ "$T5CI_JOB_TYPE" == "sno-cnftests" ]]; then
    export FEATURES="${FEATURES:-performance sriov sctp dpdk}"
else
    export FEATURES="${FEATURES:-sriov performance sctp xt_u32 ovn metallb multinetworkpolicy vrf bondcni tuningcni dpdk}"
fi
