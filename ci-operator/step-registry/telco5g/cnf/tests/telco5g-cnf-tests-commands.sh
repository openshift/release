...
if $RUN_VALIDATIONS; then
    ...
    FEATURES=$VALIDATIONS_FEATURES FEATURES_ENVIRONMENT="ci" stdbuf -o0 make feature-deploy-on-ci 2>&1 | tee ${SHARED_DIR}/cnf-validations-run.log ${ARTIFACT_DIR}/cnf-validations-run2.log || val_status=$?
fi
...
if [[ ${val_status} -eq 0 ]] && $RUN_TESTS; then
    ...
    FEATURES=$TEST_RUN_FEATURES FEATURES_ENVIRONMENT="ci" stdbuf -o0 make functests 2>&1 | tee ${SHARED_DIR}/cnf-tests-run.log ${ARTIFACT_DIR}/cnf-tests-run2.log || status=$?
fi
popd
