...
- name: Fetch kubeconfigs for cluster
  ...
  - name: Copy kubeconfig from ZTP directory
    shell: cp /home/kni/ztp-jobs/${SNO_NAME}/ztphub/out/${CLUSTER_NAME}-kubeadmin-kubeconfig /home/kni/.kube/ztp_config_${CLUSTER_NAME}
...
ANSIBLE_LOG_PATH=$ARTIFACT_DIR/ansible.log ANSIBLE_STDOUT_CALLBACK=debug ansible-playbook \
    ...
    -e disconnected_env_pull_secret=/home/kni/pull-secret.txt $PLAYBOOK_ARGS || status=$?
...
if [[ "$T5_JOB_DESC" != "periodic-ztp-cnftests" ]]; then
