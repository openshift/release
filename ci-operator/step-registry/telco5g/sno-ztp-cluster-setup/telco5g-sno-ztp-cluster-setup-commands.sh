...
cat > "${SHARED_DIR}/diagnose-telco5g-sno-ztp-cluster-setup.early" << EOF
export JOB_NAME=${JOB_NAME:-unknown}
export STEP_NAME=telco5g-sno-ztp-cluster-setup
...
rm -f "${SHARED_DIR}/diagnose-telco5g-sno-ztp-cluster-setup.early"
cat > "${SHARED_DIR}/diagnose-telco5g-sno-ztp-cluster-setup" << EOF
export JOB_NAME=${JOB_NAME:-unknown}
export STEP_NAME=telco5g-sno-ztp-cluster-setup
export T5CI_VERSION=${T5CI_VERSION:-unknown}
export T5CI_JOB_TYPE=${T5CI_JOB_TYPE:-unknown}
...
if [[ "$T5_JOB_DESC" != "periodic-sno-ztp-cnftests" ]]; then
...
[[ ${status} -eq 0 ]] && rm -f "${SHARED_DIR}/diagnose-telco5g-sno-ztp-cluster-setup"
