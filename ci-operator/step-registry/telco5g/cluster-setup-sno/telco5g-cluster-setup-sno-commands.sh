...
- name: Grab and run kcli to install openshift cluster
  ...
  - name: Create a file with cluster name
    shell: echo "sno-{{ cluster.stdout }}" > $SHARED_DIR/cluster_name
...
PLAN_NAME="sno-${CLUSTER_NAME}_ci"
...
- name: Grab and run kcli to install openshift cluster
  ...
  - name: Remove lock file
    file:
      path: /home/kni/us_sno-${CLUSTER_NAME}_ready.txt
      state: absent

  - name: Run deployment
    shell: kcli create plan --force --paramfile /home/kni/params_sno-${CLUSTER_NAME}.yaml ${PLAN_NAME} $KCLI_PARAM
    args:
      chdir: ~/kcli-openshift4-baremetal

  - name: Try to grab file to see install finished
    shell: >-
      kcli scp root@sno-${CLUSTER_NAME}-installer:/root/cluster_ready.txt /home/kni/us_sno-${CLUSTER_NAME}_ready.txt &&
      ls /home/kni/us_sno-${CLUSTER_NAME}_ready.txt
    register: result
  ...
  - name: Check if successful
    stat: path=/home/kni/us_sno-${CLUSTER_NAME}_ready.txt
    register: ready

  - name: Grab the kcli log from installer
    shell: >-
      kcli scp root@sno-${CLUSTER_NAME}-installer:/var/log/cloud-init-output.log /tmp/kcli_sno-${CLUSTER_NAME}_cloud-init-output.log
    ignore_errors: true

  - name: Grab the log from HV to artifacts
    fetch:
      src: /tmp/kcli_sno-${CLUSTER_NAME}_cloud-init-output.log
  ...
  - name: Show last logs from cloud init if failed
    shell: >-
      kcli ssh root@sno-${CLUSTER_NAME}-installer 'tail -100 /var/log/cloud-init-output.log'
    when: ready.stat.exists == False
    ignore_errors: true

  - name: Show bmh objects when failed to install
    shell: >-
      kcli ssh root@sno-${CLUSTER_NAME}-installer 'oc get bmh -A'
    when: ready.stat.exists == False
    ignore_errors: true
...
- name: Fetch kubeconfig for cluster
  ...
  - name: Copy kubeconfig from installer vm
    shell: kcli scp root@sno-${CLUSTER_NAME}-installer:/root/ocp/auth/kubeconfig /home/kni/.kube/config_sno-${CLUSTER_NAME}

  - name: Add skip-tls-verify to kubeconfig
    replace:
      path: /home/kni/.kube/config_sno-${CLUSTER_NAME}
      regexp: '    certificate-authority-data:.*'
      replace: '    insecure-skip-tls-verify: true'

  - name: Grab the kubeconfig
    fetch:
      src: /home/kni/.kube/config_sno-${CLUSTER_NAME}
...
- name: Fetch information about cluster
  ...
  - name: Get cluster version
    shell: kcli ssh root@sno-${CLUSTER_NAME}-installer 'oc get clusterversion'

  - name: Get bmh objects
    shell: kcli ssh root@sno-${CLUSTER_NAME}-installer 'oc get bmh -A'

  - name: Get nodes
    shell: kcli ssh root@sno-${CLUSTER_NAME}-installer 'oc get node'
EOF
...
- name: Check if cluster is ready
  ...
  - name: Check if cluster is available
    shell: kcli ssh root@sno-${CLUSTER_NAME}-installer "oc get clusterversion -o=jsonpath='{.items[0].status.conditions[?(@.type=='\''Available'\'')].status}'"
