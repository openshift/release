...
- name: Grab and run kcli to install openshift cluster
  ...
  - name: Discover cluster to run job
    command: python3 ~/telco5g-lab-deployment/scripts/upstream_cluster_all.py --get-cluster --cluster-name cnfdu14
    register: cluster
    environment:
...
- name: Release cluster $CLUSTER_NAME
  ...
  - name: Release cluster from job
    command: python3 ~/telco5g-lab-deployment/scripts/upstream_cluster_all.py --release-cluster cnfdu14
EOF
