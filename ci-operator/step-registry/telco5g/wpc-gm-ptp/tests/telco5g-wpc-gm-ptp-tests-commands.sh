...
export PTP_TEST_MODE=t-gm
export JUNIT_OUTPUT_FILE=test_results_${PTP_TEST_MODE}.xml
set_events_output_file
make functests || temp_status_wpcgm=$?
...
status=0
if [[ $temp_status_wpcgm != 0 || $temp_status_vst != 0 ]]; then
  echo  "status for WPC GM scenario = $temp_status_wpcgm"
  echo  "status for VSE sync tests = $temp_status_vst"
  status=1
fi
