...
echo "Waiting for Windows nodes to become Ready..."
timeout 30m bash -c '
    while true; do
        READY=$(oc get nodes -l kubernetes.io/os=windows --no-headers 2>/dev/null | grep -c "Ready" || echo 0)
        echo "Windows nodes ready: ${READY}/'${BYOH_NUM_WORKERS}'"
