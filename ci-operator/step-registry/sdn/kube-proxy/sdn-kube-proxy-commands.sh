...
date
if ! oc wait --for=condition=Ready -n kube-proxy-test pod/kube-proxy; then
    echo "kube-proxy failed to become ready. Dumping logs"
...
for service in kubernetes.default dns-default.openshift-dns router-default.openshift-ingress; do
    ...
    echo "Looking for ${service} cluster IP (${clusterIP}) and endpoint IPs (${endpointIPs}) in ruleset"
    for ip in ${clusterIP} ${endpointIPs}; do
        if ! grep --fixed-strings "${ip}" "${TMPDIR}/nft.out" | indent; then
