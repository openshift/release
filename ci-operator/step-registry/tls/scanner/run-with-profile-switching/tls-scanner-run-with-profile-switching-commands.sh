...
switch_profile() {
    ...
    current_profile=$(oc get apiserver cluster -o jsonpath='{.spec.tlsSecurityProfile.type}')
    ...
    degraded_operators=$(oc get co -o json | jq -r '.items[] | select(.status.conditions[] | select(.type=="Degraded" and .status=="True")) | .metadata.name' || echo "")
