...
# Query datagrepper for latest resolved WMCO index image
function get_latest_wmco_index_image() {
  ...
  log "Searching for WMCO index image for OCP ${version} (tag: ${ocp_tag})"
  local image
  image=$(curl -sf "https://datagrepper.engineering.redhat.com/raw?topic=/topic/VirtualTopic.eng.iib.build.state&contains=windows-machine-conf-tenant&start=${start}&end=${end}" \
  ...
  if [ -z "$image" ]; then
    log "Error: Could not fetch WMCO index image for OCP ${version} from datagrepper."
    return 1
  fi

  log "Found WMCO index image: ${image}"
  echo "${image}"
}
