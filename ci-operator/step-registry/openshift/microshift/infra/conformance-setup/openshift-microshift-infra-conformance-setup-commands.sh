...
ssh "${INSTANCE_PREFIX}" "sudo systemctl stop microshift crio kubepods.slice || true"
ssh "${INSTANCE_PREFIX}" "sudo systemctl status crio"
ssh "${INSTANCE_PREFIX}" "sudo crictl ps -a"
ssh "${INSTANCE_PREFIX}" "sudo sed -i 's/resources/#&/g' /etc/crio/crio.conf.d/11-microshift-ovn.conf"
ssh "${INSTANCE_PREFIX}" "sudo cat /etc/crio/crio.conf.d/11-microshift-ovn.conf"
ssh "${INSTANCE_PREFIX}" "sudo systemctl daemon-reload"
ssh "${INSTANCE_PREFIX}" "sudo systemctl restart microshift"
