...
if [[ "${SUITE}" == "nvidia-device-plugin" ]]; then
    ssh "${INSTANCE_PREFIX}" \
        "git clone --filter=blob:none --no-checkout --depth 1 --sparse https://github.com/pmtk/microshift.git --branch reorg-nvidia-setup /home/${HOST_USER}/microshift-main && \
            cd /home/${HOST_USER}/microshift-main && \
            git sparse-checkout add ./scripts/ci-nvidia-device-plugin/ && \
            git checkout reorg-nvidia-setup"
    ...
    #         cd /home/${HOST_USER}/microshift-main && \
    #         git sparse-checkout add ./scripts/ci-ai-model-serving/ && \
    #         git checkout main"
