...
if ! curl -Ls "${YQ_URL}" | tee "$HOME/bin/yq" | sha256sum -c /tmp/sum.txt >/dev/null 2>/dev/null; then
  echo "ERROR: Expected file at ${YQ_URL} to have checksum ${YQ_HASH} but instead got $(sha256sum <"$HOME/bin/yq" | cut -d' ' -f1)"
  strings "$HOME/bin/yq"
  exit 1
fi
