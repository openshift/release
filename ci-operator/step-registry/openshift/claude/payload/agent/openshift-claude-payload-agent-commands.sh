...
if [[ "${JOB_TYPE:-}" == "presubmit" ]]; then
    if [[ "${JOB_NAME:-}" == rehearse-* ]]; then
        SLACK_MESSAGE_PREFIX=":test_tube: *REHEARSAL*"
    else
        SLACK_MESSAGE_PREFIX=":test_tube: *PRESUBMIT*"
    fi
    if [[ -n "${REPO_OWNER:-}" && -n "${REPO_NAME:-}" && -n "${PULL_NUMBER:-}" ]]; then
        SLACK_MESSAGE_PREFIX+=" — generated by <https://github.com/${REPO_OWNER}/${REPO_NAME}/pull/${PULL_NUMBER}|${REPO_OWNER}/${REPO_NAME}#${PULL_NUMBER}>"
    fi
    SLACK_MESSAGE_PREFIX+="
    ...
    if [[ "${NOT_STARTED}" -gt 0 ]]; then
        ...
        if [[ "${FAILED}" -eq 0 ]]; then
            ...
            if [[ -n "${SLACK_WEBHOOK}" ]]; then
                ...
                SLACK_TEXT="${SLACK_MESSAGE_PREFIX}:green-check: *Payload Accepted for <${PAYLOAD_URL}|${PAYLOAD_TAG}>*
...
if [[ -n "${SLACK_WEBHOOK}" ]]; then
    SLACK_TEXT="${SLACK_MESSAGE_PREFIX}:claude-thinking: *Payload Analysis for <${PAYLOAD_URL}|${PAYLOAD_TAG}>*
