...
# private pvcctl image can be pulled.
#
# Requires HOME to already be set to the private directory created by
# install_required_tools. The exported REGISTRY_AUTH_FILE persists for later
# functions (e.g. setup_config_yaml) that also run podman.
#
# Globals:
#   SECRETS_DIR        – (in)  directory containing the quay-io-ci auth file.
#   HOME               – (in)  private directory the auth.json is written under.
#   REGISTRY_AUTH_FILE – (out) exported so podman reads the installed auth.json.
# Returns:
#   0 on success; exits non-zero if the quay-io-ci secret is missing.
#######################################
function setup_registry_auth() {
	log_info "Installing registry credentials..."
...
#      resolvable on PATH.
#   5. Installs the quay.io registry credentials (via setup_registry_auth) and
#      pulls the private quay.io/powercloud/pvcctl:${PVCCTL_VERSION} image,
#      retrying the pull up to 3 times before exiting non-zero.
#   6. Writes a pvcctl wrapper script into /tmp/bin that runs that image.
#   7. Verifies that PowerVC-Tool, UploadRhcos, yq-v4, and pvcctl are
#      resolvable on PATH.
#
# Globals:
...
#   SECRETS_DIR          – (in)  used by setup_registry_auth for quay-io-ci.
#   HOME                 – (out) overwritten with the newly created private
#                                temp directory.
#   PATH                 – (out) /tmp/bin prepended so downloaded tools are
#                                found first.
#   REGISTRY_AUTH_FILE   – (out) exported by setup_registry_auth for podman.
# Returns:
...
function install_required_tools() {
	...
	local podman_location="quay.io/powercloud/pvcctl:${PVCCTL_VERSION}"

	# Install the registry credentials before pulling the private pvcctl image.
	setup_registry_auth

	# First make sure that we can download pvcctl
	if ! retry_command 3 5 "podman pull ${podman_location}" \
		podman pull "${podman_location}"; then
		log_error "Could not podman pull ${podman_location}"
		exit 1
	fi

	# Create pvcctl command. The heredoc delimiter is unquoted so podman_location
...
#!/bin/bash
podman run --rm -v "\${PWD}:/work:Z" -w /work "${podman_location}" "\$@"
___EOF___
...
#
# The function performs the following steps in order:
#   1. Changes to ${HOME} so config.yaml is written to a private, writable path.
#   2. Runs `pvcctl image import --gen-config` to emit a config.yaml template
#      into the current directory. Registry auth was already installed by
#      setup_registry_auth (from install_required_tools).
...
#
# Globals:
#   SECRETS_DIR        – (in)  directory containing the SVC_* files and
#                              clouds.yaml mounted from the CI secret.
#   HOME               – (in)  private directory set by install_required_tools;
#                              config.yaml is written under it.
#   REGISTRY_AUTH_FILE – (in)  set by setup_registry_auth so podman can run.
# Returns:
...
function setup_config_yaml() {
	...
	# Registry credentials and REGISTRY_AUTH_FILE were set up earlier by
	# setup_registry_auth (called from install_required_tools) so the private
	# pvcctl image is already available here.

	# Generate the config.yaml template into the current directory (${HOME}).
	...
	# Read each SVC secret in a standalone assignment first. If a value were read
	# inline as a command prefix (SVC_HOST="$(read_secret ...)" yq-v4 ...), the
	# read_secret exit would only kill the command-substitution subshell while
	# yq-v4 still returned success, letting an empty value be written. Declaring
	# local separately from the assignment keeps errexit able to see the failure
	# (a combined `local var=$(...)` would mask it).
	local svc_host
	local svc_user
	local svc_password
