...
echo "Checking SSH connectivity to ${HOST_PUBLIC_IP} (best-effort from this step)..."
ssh_ok=0
...
if [ "${ssh_ok}" -eq 0 ]; then
  echo "WARNING: SSH not reachable from this step after 20 attempts — downstream steps will retry with their own SSH wrappers"
fi
