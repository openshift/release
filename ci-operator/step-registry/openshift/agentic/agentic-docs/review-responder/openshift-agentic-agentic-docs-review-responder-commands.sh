...
echo "=== Agentic Docs Review Responder ==="
...
export GITHUB_TOKEN
TASK_DESCRIPTOR=$(cat "${SHARED_DIR}/task-descriptor")

# Derive fork repo from SHARED_DIR (written by doc-creator) or from UPSTREAM_REPO
if [[ -f "${SHARED_DIR}/fork-repo" ]]; then
    FORK_REPO=$(cat "${SHARED_DIR}/fork-repo")
else
    FORK_REPO="openshift-agentic-docs/${UPSTREAM_REPO##*/}"
fi
...
if [[ -f "${SHARED_DIR}/pr-number" ]]; then
    ...
    echo "Searching for PR associated with ${TASK_DESCRIPTOR}..."
    PR_JSON=$(gh pr list --repo "${UPSTREAM_REPO}" --state open --search "${TASK_DESCRIPTOR}" --json number --limit 1 2>/dev/null || echo "[]")
    PR_NUM=$(echo "${PR_JSON}" | jq -r '.[0].number // empty')
    if [[ -z "${PR_NUM}" ]]; then
        echo "No open PR found for ${TASK_DESCRIPTOR}. Nothing to do."
        exit 0
    fi
...
Find the PR associated with the specified issue and address all review comments.
...
6. Reply to each **new** comment you are addressing on the PR. Do not re-reply to comments you already acted on, as determined in step 4. Use the correct endpoint for the comment type:
   ...
   - **PR conversation comments** (from `issues/PR_NUMBER/comments`): post a new comment:
     ...
     ```
7. If a comment is not actionable, reply explaining why.
...
while true; do
    ...
    if [[ "${has_work}" == "true" ]]; then
        ...
        timeout 1800 claude \
            ...
            -p "Address the review comments and fix any failing CI checks for ${TASK_DESCRIPTOR}. The PR is #${PR_NUM} on ${UPSTREAM_REPO}.
...
echo "=== Agentic Docs Review Responder Complete ==="
