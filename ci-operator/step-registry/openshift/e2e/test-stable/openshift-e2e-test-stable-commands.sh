...
openshift-tests run "${test_suite}" --dry-run | \
    grep -v "Kubectl Port forwarding With a server listening on localhost that expects a client request should support a client that connects, sends DATA" | \
    grep -v "The HAProxy router should expose prometheus metrics for a route" | \
    openshift-tests run --provider "${TEST_PROVIDER}" \
