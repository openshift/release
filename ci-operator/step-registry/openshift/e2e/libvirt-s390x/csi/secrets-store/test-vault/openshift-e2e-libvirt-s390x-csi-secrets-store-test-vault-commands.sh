...
check_arch_and_deps() {
    ...
    # Verify tools already present in the step image
    for tool in oc curl tar git python3; do
...
clone_repo() {
    ...
    local repo_branch="${BRANCH:-main}"
    if [[ "${repo_branch}" =~ ^[0-9]+\.[0-9]+$ ]]; then
        repo_branch="release-${repo_branch}"
    fi

    if [ -d "${REPO_DIR}" ]; then
        echo "Repository already exists at '${REPO_DIR}'. Using existing checkout."
    else
        echo "Cloning ${REPO_URL} (branch ${repo_branch})..."
        if ! git clone --depth 1 --branch "${repo_branch}" "${REPO_URL}" "${REPO_DIR}"; then
            echo "WARNING: Branch '${repo_branch}' not found, cloning default branch."
...
PYEOF
    ...
    echo
    echo "Raising timeout values below 300s to 300s (preserving longer values)..."
    python3 - "${BATS_FULL_PATH}" << 'PYEOF'
import re, sys
path = sys.argv[1]
with open(path) as f:
    content = f.read()
