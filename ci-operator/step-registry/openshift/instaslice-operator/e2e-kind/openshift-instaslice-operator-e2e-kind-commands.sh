...
gcloud compute ssh "$VM_NAME" --zone="$GOOGLE_COMPUTE_ZONE" --command='
  ...
  sudo apt install gcc make git ginkgo -y
...
#(TODO) Uncomment the code when the reboot issue is fixed
# Ref: https://forums.developer.nvidia.com/t/linux-wont-boot-after-mig-enabled/325969
#echo "Rebooting the VM: $VM_NAME"
#gcloud compute instances reset "$VM_NAME" --zone=$GOOGLE_COMPUTE_ZONE
# wait for the VM to be back in Running state
#echo "Waiting for VM to become RUNNING..."
...
echo "Cloning the instaslice operator"

gcloud compute ssh "$VM_NAME" --zone="$GOOGLE_COMPUTE_ZONE" --command='
set -eux

git clone https://github.com/openshift/instaslice-operator.git
