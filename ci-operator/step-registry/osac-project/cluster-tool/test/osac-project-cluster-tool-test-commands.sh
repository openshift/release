...
function collect_artifacts() {
    ...
    timeout -s 9 2m scp -F "${SHARED_DIR}/ssh_config" \
        "ci_machine:${REMOTE_RESULTS_DIR}/junit_e2e.xml" \
        "${ARTIFACT_DIR}/junit_e2e.xml" 2>/dev/null || true
}
trap collect_artifacts EXIT

echo "Creating test script on machine..."
timeout -s 9 1m ssh -F "${SHARED_DIR}/ssh_config" ci_machine bash -c 'cat > /root/test.sh' <<'REMOTE_SCRIPT'
set -euo pipefail
...
echo "Running e2e tests from ${TEST_DIR}..."
podman run --authfile /root/pull-secret --rm --network=host \
    ...
    -v /root/.config/cluster-tool:/root/.config/cluster-tool:z \
...
echo "Executing test script on machine..."
TEST_EXIT=0
timeout -s 9 60m ssh -F "${SHARED_DIR}/ssh_config" ci_machine \
    "bash /root/test.sh \
    '${E2E_NAMESPACE}' \
    '${E2E_VM_TEMPLATE}' \
