...
# format, then we will still get a new one created with this script
LAST_PR_NUM=$(
...
# if our last d/s merge PR is still not merged, then we don't need a new one yet
# But, we will need one if there was no "last pr" found at all (e.g., new branch)
if [[ -n "$LAST_PR_NUM" && "$MERGED" != "true" ]]; then
  echo "Last PR #${LAST_PR_NUM} not merged; exiting."
  exit 0
fi

# to save on overhead we don't need to open a new d/s merge PR until we have enough commits to bring in
# the env var MIN_COMMITS can be used to control that number
NEW_COMMITS=$(git rev-list origin/${DEFAULT_BRANCH}..upstream/${DEFAULT_BRANCH} --count)
echo "Found $NEW_COMMITS new commits upstream."
(( NEW_COMMITS < MIN_COMMITS )) && { echo "Not enough commits in upstream to make it worth opening a new PR; exiting."; exit 0; }

# if we made it this far, we can create the merge and push the PR
BRANCH="d/s-merge-$(date +%m-%d-%Y)"
git checkout -b "$BRANCH" origin/${DEFAULT_BRANCH}
# if there happens to be a merge conflict we can still push it and create a PR for the humans to take a look at
# and resolve. leave a /hold comment on it to make it obvious
...
if [[ "${CONFLICT:-false}" == "true" ]]; then
  ...
  curl -s -H "Authorization: token ${GITHUB_TOKEN}" -X PATCH -d "{\"title\":\"${NEW_TITLE}\"}" \
    "https://api.github.com/repos/${DOWNSTREAM_REPO}/pulls/${PR_NUM}"
  curl -s -H "Authorization: token ${GITHUB_TOKEN}" -X POST -d '{"body":"/hold\nneeds conflict resolution"}' \
