...
in sync with upstream test modifications and rules.go changes."
      ...
      if [[ $VALIDATION_EXIT -eq 1 ]]; then
        ...
        # Extract orphaned and new tests from validation output
...
if [[ "${CONFLICT:-false}" == "true" || "${GO_MOD_FAILED:-false}" == "true" || "${TEST_ANNOTATIONS_FAILED:-false}" == "true" || "${TEST_LIST_MISMATCH:-false}" == "true" || "${TEST_VALIDATION_FAILED:-false}" == "true" ]]; then
  ...
  [[ "${TEST_LIST_MISMATCH:-false}" == "true" ]] && FAILURES+=("TEST LIST MISMATCH") && STEPS+=("Update openshift/test/tests.go with orphaned/new tests")
  [[ "${TEST_VALIDATION_FAILED:-false}" == "true" ]] && FAILURES+=("TEST VALIDATION FAILED") && STEPS+=("Check validation script output for errors")
