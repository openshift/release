...
cleanup_stale_branches() {
  ...
  echo "🧹 Scanning ${DOWNSTREAM_REPO} for stale d/s-merge branches…"
  local current_branch="d/s-merge-$(date +%m-%d-%Y)"
  local cutoff owner url headers resp
  cutoff=$(date -d "-${STALE_BRANCH_AGE_DAYS} days" +%s)
  owner="${DOWNSTREAM_REPO%%/*}"
  url="https://api.github.com/repos/${DOWNSTREAM_REPO}/branches?per_page=100"
  ...
  while [[ -n "$url" ]]; do
    ...
    done < <(echo "$resp" | jq -r '.[].name' | grep -E '^d/s-merge-[0-9]{2}-[0-9]{2}-[0-9]{4}$')
  ...
  for branch in "${branches[@]}"; do
    ...
    pr_json=$(curl -sS -H "Authorization: token ${GITHUB_TOKEN}" \
      "https://api.github.com/repos/${DOWNSTREAM_REPO}/pulls?state=all&head=${owner}:${branch}&per_page=100")
    if [[ -z "$pr_json" ]]; then
    ...
    if curl -sS -f -H "Authorization: token ${GITHUB_TOKEN}" -X DELETE \
      "https://api.github.com/repos/${DOWNSTREAM_REPO}/git/refs/heads/${branch}" > /dev/null; then
