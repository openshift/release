...
OPENSHIT_VERSION=${OPENSHIT_VERSION:-}
...
# versions=$(rosa list versions --channel-group ${CHANNEL_GROUP})
versionList=$(rosa list versions --channel-group ${CHANNEL_GROUP} -o json | jq '.[].raw_id')
echo -e "Available cluster versions:\n${versionList}"
if [[ -z "$OPENSHIT_VERSION" ]]; then
  OPENSHIT_VERSION=$(echo "$versionList" | head -1 | tr -d '"')
elif [[ $OPENSHIT_VERSION =~ ^[0-9]+\.[0-9]+$ ]]; then
  OPENSHIT_VERSION=$(echo "$versionList" | grep ${OPENSHIT_VERSION} | tail -1 | tr -d '"')
else
  # Match the whole line
  OPENSHIT_VERSION=$(echo "$versionList" | grep -x ${OPENSHIT_VERSION} | tr -d '"')
fi
...
echo "  Openshift version: ${OPENSHIT_VERSION}"
...
mkdir -p "${SHARED_DIR}"
CLUSTER_ID_FILE="${SHARED_DIR}/rosa-sts-cluster-id"
CLUSTER_INFO="${ARTIFACT_DIR}/rosa-sts-cluster.txt"
CLUSTER_INSTALL_LOG="${ARTIFACT_DIR}/.rosa_sts_install.log"
...
if [ $? -ne 0 ]; then
  ...
  echo "Sleep 90 seconds to wait for the configuration to be enabled"
  sleep 90
fi
...
echo "${CONSOLE_URL}" > "${SHARED_DIR}/rosa-sts-console.url"
echo "oc login ${API_URL} -u rosa-admin -p ${IDP_PASSWD}" > "${SHARED_DIR}/rosa-sts-api.login"

oc login ${API_URL} -u rosa-admin -p ${IDP_PASSWD}
