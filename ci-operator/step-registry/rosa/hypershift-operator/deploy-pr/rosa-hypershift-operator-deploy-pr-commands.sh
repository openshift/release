...
# Wait for HO cache to prime before handing off to provision
echo "Waiting for HyperShift operator cache to prime..."
timeout 120 bash -c "until KUBECONFIG=\"\${MC_KUBECONFIG}\" oc get hostedclusters -A --no-headers >/dev/null 2>&1; do sleep 10; done" || true
