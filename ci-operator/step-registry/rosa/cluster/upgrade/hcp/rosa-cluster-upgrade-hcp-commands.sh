...
if [[ -n "${CLUSTER_ID}" ]]; then
  log "The upgraded cluster is ${CLUSTER_ID}"
...
if [[ -z "$cluster_id" ]]; then
  log "The CLUSTER_ID is mandatory!"
  exit 1
fi
...
function get_cluster_upgrade_path () {
  ...
  current_y_stream=$(echo "$current_version" | awk -F. '{print $2}')
  up_y_stream=$(echo "$upgrade_to_version" | awk -F. '{print $2}')
  count=`expr $up_y_stream - $current_y_stream`
  yStreamUpPath=()
  zStreamUpVersion=""

  raw_data=$(ocm get /api/clusters_mgmt/v1/versions/openshift-v${current_version}-${CHANNEL_GROUP} | jq -r .available_upgrades)
  clean_data=$(echo "$raw_data" | sed 's/[][",]//g')
  readarray -t versionsList <<< "$(echo "$clean_data" | sed '/^[[:space:]]*$/d' | awk '{$1=$1;print}')"
  prefix=$(echo "$current_version" | awk -F. '{print $1"."$2}')

  if [[ ! "$count" =~ 0 ]]; then
     #confirm if we can upgrade to Y+2/Y+3... version
     echo "Find Y stream upgrade path for v${current_version}-${CHANNEL_GROUP}"
     for ((i=1;i<=$count;i++)); do
      ...
      #  prefix=$(echo "$current_version" | awk -F. '{print $1"."$2}')
       ...
       yStreamUpPath[i]=$(echo "${result[0]}" | sed 's/ //g')
       current_version=${yStreamUpPath[i]}
     done
      log "Y Stream Upgrade path: (${yStreamUpPath[*]})"
  else
    log "Find Z stream upgrade path"
    IFS=$'\n' read -r -d '' -a sorted_versions < <(printf "%s\n" "${versionsList[@]}" | sort -Vr && printf '\0')
    for ver in "${sorted_versions[@]}"; do
         if [[ ! "$ver" == "$prefix"* ]]; then
            zStreamUpVersion="$ver"
            break
         fi
    done
    log "Z Stream Upgrade path: ${zStreamUpVersion}"
  fi
}
...
get_cluster_upgrade_path $current_version $UPGRADED_TO_VERSION

# For Y-Stream upgrade, will upgrade to the specified Y stream version
# if [[ ${#yStreamUpPath[@]} -gt 0 ]]; then
#   for version in "${yStreamUpPath[@]}"; do
#     upgrade_cluster_to $version
#     upgrade_machinepool_to $version
#   done
# fi

# For Z-Stream upgrade, will upgrade to the latest z version
# if [[ -n "$zStreamUpVersion" ]]; then
#   upgrade_cluster_to $zStreamUpVersion
#   upgrade_machinepool_to $version
# fi

# If there is no upgrade versions for cluster, will skip upgrade step
...
elif [[ -n "$zStreamUpVersion" ]]; then
   upgrade_cluster_to $zStreamUpVersion
   upgrade_machinepool_to $version
else
  log "There is available upgrade version for cluster $cluster_id" 
fi