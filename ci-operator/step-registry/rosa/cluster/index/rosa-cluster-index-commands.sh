...
if is_valid_json "${SHARED_DIR}/${METADATA_FILE}" && is_valid_es "${ES_SERVER}" "${ES_INDEX}" ; then
  cat "${SHARED_DIR}/${METADATA_FILE}" | jq 'to_entries | map(select(.key | contains("AWS") | not)) | from_entries | .timestamp |= (now | strftime("%Y-%m-%dT%H:%M:%SZ"))' > "${SHARED_DIR}/${METADATA_FILE}_filtered"
