...
CLUSTER_CLEAN_LOG="${ARTIFACT_DIR}/cluster_clean.log"
cms --ginkgo.v --ginkgo.no-color --ginkgo.timeout 1h --ginkgo.focus CleanClusterByProfile --ginkgo.label-filter feature-cleaner --ginkgo.junit-report post-clean.xml > "${CLUSTER_CLEAN_LOG}" || true
