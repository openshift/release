...
CLUSTER_PREPARE_LOG="${ARTIFACT_DIR}/cluster_creation.log"
cms --ginkgo.v --ginkgo.no-color --ginkgo.timeout 2h --ginkgo.focus CreateClusterByYAMLProfile --ginkgo.label-filter feature-cluster-creation --ginkgo.junit-report pre-cluster-creation.xml > "${CLUSTER_PREPARE_LOG}" || true

CREATE_INFO=$(cat "${CLUSTER_PREPARE_LOG}")
if [[ "${CREATE_INFO}" =~ "1 Failed" ]] ; then
  echo "${CREATE_INFO}"
  exit 1 
fi
