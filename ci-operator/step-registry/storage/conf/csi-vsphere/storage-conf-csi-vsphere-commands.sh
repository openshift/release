...
if [ -z "${TEST_OCP_CSI_DRIVER_MANIFEST:-}" ] && [ "${ENABLE_VSPHERE_FILESHARE_TESTS}" = "true" ]; then
    echo "Enabling vsphere fileshare tests"
    sed -i '/^[[:space:]]*Capabilities:/a\    RWX: true' "${SHARED_DIR}/${TEST_CSI_DRIVER_MANIFEST}"
fi
