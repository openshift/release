...
function collect_bootstrap_handler() {
  ...
  SSH_PRIV_KEY_PATH=\${CLUSTER_PROFILE_DIR}/ssh-private
  SSH_ARGS="-o ConnectTimeout=10 -o StrictHostKeyChecking=no -i \${SSH_PRIV_KEY_PATH}"
  SSH_CMD="ssh \${SSH_ARGS} core@\${BOOTSTRAP_IP}"

  #if ! command -v ssh &> /dev/null; then
  #  log "[\${handler_name}]: ERROR: ssh not installed, aborting log collection..."
  #  return
  #fi
  # TODO /FIXME: ssh is not available, do we need to validte this?
  #if ! \${SSH_CMD} uname -a; then
  #  log "[\${handler_name}]: ERROR: Bootstrap node is not reachable via \${BOOTSTRAP_IP}"
  #else
