...
#export OCI_CLI_CONFIG_FILE=${CUSTOM_PROFILE_DIR}/opct-oci-splat-user-config
function upi_conf_provider() {
  ...
  pip install oci-cli==3.33.4 > /dev/null
}
...
wget -qO ${WORKDIR}/yq "https://github.com/mikefarah/yq/releases/download/v4.34.1/yq_linux_amd64"
chmod u+x ${WORKDIR}/yq

echo_date "Installing butane"
wget -qO ${WORKDIR}/butane "https://github.com/coreos/butane/releases/download/v0.18.0/butane-x86_64-unknown-linux-gnu"
chmod u+x ${WORKDIR}/butane
...
export OCI_CMD="oci --config-file ${OCI_CONFIG} --debug "
export OCI_CMD_REG="oci --config-file ${OCI_CONFIG} --region ${REGION} --debug "
...
COMPARTMENT_ID_OPENSHIFT=$(${OCI_CMD} iam compartment create \
...
COMPARTMENT_ID_OPENSHIFT=$PARENT_COMPARTMENT_ID
...
TAG_NAMESPACE_ID=$(${OCI_CMD} iam tag-namespace create \
...
${OCI_CMD} iam dynamic-group create \
...
${OCI_CMD} iam policy create --name $POLICY_NAME \
    --compartment-id $COMPARTMENT_ID_OPENSHIFT \
...
NGW_ID=$(${OCI_CMD} network nat-gateway create \
  --compartment-id ${COMPARTMENT_ID_OPENSHIFT} \
...
RTB_PUB_ID=$(${OCI_CMD} network route-table create \
  --compartment-id ${COMPARTMENT_ID_OPENSHIFT} \
  --vcn-id $VCN_ID \
...
# Route Table: Private
RTB_PVT_ID=$(${OCI_CMD} network route-table create \
  --compartment-id ${COMPARTMENT_ID_OPENSHIFT} \
  --vcn-id $VCN_ID \
...
SUBNET_ID_PUBLIC=$(${OCI_CMD} network subnet create \
  --compartment-id ${COMPARTMENT_ID_OPENSHIFT} \
  --vcn-id $VCN_ID \
...
SUBNET_ID_PRIVATE=$(${OCI_CMD} network subnet create \
  --compartment-id ${COMPARTMENT_ID_OPENSHIFT} \
  --vcn-id $VCN_ID \
