...
This Step will do the following:
 ...
 2. For each entry where `postTaskStep` is `trigger-jobs-post-interop-ocp-watcher-bot-send-message`, evaluate the execution flags.
 3. Check if `(postTaskFlgs && JT__POST_TASK_EXEC_FLGS)` is evaluated to non-0. If not, skip this post task.
 4. Check post task execution marker to determine whether this post task is supposed to be executed.
