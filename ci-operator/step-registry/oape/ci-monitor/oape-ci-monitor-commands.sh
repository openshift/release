...
if [[ ! -f "${OAPE_ROOT}/scripts/ci-monitor/monitor.sh" ]]; then
  ...
  OAPE_AI_E2E_BRANCH="${OAPE_AI_E2E_BRANCH:-CI-Monitor}"
  ...
  # Shallow clone CI-Monitor branch (pinned commit is branch tip); full clone fallback.
  if ! git clone --depth 1 -b "${OAPE_AI_E2E_BRANCH}" "${OAPE_AI_E2E_REPO}" "${OAPE_CLONE_DIR}"; then
    git clone "${OAPE_AI_E2E_REPO}" "${OAPE_CLONE_DIR}"
    git -C "${OAPE_CLONE_DIR}" checkout "${OAPE_AI_E2E_COMMIT}"
  elif [[ "$(git -C "${OAPE_CLONE_DIR}" rev-parse HEAD)" != "${OAPE_AI_E2E_COMMIT}" ]]; then
    git -C "${OAPE_CLONE_DIR}" fetch --depth 1 origin "${OAPE_AI_E2E_COMMIT}"
    git -C "${OAPE_CLONE_DIR}" checkout "${OAPE_AI_E2E_COMMIT}"
  fi
