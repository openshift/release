...
if [[ -z "${PROJECT_NUMBER}" ]]; then echo "ERROR: .project_number missing in ${WIF_CONFIG}"; exit 1; fi
if [[ -z "${POOL_ID}" ]]; then echo "ERROR: .pool_id missing in ${WIF_CONFIG}"; exit 1; fi
if [[ -z "${SERVICE_ACCOUNT}" ]]; then echo "ERROR: .service_account missing in ${WIF_CONFIG}"; exit 1; fi
...
echo "WIF credential config written to ${SHARED_DIR}/wif-cred.json"
