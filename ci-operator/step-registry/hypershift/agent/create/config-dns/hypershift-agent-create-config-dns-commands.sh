...
echo "address=/.apps.\$HOSTEDCLUSTER_NAME.ostest.test.metalkube.org/192.168.111.30" >> /etc/NetworkManager/dnsmasq.d/openshift-ostest.conf
systemctl restart NetworkManager.service

virsh net-dumpxml ostestbm > /tmp/ostestbm.xml
sed -i 's/<dns>/<dns>\n    <forwarder domain='"'"'ostest.test.metalkube.org'"'"' addr='"'"'127.0.0.1'"'"'\/>/' /tmp/ostestbm.xml
