...
if [ -d "${METALLB_SRC_DIR}" ]; then
  ...
  if [[ -n "${METALLB_CONFIG:-}" ]]; then
    readarray -t config <<< "${METALLB_CONFIG}"
...
ssh "${SSHOPTS[@]}" "root@${IP}" "cd /root/dev-scripts/metallb/openshift-ci/ && export KUBECONFIG=${HOME}/nested_kubeconfig && bash deploy_metallb.sh"
