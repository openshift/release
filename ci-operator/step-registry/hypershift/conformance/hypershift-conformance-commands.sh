...
function patch_image_streams() {
  ...
  if [ "$OCP_MINOR_VERSION" -le "15" ]; then
     ...
     oc patch image.config.openshift.io/cluster --patch '{"spec":{"additionalTrustedCA":{"name":"registry-config"}}}' --type=merge
      ...
      # workaround for 3 kube-apiserver tls certificate cases
     oc --kubeconfig="${HYPERSHIFT_MANAGEMENT_CLUSTER_KUBECONFIG}" delete cm registry-config -n openshift-config
