...
# Use custom AWS credentials if provided, otherwise fall back to cluster profile
if [[ -n "${STANDALONE_AWS_CREDS_FILE:-}" && -r "${STANDALONE_AWS_CREDS_FILE}" ]]; then
  export AWS_SHARED_CREDENTIALS_FILE="${STANDALONE_AWS_CREDS_FILE}"
  echo "Using custom AWS credentials from: ${STANDALONE_AWS_CREDS_FILE}"
else
  export AWS_SHARED_CREDENTIALS_FILE="${CLUSTER_PROFILE_DIR}/.awscred"
  echo "Using cluster profile AWS credentials"
fi
...
# Use fixed region instead of LEASED_RESOURCE
REGION="${FIXED_AWS_REGION:-us-east-1}"
aws_source_region="${REGION}"

echo "Using fixed AWS region: ${REGION}"

# Parse fixed zones from comma-separated list
IFS=',' read -ra ZONES <<< "${FIXED_AWS_ZONES:-us-east-1a,us-east-1b,us-east-1c}"
ZONES_COUNT=${#ZONES[@]}
MAX_ZONES_COUNT=${ZONES_COUNT}

echo "Using fixed zones: ${ZONES[*]}"
echo "Zones count: ${ZONES_COUNT}"
...
# Apply fixed zones to install-config.yaml
