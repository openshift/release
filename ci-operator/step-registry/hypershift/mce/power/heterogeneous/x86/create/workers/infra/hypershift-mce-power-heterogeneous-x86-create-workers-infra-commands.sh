...
VPC_VSI_NAME="${HOSTED_CLUSTER_NAME}-worker-x"
...
for instance in "${INSTANCE_NAMES[@]}"; do
  instance_ip=$(ibmcloud is instance-create $instance "${VPC_NAME}" "${ZONE}" ${PROFILE_NAME} "${VPC_SUBNET_ID}" --image "${IMAGE_ID}" --key-ids "${SSH_KEY_ID}" --output JSON | jq -r '.primary_network_attachment.primary_ip.address')
  sleep 2
