...
boot_power_workers() {
    # Power VM details
    INSTANCE_NAMES=$1
    MAC_ADDRESSES=$(cat "${SHARED_DIR}/macpoweraddr")
    IP_ADDRESSES=$(cat "${SHARED_DIR}/ippoweraddr")
    INSTANCE_ID=$(cat "${SHARED_DIR}/ippowerids")
...
add_power_workers() {
    INSTANCE_NAMES=$(cat "${SHARED_DIR}/ippowernames")
    boot_power_workers "${INSTANCE_NAMES}"
    approve_agents "${INSTANCE_NAMES}"
    set_proxy
