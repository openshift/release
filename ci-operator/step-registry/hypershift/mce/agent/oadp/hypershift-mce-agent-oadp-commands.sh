...
#oc patch backupstoragelocation dpa-sample-1 -n openshift-adp --type merge --patch '{"spec":{"accessMode":"ReadOnly"}}'
...
EOF
while true; do
    if oc get statefulset etcd -n "local-cluster-${CLUSTER_NAME}" > /dev/null 2>&1; then
        DESIRED_REPLICAS=$(oc get statefulset etcd -n "local-cluster-${CLUSTER_NAME}" -o jsonpath='{.spec.replicas}')
        CURRENT_REPLICAS=$(oc get statefulset etcd -n "local-cluster-${CLUSTER_NAME}" -o jsonpath='{.status.readyReplicas}')

        if [ "$DESIRED_REPLICAS" == "$CURRENT_REPLICAS" ]; then
            echo "StatefulSet etcd is fully available in namespace $NAMESPACE"
            oc patch hostedcluster -n local-cluster ${CLUSTER_NAME}  --type json -p '[{"op": "add", "path": "/spec/pausedUntil", "value": "false"}]'
            oc patch nodepool -n local-cluster ${CLUSTER_NAME}  --type json -p '[{"op": "add", "path": "/spec/pausedUntil", "value": "false"}]'
            exit 0
        fi
...
###
#oc wait --timeout=45m --for=jsonpath='{.status.phase}'=Completed restore/hc-clusters-hosted-restore -n openshift-adp || true
