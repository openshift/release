...
if [[ "$DISCONNECTED" == "true" ]]; then
  _REPO=$(head -n 1 "${SHARED_DIR}/mirror_registry_url" | sed 's/5000/6003/g')/acm-d/mce-custom-registry
  ...
  oc apply -f - <<EOF
    ...
    spec:
      ...
      - mirrors:
        - $(head -n 1 "${SHARED_DIR}/mirror_registry_url" | sed 's/5000/6003/g')/acm-d
