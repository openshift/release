...
# Use hosted HyperShift kubevirt cluster kubeconfig like hypershift-conformance.
...
# Detect platform from the hosted cluster infrastructure API (kubevirt for HCP virt).
CLUSTER_TYPE="${CLUSTER_TYPE:-}"
if [[ -f "${SHARED_DIR}/nested_kubeconfig" ]]; then
    INFRA_PLATFORM="$(oc get infrastructure cluster -ojsonpath='{.status.platform}' 2>/dev/null | tr '[:upper:]' '[:lower:]' || true)"
    if [[ -n "${INFRA_PLATFORM}" ]]; then
        CLUSTER_TYPE="${INFRA_PLATFORM}"
        export CLUSTER_TYPE
    fi
fi
if [[ -z "${CLUSTER_TYPE}" ]]; then
    echo >&2 "unable to detect CLUSTER_TYPE from hosted cluster infrastructure"
    exit 1
fi
