...
function save_credentials () {
  cluster_libvirt_save_credentials "${INSTALL_DIR}"
}

function prepare_next_steps () {
  EXIT_CODE=$?
  echo "${EXIT_CODE}" > "${CLUSTER_DIR}/install-status.txt"
  if [[ ${EXIT_CODE} != 0 ]]; then
    exit ${EXIT_CODE}
  fi
  set +e
  echo "Setup phase finished for ${CLUSTER_ROLE} cluster, prepare env for next steps"
...
echo "Installing ${CLUSTER_ROLE} OpenShift cluster (${CLUSTER_NAME}) on libvirt..."
...
cp "${CLUSTER_DIR}/install-config.yaml" "${INSTALL_DIR}"
...
NODE_TUNING_YAML="${CLUSTER_DIR}/99-sysctl-worker.yaml"
...
CHRONY_WORKER_YAML="${CLUSTER_DIR}/99-chrony-worker.yaml"
...
CHRONY_MASTER_YAML="${CLUSTER_DIR}/99-chrony-master.yaml"
...
date "+%F %X" > "${CLUSTER_DIR}/CLUSTER_INSTALL_START_TIME"
...
date "+%F %X" > "${CLUSTER_DIR}/CLUSTER_INSTALL_END_TIME"
