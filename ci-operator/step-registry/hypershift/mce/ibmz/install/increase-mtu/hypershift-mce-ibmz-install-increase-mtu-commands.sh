...
# --- Wait for MachineConfigPool worker to finish rolling reboot ---
# MCO reboots nodes one at a time; this can take 20-30 min per pool.
echo "$(date) Waiting for MachineConfigPool worker to finish applying MTU config..."
MCP_INTERVAL=60
MCP_ELAPSED=0

while [[ ${MCP_ELAPSED} -lt ${MCP_WORKER_WAIT_TIMEOUT} ]]; do
...
if [[ "${MCP_ELAPSED}" -ge "${MCP_WORKER_WAIT_TIMEOUT}" ]]; then
  echo "$(date) ERROR: MCP worker did not finish within ${MCP_WORKER_WAIT_TIMEOUT}s"
  oc get mcp || true
  oc get nodes || true
  exit 1
fi

# --- Wait for MachineConfigPool master to finish rolling reboot ---
# Master MCP has 3 nodes and reboots sequentially — allow 1.5 hours.
echo "$(date) Waiting for MachineConfigPool master to finish applying MTU config..."
MCP_ELAPSED=0
while [[ ${MCP_ELAPSED} -lt ${MCP_MASTER_WAIT_TIMEOUT} ]]; do
...
if [[ "${MCP_ELAPSED}" -ge "${MCP_MASTER_WAIT_TIMEOUT}" ]]; then
  echo "$(date) ERROR: MCP master did not finish within ${MCP_MASTER_WAIT_TIMEOUT}s"
  oc get mcp || true
  oc get nodes || true
  exit 1
fi
