...
if [[ "${HYPERSHIFT_EXTERNAL_DNS_DOMAIN:-}" != "" ]]; then
  EXTERNAL_DNS_ARGS="--e2e.external-dns-domain=${HYPERSHIFT_EXTERNAL_DNS_DOMAIN} --e2e.external-dns-provider=azure --e2e.external-dns-credentials=/etc/hypershift-aks-e2e-dns-credentials/credentials.json"
fi
