...
if [[ -f "${SHARED_DIR}/mgmt_kubeconfig" ]]; then
  ...
  echo "No kubeconfig found, cannot query HostedClusters"
  exit 0
fi

# Get all infraIDs from HostedClusters on the management cluster
# This captures all clusters created in THIS test run, whether it's e2e (multiple clusters) or conformance (single cluster)
...
if [[ -z "${INFRA_IDS}" ]]; then
  echo "No HostedClusters found on management cluster, nothing to clean up"
  exit 0
fi
