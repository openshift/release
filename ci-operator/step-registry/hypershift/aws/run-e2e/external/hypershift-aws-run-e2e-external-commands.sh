...
if [[ "${OAUTH_EXTERNAL_OIDC_PROVIDER}" != "" ]]; then
  ...
  OAUTH_EXTERNAL_OIDC_PARAM="--e2e.external-oidc-provider=${OAUTH_EXTERNAL_OIDC_PROVIDER} --e2e.external-oidc-cli-client-id=${KEYCLOAK_CLI_CLIENT_ID} --e2e.external-oidc-console-client-id=${CONSOLE_CLIENT_ID} --e2e.external-oidc-issuer-url=${KEYCLOAK_ISSUER} --e2e.external-oidc-console-secret=${CONSOLE_CLIENT_SECRET_VALUE} --e2e.external-oidc-ca-bundle-file=${KEYCLOAK_CA_BUNDLE_FILE}  "
fi
