...
# Wait for manual debugging if ENABLE_DEBUG_WAIT is set to "true"
if [[ "${ENABLE_DEBUG_WAIT:-false}" == "true" ]]; then
  echo "======================================================================"
  echo "DEBUG WAIT MODE ENABLED"
  echo "======================================================================"
  echo "Test execution is paused for manual debugging."
  echo ""
  ...
  echo ""
  echo "To manually execute the e2e tests, run:"
  echo "======================================================================"
  echo "hack/ci-test-e2e.sh -test.v \\"
  echo "  -test.run=${CI_TESTS_RUN:-''} \\"
  echo "  -test.parallel=20 \\"
  echo "  --e2e.aws-credentials-file=/etc/hypershift-pool-aws-credentials/credentials \\"
  echo "  --e2e.aws-zones=us-east-1a,us-east-1b,us-east-1c \\"
  echo "  ${AWS_OBJECT_PARAMS:-} \\"
  echo "  --e2e.pull-secret-file=/etc/ci-pull-credentials/.dockerconfigjson \\"
  echo "  --e2e.base-domain=ci.hypershift.devcluster.openshift.com \\"
  echo "  --e2e.latest-release-image=${OCP_IMAGE_LATEST} \\"
  echo "  --e2e.previous-release-image=${OCP_IMAGE_PREVIOUS} \\"
  echo "  ${PKI_RECONCILIATION_PARAMS:-} \\"
  echo "  ${N1_NP_VERSION_TEST_ARGS:-} \\"
  echo "  ${N2_NP_VERSION_TEST_ARGS:-} \\"
  echo "  ${N3_NP_VERSION_TEST_ARGS:-} \\"
  echo "  ${N4_NP_VERSION_TEST_ARGS:-} \\"
  echo "  --e2e.additional-tags=\"expirationDate=$(date -d '4 hours' --iso=minutes --utc)\" \\"
  echo "  --e2e.aws-endpoint-access=PublicAndPrivate \\"
  echo "  --e2e.external-dns-domain=service.ci.hypershift.devcluster.openshift.com \\"
  echo "  ${AWS_MULTI_ARCH_PARAMS:-} \\"
  echo "  ${REQUEST_SERVING_COMPONENT_PARAMS:-} \\"
  echo "  ${OAUTH_EXTERNAL_OIDC_PARAM:-} \\"
  echo "  ${RUN_UPGRADE_PARAM}"
  echo "======================================================================"
  echo ""
  echo "To run a specific test (e.g., TestNodePoolImageType):"
  echo "======================================================================"
  echo "hack/ci-test-e2e.sh -test.v \\"
  echo "  -test.run='TestNodePoolImageType' \\"
  ...
  echo "  1. Find this pod: kubectl get pods -A | grep ${HOSTNAME}"
  echo "  2. Exec into pod: kubectl exec -it ${HOSTNAME} -n <namespace> -- /bin/bash"
  echo "  3. Set kubeconfig: export KUBECONFIG=${KUBECONFIG}"
  echo ""
  echo "To resume automatic test execution:"
  echo "  - Create file: touch ${SHARED_DIR}/continue-tests"
  echo ""
  echo "Waiting indefinitely... (job will timeout after grace period)"
  echo "======================================================================"

  while true; do
    if [[ -f "${SHARED_DIR}/continue-tests" ]]; then
      echo "Continue signal detected. Resuming automatic test execution..."
      rm -f "${SHARED_DIR}/continue-tests"
      break
    fi
    sleep 60
  done

  echo "Debug wait completed. Starting automatic test execution..."
fi
