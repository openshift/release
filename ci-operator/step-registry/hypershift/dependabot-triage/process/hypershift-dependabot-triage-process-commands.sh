...
if [ -n "$PR_NUM" ] && [ -n "${BUILD_ID:-}" ] && [ -n "${JOB_NAME:-}" ]; then
  ...
  if [ "${JOB_TYPE:-}" = "periodic" ]; then
    REPORT_URL="https://gcs.ci.openshift.org/gcs/test-platform-results-public/logs/${JOB_NAME}/${BUILD_ID}/artifacts/dependabot-triage/hypershift-dependabot-triage-report/artifacts/dependabot-triage-report.html"
  else
    REPORT_URL="https://gcs.ci.openshift.org/gcs/test-platform-results-public/pr-logs/pull/openshift_release/${PULL_NUMBER:-0}/${JOB_NAME}/${BUILD_ID}/artifacts/dependabot-triage/hypershift-dependabot-triage-report/artifacts/dependabot-triage-report.html"
  fi
