...
grep '^{' "$CLAUDE_OUTPUT_FILE" > "/tmp/claude-dependabot-filtered.json" 2>/dev/null || true
...
}' "/tmp/claude-dependabot-filtered.json" > "${SHARED_DIR}/claude-dependabot-tokens.json" 2>/dev/null || \
  echo '{"input_tokens":0,"output_tokens":0,"cache_read_input_tokens":0,"cache_creation_input_tokens":0,"model":"unknown"}' > "${SHARED_DIR}/claude-dependabot-tokens.json"
echo "Token usage: $(cat "${SHARED_DIR}/claude-dependabot-tokens.json")"

# Extract Claude text output and tool usage summaries
jq -j 'select(.type == "assistant") | .message.content[]? | select(.type == "text") | .text // empty' "$CLAUDE_OUTPUT_FILE" > "${SHARED_DIR}/claude-dependabot-output-text.txt" 2>/dev/null || true
