...
if [[ ! -f "${SHARED_DIR}/nested_kubeconfig" ]]; then
  echo "error: ${SHARED_DIR}/nested_kubeconfig not found (HyperShift hosted cluster required)" >&2
  exit 1
fi

# Fetch packet basic configuration
source "${SHARED_DIR}/packet-conf.sh"
GUEST_KUBECONFIG_REMOTE="/root/nested_kubeconfig"
scp "${SSHOPTS[@]}" "${SHARED_DIR}/nested_kubeconfig" "root@${IP}:${GUEST_KUBECONFIG_REMOTE}"

ssh "${SSHOPTS[@]}" "root@${IP}" "FRR_IMAGE='$FRR_IMAGE' ADVERTISE_DEFAULT_NETWORK='${ADVERTISE_DEFAULT_NETWORK:-true}' GUEST_KUBECONFIG='${GUEST_KUBECONFIG_REMOTE}'" bash -x - << 'EOFTOP'
#!/bin/bash
set -o nounset
set -o errexit
set -o pipefail
...
echo "[$(date +'%Y-%m-%d %H:%M:%S')] Starting hypershift-kubevirt-baremetalds-ovn-bgp-pre setup (guest cluster)"
...
KCLI="oc --kubeconfig=${GUEST_KUBECONFIG}"
if ! command -v oc; then
    echo "error: oc CLI required on packet host for guest BGP pre" >&2
    exit 1
fi
...
# Resolve pod CIDRs from the guest cluster (oc uses nested guest kubeconfig).
if [ "$ADVERTISE_DEFAULT_NETWORK" = "true" ]; then
  cluster_cidrs=$(oc get network.config.openshift.io cluster -o jsonpath='{.spec.clusterNetwork[*].cidr}')
  CLUSTER_NETWORK_V4=$(awk '{for (i = 1; i <= NF; i++) if ($i !~ /:/) { print $i; exit }}' <<<"$cluster_cidrs")
  CLUSTER_NETWORK_V6=$(awk '{for (i = 1; i <= NF; i++) if ($i ~ /:/) { print $i; exit }}' <<<"$cluster_cidrs")

  if [ -n "$CLUSTER_NETWORK_V4" ]; then
