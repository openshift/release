...
wait_nodepools_updating() {
  ...
  while [[ $counter -lt 1500 ]]; do
    ...
    for np in $nodepools; do
      ...
      if [[ "$updating_status" != "False" && "$updating_status" != "Unknown" ]]; then
        all_ready=false
    ...
    log "Summary: $ready_count ready, $updating_count updating out of $(echo $nodepools | wc -w) total nodepools"
    ...
    log "Still waiting for nodepools to finish updating... ($counter/1500s elapsed, next check in 20s)"
  done

  log "Timeout waiting for nodepools to finish updating after 1500 seconds"
...
check_idms_on_guest() {
  ...
  local idms_count
  idms_count=$(oc --kubeconfig="$guest_kubeconfig" get imagedigestmirrorsets --no-headers 2>/dev/null | wc -l || echo "0")

  if [[ "$idms_count" -gt 0 ]]; then
    log "Found $idms_count IDMS resources on guest cluster"
    oc --kubeconfig="$guest_kubeconfig" get imagedigestmirrorsets
    return 0
  else
...
check_registries_conf() {
  ...
  registries_content=$(oc --kubeconfig="$guest_kubeconfig" debug "node/$node_name" -- cat /host/etc/containers/registries.conf 2>/dev/null)

  if [[ $? -ne 0 ]]; then
    log "Failed to read registries.conf from node $node_name"
    return 1
  fi
...
delete_guest_nodes() {
  ...
  log "Deleting all worker nodes in guest cluster to force recreation"
...
verify_idms_propagation() {
  ...
  # If we get here, all retries failed - delete nodes to force recreation
  ...
  hc_namespace=$(get_hosted_cluster_namespace "$hc_name") || {
    log "failed to get hosted cluster namespace for final verification"
    return 1
  }

  if delete_guest_nodes "$hc_name" "$hc_namespace"; then
    log "Nodes recreated, performing final verification"
    sleep 60  # Wait a bit for IDMS to propagate
    wait_nodepools_updating "$hc_name" "$hc_namespace" || {
      log "failed to wait the nodepools updating"
      return 1
    }

    # Retry to check the registries.conf on the node
    retry=0
    while [[ $retry -lt $max_retries ]]; do
      retry=$((retry + 1))
      log "Verification attempt $retry/$max_retries"
...
main() {
  ...
  wait_nodepools_updating "$hc_name" "$hc_namespace" || {
    echo "failed to wait the nodepools updating"
    return 1
  }
