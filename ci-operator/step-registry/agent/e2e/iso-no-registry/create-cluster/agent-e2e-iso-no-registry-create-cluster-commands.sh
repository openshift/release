...
timeout -s 9 180m ssh "${SSHOPTS[@]}" "root@${IP}" bash - << 'EOF' |& sed -e '/auths/ s/.*/*** PULL_SECRET ***/'
set -xeuo pipefail

cd /root/dev-scripts

set +e
timeout -s 9 180m make agent_create_cluster
rv=$?
...
ssh "${SSHOPTS[@]}" "root@${IP}" bash - << 'EOF' |& sed -e 's/.*auths.*/*** PULL_SECRET ***/g'
console_host="$(oc -n openshift-console get routes console -o=jsonpath='{.spec.host}')"
test -n "${console_host}"
echo "https://${console_host}" > /tmp/console.url
EOF
