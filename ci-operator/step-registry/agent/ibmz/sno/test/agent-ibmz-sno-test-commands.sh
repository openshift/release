...
ssh "${ssh_options[@]}" root@$bastion_vsi_ip "oc wait no --all --for=condition=Ready=true --timeout=30m --kubeconfig /root/$CLUSTER_NAME/auth/kubeconfig"
echo "$(date) SNO cluster is ready"

# Verifying the SNO cluster operators status
echo "$(date) Verifying the cluster operators status"
ssh "${ssh_options[@]}" root@$bastion_vsi_ip "oc wait --all=true co --for=condition=Available=True --timeout=30m --kubeconfig /root/$CLUSTER_NAME/auth/kubeconfig"
