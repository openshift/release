...
upload_bmc_cert() {
  ...
  # Debug output
  echo "    [DEBUG] BMC IP: ${BMC_IP}"
  echo "    [DEBUG] User: ${USER}"
  echo "    [DEBUG] Cert file: ${CERT_FILE}"

  # Test Redfish endpoint accessibility
  echo "    [DEBUG] Testing Redfish endpoint..."
  TEST_RESPONSE=$(curl -sk --connect-timeout 5 -u "${USER}:${PASS}" \
    "https://${BMC_IP}/redfish/v1" 2>&1)
  if echo "$TEST_RESPONSE" | grep -q "ServiceRoot"; then
    echo "    [DEBUG] Redfish endpoint accessible"
  else
    echo "    [WARNING] Redfish endpoint test failed: ${TEST_RESPONSE:0:100}"
  fi
  ...
  # Certificate management always uses https:// and doesn't need redfish_base_uri
  ...
  echo "    [DEBUG] Uploading to HPE iLO endpoint: ${CERT_UPLOAD_URL}"
  ...
  if [[ "$HTTP_CODE" == "000" ]]; then
    ...
    echo "$RESPONSE" | grep -E "(curl:|connect|SSL|TLS|error|failed)" | head -10 || echo "    (no specific error details found)"
  fi

  if [[ "$HTTP_CODE" == "200" ]] || [[ "$HTTP_CODE" == "202" ]] || [[ "$HTTP_CODE" == "204" ]]; then
    echo "    ✓ Certificate uploaded to ${BMC_NAME}"
    echo "    [DEBUG] Function returning 0"
    return 0
  else
    echo "    ✗ Failed to upload certificate to ${BMC_NAME} (HTTP ${HTTP_CODE})"
    echo "    [DEBUG] Function returning 1"
    return 1
  fi
}
...
# Upload to each BMC
for bmhost in $(yq e -o=j -I=0 '.[]' "$SHARED_DIR/hosts.yaml"); do
  echo "[DEBUG] Processing next host from hosts.yaml..."
  # shellcheck disable=SC1090
  . <(echo "$bmhost" | yq e 'to_entries | .[] | (.key + "=\"" + .value + "\"")')
  echo "[DEBUG] Parsed host: name=${name}, vendor=${vendor:-unknown}"
  ...
  echo "  [DEBUG] Sleeping 2 seconds before next host..."
  sleep 2
  echo "  [DEBUG] Continuing to next host..."
done

echo ""
echo "[DEBUG] Upload loop completed"
echo "  Upload results: ${SUCCESS_COUNT} successful, ${FAIL_COUNT} failed"
echo ""

# Step 4: Wait for BMCs to restart and verify
if [[ $SUCCESS_COUNT -gt 0 ]]; then
  echo "[4/4] Waiting for BMC controllers to restart (120 seconds)..."
  sleep 120

  echo ""
  echo "Verifying certificate installation..."
  echo "  ⚠ Skipping verification (not supported through proxy)"
  echo ""
fi
...
if [[ $SUCCESS_COUNT -eq 0 ]] || [[ $FAIL_COUNT -gt 0 ]]; then
  ...
  [[ $SUCCESS_COUNT -eq 0 ]] && echo "  No HP/HPE servers found or all uploads failed."
  exit 1
fi
