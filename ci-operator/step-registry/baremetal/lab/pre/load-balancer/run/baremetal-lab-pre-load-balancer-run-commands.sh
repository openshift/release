...
timeout -s 9 21m ssh "${SSHOPTS[@]}" "root@${AUX_HOST}" bash -s -- \
  "${CLUSTER_NAME}" "${DISCONNECTED}" "'${HAPROXY}'" "'${DHCLIENT}'" \
  "${ipv4_enabled:-false}" "${ipv6_enabled:-false}" "${HOST_ID}" << 'EOF'
set -o nounset
set -o errexit
set -o pipefail
...
ipv6_enabled="${6}"
HOST_ID="${7}"
PADDED_HOST=$(printf "%04d" "${HOST_ID}")
...
INTERNAL_API_IPV4="192.168.81.${HOST_ID}/22"
INTERNAL_API_IPV6="fd99:2222:3456:0000:0000:0000:0001:${PADDED_HOST}/64"

INTERNAL_INGRESS_IPV4="192.168.82.${HOST_ID}/22"
INTERNAL_INGRESS_IPV6="fd99:2222:3456:0000:0000:0000:0002:${PADDED_HOST}/64"

CONTAINER_NAME="haproxy-$CLUSTER_NAME"

MAC_ADDRESS_EXT=$(echo -n "${CLUSTER_NAME}-ETH1" | md5sum | cut -c1-10 | sed 's/\(..\)/\1:/g; s/^/02:/; s/:$//')
MAC_ADDRESS_INT=$(echo -n "${CLUSTER_NAME}-ETH2" | md5sum | cut -c1-10 | sed 's/\(..\)/\1:/g; s/^/02:/; s/:$//')
...
if [ "${ipv4_enabled}" == "true" ]; then
  nsenter -t "$CONTAINER_PID" -n /sbin/ip addr add "$INTERNAL_API_IPV4" dev eth2
  if [ "${DISCONNECTED}" == "true" ]; then
    echo "Disconnected environment: Adding Ingress IP address to eth2 as alias..."
    nsenter -t "$CONTAINER_PID" -n /sbin/ip addr add "$INTERNAL_INGRESS_IPV4" dev eth2
  fi
fi

if [ "${ipv6_enabled}" == "true" ]; then
  nsenter -t "$CONTAINER_PID" -n /sbin/ip addr add "$INTERNAL_API_IPV6" dev eth2
  if [ "${DISCONNECTED}" == "true" ]; then
    echo "Disconnected environment: Adding Ingress IPv6 address to eth2 as alias..."
    nsenter -t "$CONTAINER_PID" -n /sbin/ip addr add "$INTERNAL_INGRESS_IPV6" dev eth2
  fi
fi
...
if [ "${ipv6_enabled}" == "true" ]; then
  if [ "${DISCONNECTED}" != "true" ]; then
    ...
    for j in {1..20}; do
      if nsenter -t "$CONTAINER_PID" -n /sbin/ip -o -6 a list eth1 | grep -q 'inet6 fe80:' && \
         ! nsenter -t "$CONTAINER_PID" -n /sbin/ip -o -6 a list eth1 | grep -q 'tentative'; then
    ...
    echo "Launching isolated IPv6 DHCP client exclusively for eth1..."
    nsenter -m -u -n -i -p -t "$CONTAINER_PID" /sbin/dhclient -6 -nw -v \
      -pf "/etc/haproxy/dhclient.v6.pid" \
      -lf "/etc/haproxy/dhclient.v6.lease" eth1 201>&-

    echo "Waiting for interfaces to obtain IPv6 global addresses inside the container namespace..."
    for i in {1..60}; do
      if nsenter -t "$CONTAINER_PID" -n /sbin/ip -o -6 a list "${api_ip_interface}" | grep global | grep -q 'inet6 '; then
        echo "IPv6 global network layer state successfully verified."
        break
      fi

      if [ "$i" -eq 60 ]; then
        echo "Timed out waiting for DHCP IPv6 assignment on eth1. Exiting."
        exit 1
      fi
      sleep 0.5
    done
    ...
    nsenter -t "$CONTAINER_PID" -n /sbin/sysctl -w net.ipv6.conf.eth1.disable_ipv6=1
    echo "Disconnected environment active. Skipping external IPv6 DHCP."
  fi
else
  echo "IPv6 is disabled. Skipping IPv6 lease request."
fi
...
API_IPv4_CLEAN="${INTERNAL_API_IPV4%%/*}"
API_IPv6_CLEAN="${INTERNAL_API_IPV6%%/*}"
INGRESS_IPv4_CLEAN="${INTERNAL_INGRESS_IPV4%%/*}"
INGRESS_IPv6_CLEAN="${INTERNAL_INGRESS_IPV6%%/*}"

if [ "${ipv4_enabled:-false}" == "true" ]; then
  if [ "${DISCONNECTED}" == "true" ]; then
    api_ip="$API_IPv4_CLEAN"
    api_int_ip="$INGRESS_IPv4_CLEAN"
  else
    api_ip=$(nsenter -t "$CONTAINER_PID" -n /sbin/ip -o -4 a list ${api_ip_interface} | sed 's/.*inet \(.*\)\/[0-9]* brd.*$/\1/')
    api_int_ip="$API_IPv4_CLEAN"
  fi
...
printf "ingress_vip: %s\napi_vip: %s\ningress_vip_v6: %s\napi_vip_v6: %s\napi_int: %s\napi_int_v6: %s" "$api_int_ip" "$api_ip" "$api_int_ip_v6" "$api_ip_v6" "$api_ip" "$api_ip_v6" > "$BUILD_DIR/external_vips.yaml"
EOF
