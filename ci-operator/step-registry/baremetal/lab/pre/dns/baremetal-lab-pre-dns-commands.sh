...
for bmhost in $(yq e -o=j -I=0 '.[]' "${SHARED_DIR}/hosts.yaml"); do
  ...
  if [ "${masters}" -eq 1 ]; then
    ...
    if [ "${ipv4_enabled:-}" == "true" ]; then
      DNS_FORWARD="${DNS_FORWARD}
api.${CLUSTER_NAME} IN A ${ip}
provisioner.${CLUSTER_NAME} IN A ${INTERNAL_NET_IP}
api-int.${CLUSTER_NAME} IN A ${ip}
*.apps.${CLUSTER_NAME} IN A ${ip}"
    fi
    if [ "${ipv6_enabled:-}" == "true" ]; then
      DNS_FORWARD="${DNS_FORWARD}
api.${CLUSTER_NAME} IN AAAA ${ipv6}
provisioner.${CLUSTER_NAME} IN AAAA ${INTERNAL_NET_IPV6}
api-int.${CLUSTER_NAME} IN AAAA ${ipv6}
*.apps.${CLUSTER_NAME} IN AAAA ${ipv6}"
    fi
   fi
  if [ "${ipv4_enabled:-}" == "true" ]; then
    DNS_FORWARD="${DNS_FORWARD}
