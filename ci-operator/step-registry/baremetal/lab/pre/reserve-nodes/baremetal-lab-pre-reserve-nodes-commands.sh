...
timeout -s 9 180m ssh "${SSHOPTS[@]}" "root@${AUX_HOST}" bash -s -- \
  ...
  "${ADDITIONAL_WORKERS:-0}" "${gnu_additional_arch:-x86_64}" "${VENDOR}" "${ADDITIONAL_WORKERS_VENDOR:-$VENDOR}" "${MAX_WAIT_MINS}" << 'EOF'
set -o nounset
set -o errexit
set -o pipefail
...
N_MASTERS=${N_MASTERS} N_WORKERS=${N_WORKERS} \
  REQUEST_BOOTSTRAP_HOST=${REQUEST_BOOTSTRAP_HOST} REQUEST_VIPS=true APPEND="false" ARCH="${ARCH}" VENDOR="${VENDOR}" MAX_WAIT_MINS="${MAX_WAIT_MINS}" /usr/bin/reserve-hosts.sh
...
if [ "${ADDITIONAL_WORKERS}" -gt 0 ]; then
  N_WORKERS="${ADDITIONAL_WORKERS}" N_MASTERS=0 REQUEST_BOOTSTRAP_HOST=false \
   ARCH="${ADDITIONAL_WORKER_ARCHITECTURE}" APPEND="true" REQUEST_VIPS=false VENDOR="${ADDITIONAL_WORKERS_VENDOR}" MAX_WAIT_MINS="${MAX_WAIT_MINS}" /usr/bin/reserve-hosts.sh
fi
EOF
