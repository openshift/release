...
for bmhost in $(yq e -o=j -I=0 '.[]' "${SHARED_DIR}/hosts.yaml"); do
  ...
  [ "$USE_CONSOLE_HOOK" == "true" ] && kargs="${kargs} ignition.config.url=http://${sINTERNAL_NET_IPV6}/${CLUSTER_NAME}/$mac_postfix-console-hook.ign"
...
menuentry 'Install ($flavor)' {
    ...
    insmod gzio
    linux  /${CLUSTER_NAME}/vmlinuz_${arch}_2 debug nosplash ip=[${ipv6}]::[${INTERNAL_NET_IPV6}]:64::${baremetal_iface}:none nameserver=${INTERNAL_NET_IPV6}$kargs coreos.live.rootfs_url=http://${sINTERNAL_NET_IPV6}/${CLUSTER_NAME}/rootfs-${arch}_2.img ignition.firstboot ignition.platform.id=metal panic=30
...
menuentry 'Install ($flavor)' {
    set gfx_payload=keep
    insmod gzio
    linux  /${CLUSTER_NAME}/vmlinuz_${arch} debug nosplash ip=[${ipv6}]::[${INTERNAL_NET_IPV6}]:64::${baremetal_iface}:none nameserver=${INTERNAL_NET_IPV6} $kargs coreos.live.rootfs_url=http://${sINTERNAL_NET_IPV6}/${CLUSTER_NAME}/rootfs-${arch}.img ignition.firstboot ignition.platform.id=metal panic=30
