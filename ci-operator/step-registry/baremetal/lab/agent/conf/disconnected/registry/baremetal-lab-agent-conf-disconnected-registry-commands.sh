...
#build_farm_secrets="/var/run/vault/secrets/.dockerconfigjson"
#extract_build_farm_auth=$(jq -c "'.auths.${MIRROR_REGISTRY_HOST}'" ${build_farm_secrets})
#final_pull_secret=$(jq -c --argjson auth "$extract_build_farm_auth" "'.auths["${MIRROR_REGISTRY_HOST}"] += $auth'" "${pull_secret_path}")
...
#build_farm_secrets="/var/run/vault/secrets/.dockerconfigjson"
#registry_cred=$(jq -c "'.auths.${MIRROR_REGISTRY_HOST}'" ${build_farm_secrets})
