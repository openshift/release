...
"iso")
  ...
  scp "${SSHOPTS[@]}" "${DAY2_INSTALL_DIR}/node.${arch}.iso" "root@${AUX_HOST}:${DATA_STORAGE}/html/${CLUSTER_NAME}.node.iso"
...
"pxe")
  ...
  scp "${SSHOPTS[@]}" "${DAY2_INSTALL_DIR}"/node.*-rootfs* \
   "root@${AUX_HOST}:${DATA_STORAGE}/html/${CLUSTER_NAME}/rootfs-${arch}_2.img"
;;
*)
