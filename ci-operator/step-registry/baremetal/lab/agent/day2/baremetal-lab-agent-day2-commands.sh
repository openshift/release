...
"pxe")
  ...
  noglob scp "${SSHOPTS[@]}" "${DAY2_INSTALL_DIR}"/boot-artifacts/*-vmlinuz* \
   "root@${AUX_HOST}:/opt/dnsmasq/tftpboot/${CLUSTER_NAME}/vmlinuz_${arch}_2"
  noglob scp "${SSHOPTS[@]}" "${DAY2_INSTALL_DIR}"/boot-artifacts/*-initrd* \
   "root@${AUX_HOST}:/opt/dnsmasq/tftpboot/${CLUSTER_NAME}/initramfs_${arch}_2.img"
  noglob scp "${SSHOPTS[@]}" "${DAY2_INSTALL_DIR}"/boot-artifacts/*-rootfs* \
