...
scp -r "${SSHOPTS[@]}" "root@${AUX_HOST}:/mnt/data-storage/html/${CLUSTER_NAME}/${COREOS_IMAGE_NAME}" "${INSTALL_DIR}/"
...
scp "${SSHOPTS[@]}" "${INSTALL_DIR}/${UNCONFIGURED_AGENT_IMAGE_FILENAME}" \
  "root@${AUX_HOST}:/mnt/data-storage/html/${CLUSTER_NAME}/${UNCONFIGURED_AGENT_IMAGE_FILENAME}"
