...
"
  ...
  for bmhost in $(yq e -o=j -I=0 '.[]' "${SHARED_DIR}/hosts.yaml"); do
    ...
    if [[ "${name}" == *-a-* ]] && [ "${ADDITIONAL_WORKERS_DAY2}" == "true" ]; then
      # Do not create config for additional workers if we need to run them as day2 (e.g., to test single-arch clusters based
...
scp "${SSHOPTS[@]}" "${CACHED_COREOS_IMAGE_PATH}" "root@${AUX_HOST}:/mnt/data-storage/html/${CLUSTER_NAME}/${COREOS_IMAGE_NAME}"
