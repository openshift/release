...
if oc get baremetalhost -n "${capi_namespace}" | grep "provisioning"; then
  ...
  oc scale machineset.cluster.x-k8s.io "${machineset}" -n openshift-cluster-api --replicas="${replicas}"
fi
...
while ! oc get machineset.cluster.x-k8s.io -n openshift-cluster-api -o jsonpath='{.items[0].status.v1beta2.availableReplicas}{"\n"}' | grep "${replicas}"; do
  echo "${#bmhlist[@]} replicas are not available in ${machineset}. Waiting 30 seconds..."
    sleep 30
done

echo "--- Waiting for ready replicas ---"
while ! oc get machineset.cluster.x-k8s.io -n openshift-cluster-api -o jsonpath='{.items[0].status.v1beta2.readyReplicas}{"\n"}' | grep "${replicas}"; do
  echo "${#bmhlist[@]} replicas are not available in ${machineset}. Waiting 30 seconds..."
    sleep 30
done
