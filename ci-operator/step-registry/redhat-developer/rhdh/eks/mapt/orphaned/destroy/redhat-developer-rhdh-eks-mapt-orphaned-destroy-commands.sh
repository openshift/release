...
if [ ! -s "${SHARED_DIR}/s3_top_level_folders.txt" ]; then
  echo "⚠️ WARNING: Input file ${SHARED_DIR}/s3_top_level_folders.txt is empty"
...
for S3_TOP_LEVEL_FOLDER in "${CORRELATE_MAPT_ARRAY[@]}"; do
  ...
  [ -z "$S3_TOP_LEVEL_FOLDER" ] && echo "⚠️ Skipping empty folder name" && continue

  echo "🗑️ Destroying MAPT for folder: ${S3_TOP_LEVEL_FOLDER}"
...
if [ "${success_count}" -gt 0 ]; then
  echo ""
  echo "🗑️ Deleting ${success_count} successfully destroyed folders from S3 bucket..."

  while IFS= read -r folder; do
    if [ -n "$folder" ]; then
      echo "🗑️ Deleting s3://${AWS_S3_BUCKET}/${folder}/..."
