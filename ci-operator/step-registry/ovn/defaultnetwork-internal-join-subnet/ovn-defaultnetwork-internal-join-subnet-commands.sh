...
oc patch networks.operator.openshift.io cluster --type=merge -p '{"spec":{"defaultNetwork":{"ovnKubernetesConfig":{"ipv4":{"internalJoinSubnet": "100.89.0.0/16"}}}}}'
...
internalJoinSubnet=$(oc get network.operator.openshift.io -o jsonpath='{.items[0].spec.defaultNetwork.ovnKubernetesConfig.ipv4.internalJoinSubnet}')
if [[ "$internalJoinSubnet" != "100.89.0.0/16" ]]; then
  echo "Error: internalJoinSubnet is misconfigured. Expected internalJoinSubnet of 100.89.0.0/16, but got:"
...
check_annotation_on_nodes "k8s.ovn.org/node-gateway-router-lrp-ifaddr" "ipv4" "100\.89\.[0-9]+\.[0-9]+/16"
