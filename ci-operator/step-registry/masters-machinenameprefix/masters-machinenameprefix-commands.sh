...
function patchMasterMachines() {
    for machine in `oc get machines.machine.openshift.io -n openshift-machine-api -l machine.openshift.io/cluster-api-machine-role=master -o='jsonpath={.items[*].metadata.name}'`; do   
        ...
        echo "current machines $(oc get machines.machine.openshift.io -n openshift-machine-api)"
        echo "current co $(oc get co)"
        echo "current node $(oc get node)"
    done
    log "The machines are ready!"

}

log "current machines $(oc get machines.machine.openshift.io -n openshift-machine-api)"
