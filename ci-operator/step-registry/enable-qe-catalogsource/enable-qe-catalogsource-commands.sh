...
function update_global_auth () {
 ...
 # run_command "cat /tmp/.dockerconfigjson | jq"
  ...
  reg_redhat_user=$(cat "/var/run/vault/mirror-registry/registry_redhat.json" | jq -r '.user')
  reg_redhat_password=$(cat "/var/run/vault/mirror-registry/registry_redhat.json" | jq -r '.password')
  redhat_registry_auth=`echo -n "${reg_redhat_user}:${reg_redhat_password}" | base64 -w 0`
  jq --argjson a "{\"brew.registry.redhat.io\": {\"auth\": \"${brew_registry_auth}\"},\"registry.redhat.io\": {\"auth\": \"${redhat_registry_auth}\"},\"quay.io/openshift-qe-optional-operators\": {\"auth\": \"${qe_registry_auth}\"},\"quay.io/openshifttest\": {\"auth\": \"${openshifttest_registry_auth}\"},\"registry.stage.redhat.io\": {\"auth\": \"$stage_registry_auth\"}}" '.auths |= . + $a' "/tmp/.dockerconfigjson" > ${new_dockerconfig}
