...
function mirror_optional_images() {
    ...
    unset_proxy
    skopeo login -u $brew_auth_user -p $brew_auth_password brew.registry.redhat.io || return 1
    skopeo login -u $mirror_user -p $mirror_password ${MIRROR_REGISTRY_HOST} || return 1
    skopeo login -u $optional_auth_user -p $optional_auth_password registry.redhat.io || return 1
    ...
    if [[ "$?" != "0" ]]  ;then
        echo "Warning, skopeo failed, abort !!!"
        return 1
    fi
...
if [ $mirror -eq 1 ]; then
    ...
    set_cluster_auth
    create_settled_icsp
    create_settled_aosqe_catalogsource
fi 
