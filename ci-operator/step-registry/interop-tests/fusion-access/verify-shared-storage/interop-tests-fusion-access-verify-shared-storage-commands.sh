...
test_start=$(start_test "Step 3: Creating PVC from Fusion Access side")
test_status="failed"
test_message=""
...
then
  ...
  if oc wait pvc fusion-shared-storage-pvc -n "${TEST_NAMESPACE}" --for=condition=Bound --timeout=5m 2>/dev/null; then
    ...
    echo "  ⚠️  Fusion Access PVC not bound within timeout"
    test_message="Fusion Access PVC not bound within 5m timeout"
    oc get pvc fusion-shared-storage-pvc -n "${TEST_NAMESPACE}" -o yaml
  fi
else
  echo "  ❌ Failed to create Fusion Access PVC"
  test_message="Failed to create Fusion Access PVC resource"
fi
...
test_start=$(start_test "Step 4: Reading data from Fusion Access PVC")
test_status="failed"
test_message=""
...
then
  ...
  if oc wait pod fusion-data-reader -n "${TEST_NAMESPACE}" --for=condition=Ready --timeout=2m 2>/dev/null; then
    ...
    test_message="Fusion Access data reader pod not ready within 2m timeout"
    oc describe pod fusion-data-reader -n "${TEST_NAMESPACE}"
  fi
else
  echo "  ❌ Failed to create Fusion Access data reader pod"
  test_message="Failed to create Fusion Access data reader pod resource"
fi
...
if oc exec fusion-data-reader -n "${TEST_NAMESPACE}" -- ls -la /shared-storage/ 2>/dev/null; then
  ...
  test_status="failed"
  test_message="${test_message}; Fusion Access pod cannot access shared storage"
fi
