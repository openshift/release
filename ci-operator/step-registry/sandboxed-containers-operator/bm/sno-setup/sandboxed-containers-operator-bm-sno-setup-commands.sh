...
PCCS_API_KEY=$(cat /usr/local/intel-dcap-secrets/secrets/PCCS_API_KEY)
...
cat << EOF > $SHARED_DIR/sno-post-installation.yml
---
- name: Check if cluster is ready
  hosts: bastion
  gather_facts: false
  ...
  - name: Enable default secrets for image registry -1 
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${SNO_HOST_NAME}/kubeconfig patch configs.imageregistry.operator.openshift.io cluster --type merge --patch '{"spec":{"managementState":"Managed"}}'
    register: ready_check

  - name: Enable default secrets for image registry -2
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${SNO_HOST_NAME}/kubeconfig patch configs.imageregistry.operator.openshift.io cluster --type merge --patch '{"spec":{"storage":{"emptyDir":{}}}}'
    register: ready_check

  - name: Enable default secrets for image registry -3
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${SNO_HOST_NAME}/kubeconfig get pod -n openshift-image-registry -l docker-registry=default
    register: ready_check

  - name: Create secrets to have pccs api key
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${SNO_HOST_NAME}/kubeconfig create secret generic pccs-api-key-secret --from-literal pccsApiKey=${PCCS_API_KEY}
    register: error_check
EOF
...
cp $SHARED_DIR/pull_secret.txt $SHARED_DIR/jetlag
...
cp $SHARED_DIR/sno-post-installation.yml $SHARED_DIR/jetlag/ansible/
...
echo "==========  Running Ansible to do SNO post installation  =========="
ANSIBLE_STDOUT_CALLBACK=debug ansible-playbook -i $SHARED_DIR/jetlag/ansible/inventory/byol-inventory-sno.sample $SHARED_DIR/jetlag/ansible/sno-post-installation.yml -vv || status=$?
