...
# Generic wait loop with condition checking
# Usage: wait_until <description> <timeout_seconds> <check_interval> <condition_command>
# Returns: 0 on success, 1 on timeout
# Example: wait_until "pod to be ready" 300 5 'oc get pod mypod -o jsonpath="{.status.phase}" | grep -q Running'
function wait_until() {
  local description="$1"
  ...
  local progress_interval=$((60 / check_interval))  # Show progress every 60 seconds
...
function wait_for_operator() {
  ...
  if [[ -n "${TRUSTEE_CATALOG_SOURCE_IMAGE}" ]]; then
    ...
    if ! wait_until "Trustee CatalogSource ${actual_catalog_name} READY" 60 5 \
      "[[ \"\$(oc get catalogsource -n openshift-marketplace '${actual_catalog_name}' -o jsonpath='{.status.connectionState.lastObservedState}' 2>/dev/null)\" == \"READY\" ]]"; then
      echo ">>> All CatalogSources in openshift-marketplace:" >&2
      oc get catalogsource -n openshift-marketplace || true
      echo ">>> Details of ${actual_catalog_name}:" >&2
  ...
  # Capture the installplan ref for next stage
  ...
  echo ">>> Subscription references InstallPlan: ${installplan_ref}" >&2

  # Stage 3: Wait for InstallPlan to be Complete (300s)
  if ! wait_until "InstallPlan ${installplan_ref} Complete" 300 5 \
    "[[ \"\$(oc get installplan -n '${TRUSTEE_NAMESPACE}' '${installplan_ref}' -o jsonpath='{.status.phase}' 2>/dev/null)\" == \"Complete\" ]]"; then
    echo ">>> ERROR: InstallPlan not Complete" >&2
  ...
  if ! wait_until "CSV Succeeded" 600 5 \
    "[[ \"\$(oc get csv -n '${TRUSTEE_NAMESPACE}' -o jsonpath='{.items[0].status.phase}' 2>/dev/null)\" == \"Succeeded\" ]]"; then
    echo ">>> ERROR: CSV not Succeeded" >&2
  ...
  echo ">>> CSV ${csv_name} is Succeeded" >&2

  # Stage 5: Wait for Deployment to be Available (600s / 10 minutes)
  if ! wait_until "operator deployment Available" 600 5 \
    "oc get deployment -n '${TRUSTEE_NAMESPACE}' -l control-plane=controller-manager -o jsonpath='{.items[0].status.conditions[?(@.type==\"Available\")].status}' 2>/dev/null | grep -q 'True'"; then
    echo ">>> ERROR: Operator deployment not Available" >&2
  ...
  if ! wait_until "operator pods Ready (1/1)" 600 5 \
    "ready_count=\$(oc get pods -n '${TRUSTEE_NAMESPACE}' -l control-plane=controller-manager -o jsonpath='{.items[*].status.containerStatuses[0].ready}' 2>/dev/null | tr ' ' '\\\n' | grep -c 'true' || echo '0'); total_count=\$(oc get pods -n '${TRUSTEE_NAMESPACE}' -l control-plane=controller-manager --no-headers 2>/dev/null | wc -l); [[ \${ready_count} -gt 0 ]] && [[ \${ready_count} -eq \${total_count} ]]"; then
    echo ">>> ERROR: Operator pods not Ready" >&2
    echo ">>> Pods:" >&2
  ...
  oc get pods -n "${TRUSTEE_NAMESPACE}" -l control-plane=controller-manager || true
  echo ">>> Operator installation complete" >&2
}
...
function get_kbs_client_tag() {
  ...
  echo ">>> WARN: Could not determine kbs-client tag, using fallback: v0.19.0" >&2
  echo "v0.19.0"
}
...
function verify_trustee_connectivity() {
  ...
  if oc exec "${kbs_client_pod}" -n "${kbs_client_namespace}" -- \
    ...
    if [[ -n "${expected_value}" ]] && [[ "${resource_value}" != "${expected_value}" ]]; then
      ...
      echo ">>> ✓ Resource value matches expected value"
