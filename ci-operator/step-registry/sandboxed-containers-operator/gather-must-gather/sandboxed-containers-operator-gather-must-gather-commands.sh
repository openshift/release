...
if [[ "${MUST_GATHER_ON_FAILURE_ONLY:-false}" == "true" ]]; then
    ...
    if [[ -f "${SHARED_DIR}/test-failures" ]] || [[ -f "${ARTIFACT_DIR}/test-failures" ]]; then
        echo "Found test failure marker file"
        test_failed=true
    fi

    # Method 3: Check for non-zero exit codes in test logs (common pattern)
    if find "${ARTIFACT_DIR}" -name "*test*.log" -type f -exec grep -l "exit code: [1-9]" {} \; 2>/dev/null | head -1 >/dev/null; then
