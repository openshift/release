...
# ipi-conf-proxy (runs before this step) sets proxy.httpProxy/httpsProxy but
# does not set noProxy, so the mirror registry would otherwise be reachable
# only through the bastion proxy. Exclude it explicitly: mirror traffic goes
# straight to the mirror (often over a private endpoint), not through squid.
if [[ -f "${SHARED_DIR}/proxy_private_url" ]]; then
    NOPROXY_PATCH="${SHARED_DIR}/install-config-noproxy.yaml.patch"
    cat > "${NOPROXY_PATCH}" <<EOF
proxy:
  noProxy: ${MIRROR_REGISTRY_HOST}
EOF
    yq-go m -x -i "${CONFIG}" "${NOPROXY_PATCH}"
fi
