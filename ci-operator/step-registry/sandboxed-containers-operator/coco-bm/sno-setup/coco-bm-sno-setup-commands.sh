...
SNO_CONTROLPLANE_LAB_INTERFACE=""
SNO_BMC_ADDRESS=""
SNO_MAC_ADDRESS=""
SNO_IP_ADDRESS=""
SNO_VENDOR=""
...
[sno]
sno-0 bmc_address=${SNO_BMC_ADDRESS} mac_address=${SNO_MAC_ADDRESS} lab_mac=${SNO_MAC_ADDRESS} ip=${SNO_IP_ADDRESS} vendor=${SNO_VENDOR} install_disk=${SNO_INSTALL_DISK} boot_iso=sno-0.iso
...
- name: Fetch information about cluster
  ...
  - name: Get cluster version
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${OCP_TYPE}-0/kubeconfig get clusterversion
    ignore_errors: true

  - name: Get bmh objects
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${OCP_TYPE}-0/kubeconfig get bmh -A
    ignore_errors: true

  - name: Get nodes
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${OCP_TYPE}-0/kubeconfig get node
    ignore_errors: true

  - name: Get MCP
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${OCP_TYPE}-0/kubeconfig get mcp
    ignore_errors: true

  - name: Get operators
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${OCP_TYPE}-0/kubeconfig get co
    ignore_errors: true
EOF
...
- name: Check if cluster is ready
  ...
  - name: Check if cluster is available
    shell: oc --kubeconfig=/root/${OCP_TYPE}/${OCP_TYPE}-0/kubeconfig get clusterversion -o=jsonpath='{.items[0].status.conditions[?(@.type=='\''Progressing'\'')].status}'
...
ANSIBLE_STDOUT_CALLBACK=debug ansible-playbook -i $SHARED_DIR/jetlag/ansible/inventory/byol-inventory-sno.sample $SHARED_DIR/jetlag/ansible/fetch-information.yml -vv || true

echo "==========  Running Ansible to check SNO cluster status  =========="
ANSIBLE_STDOUT_CALLBACK=debug ansible-playbook -i $SHARED_DIR/jetlag/ansible/inventory/byol-inventory-sno.sample $SHARED_DIR/jetlag/ansible/check-cluster.yml -vv
