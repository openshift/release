...
STEP_NAME="SKELETON2"
ENABLE_VAL="${TEST_SKELETON2_ENABLE:-false}"
...
if [[ "${ENABLE_VAL}" != "true" ]]; then
    echo "skeleton2 suite DISABLED (TEST_${STEP_NAME}_ENABLE=${ENABLE_VAL}); exiting 0."
    cat > "${JUNIT}" <<EOF
...
<testsuite name="sandboxed-containers-operator-testsuites-skeleton2" tests="1" failures="0" errors="0" skipped="1">
  <testcase name="skeleton2" classname="osc.testsuites.skeleton2" time="0">
    <skipped message="TEST_SKELETON2_ENABLE=${ENABLE_VAL}"/>
  </testcase>
</testsuite>
EOF
    exit 0
...
echo "skeleton2 suite ENABLED; always succeeds -- demonstrates it runs even after skeleton fails."
cat > "${JUNIT}" <<EOF
<?xml version="1.0" encoding="UTF-8"?>
