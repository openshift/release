...
show_usage() {
    ...
    echo "run               Run one prowjob from YAML (requires job_yaml_file and"
    echo "                   exactly one job name)"
    echo "update_templates  Regenerate the ci-operator/config/openshift/"
    echo "                   sandboxed-containers-operator templates using default"
    echo "                   values (unless overridden)"
    echo ""
    echo "Examples:"
    echo "$0 create"
    echo "$0 run /path/to/job_yaml.yaml azure-ipi-kata"
    echo ""
    echo "Environment variables for 'create' command (defaults shown):"
    echo ""
    echo "ARO_CLUSTER_VERSION       - ARO cluster version (default: ${ARO_CLUSTER_VERSION})"
    echo "OCP_VERSION               - OpenShift Container Platform version."
    echo "                            Supports X.Y (latest), X.Y.Z (specific), or"
    echo "                            X.Y.Z-rc.N/ec.N (candidate). (default: 4.19)"
    echo "                            Validation: If a specific version is"
    echo "                            requested, it must exist in the channel."
    echo "OCP_CHANNEL               - OCP release channel. (default: fast) Fast"
    echo "                            contains all versions that could become"
    echo "                            'stable'. Validation: stable, fast,"
    echo "                            candidate, or eus. RC/EC require candidate."
    echo "PROW_RUN_TYPE             - Prow job run type. Derived from"
    echo "                            TEST_RELEASE_TYPE (Pre-GA -> candidate,"
    echo "                            GA -> release). (default: candidate when Pre-GA)"
    echo "AWS_REGION_OVERRIDE       - AWS region for testing. (default: us-east-2)"
    echo "AWS_ALLOWED_REGIONS       - Space-separated list of allowed AWS regions;"
    echo "                            for validating/selecting region."
    echo "CUSTOM_AZURE_REGION       - Azure region for testing. (default: eastus)"
    echo "OSC_CATALOG_TAG           - (default: ':latest') Resolved at runtime to"
    echo "                            latest build by env-cm. Override with tag"
    echo "                            (e.g. 1.11.1-1766149846) or SHA."
    echo "INSTALL_KATA_RPM          - Whether to install Kata RPM on worker nodes."
    echo "                            (default: false)"
    echo "KATA_RPM_VERSION          - Kata RPM version when INSTALL_KATA_RPM=true."
    echo "                            (default: 3.17.0-3.rhaos4.19.el9)"
    echo "KATA_RPM_BUILD_TASK       - Alternative to  KATA_RPM_VERSION;"
    echo "                            build task of RPM from brew/koji."
    echo "SLEEP_DURATION            - Time to keep cluster alive after tests (up"
    echo "                            to 12h for manual testing). (default: 0h)."
    echo "                            Must be number followed by 'h'."
    echo "TEST_RELEASE_TYPE         - Release type. Pre-GA = development catalogs;"
    echo "                            GA = production catalog sources."
    echo "                            (default: Pre-GA)"
    echo "TEST_TIMEOUT              - Test timeout in minutes. (default: 90)"
    echo "TEST_SCENARIOS            - Test scenarios filter."
    echo "                            (default: sig-kata.*Kata Author)"
    echo "ENABLE_MUST_GATHER        - Whether to run the must-gather step."
    echo "                            (default: true)"
    echo "MUST_GATHER_IMAGE         - Must-gather image for sandboxed containers"
    echo "                            operator diagnostics. (default:"
    echo "                            registry.redhat.io/openshift-sandboxed-"
    echo "                            containers/osc-must-gather-rhel9:latest)"
    echo "MUST_GATHER_ON_FAILURE_ONLY - When true, only run must-gather if tests"
    echo "                            failed. (default: true)"
    echo "MUST_GATHER_TIMEOUT       - (From gather-must-gather step.) Timeout for"
    echo "                            oc adm must-gather. (default in workflow: 35m)"
    echo "TRUSTEE_URL               - Points OSC operator to an existing external"
    echo "                            Trustee instance. (default: empty)"
    echo "INITDATA                  - Gzipped, base64-encoded initdata.toml from"
    echo "                            Trustee for peer-pods-cm."
    echo "                            (default: empty)"
}
