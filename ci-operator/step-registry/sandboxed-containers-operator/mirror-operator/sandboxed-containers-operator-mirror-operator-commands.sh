...
if [[ "${USE_ACR}" == "true" ]]; then
    printf '%s' "${mirror_registry_password}" | skopeo login "${MIRROR_REGISTRY_HOST}" -u "${mirror_registry_user}" --password-stdin
else
    printf '%s' "${mirror_registry_password}" | skopeo login "${MIRROR_REGISTRY_HOST}" -u "${mirror_registry_user}" --password-stdin --tls-verify=false
fi
printf '%s' "${redhat_auth_password}" | skopeo login registry.redhat.io -u "${redhat_auth_user}" --password-stdin
set -x

# The mirror registry no longer allows anonymous pulls. Publish a
# dockerconfigjson Secret in the default namespace so peer-pods (kata-remote)
# test workloads can reference it via imagePullSecrets to pull mirrored
# images inside the podvm. See docs for osc-mirror-pull-secret.
set +x
oc create secret docker-registry osc-mirror-pull-secret \
    --docker-server="${MIRROR_REGISTRY_HOST}" \
    --docker-username="${mirror_registry_user}" \
    --docker-password="${mirror_registry_password}" \
    -n default --dry-run=client -o yaml | oc apply -f -
set -x
...
if [[ -n "${EXTRA_IMAGES}" ]]; then
    ...
    for img in $(echo "${EXTRA_IMAGES}" | tr ',' ' '); do
        ...
        skopeo copy --all \
            "docker://${img}" \
            "${dest}" \
            "${DEST_TLS_ARGS[@]}" || { echo "ERROR: failed to copy ${img}" >&2; exit 1; }
    done
fi
