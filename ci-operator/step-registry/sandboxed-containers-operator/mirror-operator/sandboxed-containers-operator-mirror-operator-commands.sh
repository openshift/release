...
if [[ -n "${CATALOG_SOURCE_IMAGE:-}" ]]; then
    ...
    for img in ${fbc_images}; do
        ...
        konflux_src=$(grep -B2 "source: ${img_repo}" "${work_dir}/konflux-mirror-set.yaml" 2>/dev/null | \
            grep -E "^\s+- " | head -1 | awk '{print $2}' || echo "")
...
sleep 30
oc wait mcp --all --for=condition=Updated --timeout=1200s || {
