...
ssh "${SSHOPTS[@]}" "root@${IP}" "cd /root/dev-scripts/ && rm -rf metallb/ && git clone ${METALLB_REPO} && mv metallb-openshift metallb && cd metallb/ && git checkout ciupdate1"
#fi
...
rc=$?
scp "${SSHOPTS[@]}" -r "root@${IP}:/root/dev-scripts/metallb/e2etest/junit-report.xml" "${ARTIFACT_DIR}/junit_e2e.xml" || true
exit "${rc}"
