...
# Install jq if not present (cli image does not include it).
if ! command -v jq &>/dev/null; then
 curl -sL https://github.com/jqlang/jq/releases/download/jq-1.8.1/jq-linux-amd64 -o /tmp/jq && chmod +x /tmp/jq
...
# Extensions repo: default matches update step (full org/repo-extensions).
EXTENSIONS_REPO="${CUSTOM_OS_EXTENSIONS_REPO:-${CUSTOM_OS_IMAGE_REPO}-extensions}"
