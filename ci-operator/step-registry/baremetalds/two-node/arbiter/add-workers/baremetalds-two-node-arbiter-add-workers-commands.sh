...
wait_for_cluster_operators_stable() {
    log_step "Waiting for all cluster operators to stabilize after adding workers"
    ...
    oc wait clusteroperators --all --for=condition=Degraded=false --timeout=5m

    log "All cluster operators are stable"
    oc get clusteroperators
}

wait_for_critical_daemonsets() {
    log_step "Waiting for critical DaemonSets to be fully deployed"

    # Wait for network operator DaemonSets
    for ds in ovnkube-node iptables-alerter; do
        if oc get daemonset "${ds}" -n openshift-network-operator &>/dev/null; then
            oc rollout status daemonset/"${ds}" -n openshift-network-operator --timeout=10m || true
        fi
    done

    # Check DaemonSet status
    log "DaemonSet status in openshift-network-operator:"
    oc get daemonsets -n openshift-network-operator
}
