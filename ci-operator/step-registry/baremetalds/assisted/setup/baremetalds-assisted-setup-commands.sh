...
echo "************ baremetalds devscripts-assisted-installer setup command ************"
...
echo "export NODES_PLATFORM=assisted" >> /root/dev-scripts/config_root.sh
echo "export IP_STACK=v4" >> /root/dev-scripts/config_root.sh
echo "export NUM_WORKERS=1" >> /root/dev-scripts/config_root.sh
echo "export INSTALL_OPERATOR_SDK=0" >> /root/dev-scripts/config_root.sh
echo "export MANAGE_BR_BRIDGE=n" >> /root/dev-scripts/config_root.sh
echo "export OPENSHIFT_VERSION=4.5" >> /root/dev-scripts/config_root.sh
...
timeout -s 9 105m make assisted
