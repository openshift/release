...
# advertise the default pod network
oc apply -f - <<EOF
...
for vrf in "${!vrf_neighbors[@]}"; do
  [ "default" = "$vrf" ] && {
    ...
    subnet_v4_var=${vrf^^}_NETWORK_SUBNET_V4
    subnet_v6_var=${vrf^^}_NETWORK_SUBNET_V6
    ip=${!subnet_v4_var/\.0\/*/.3}
    ip6=${!subnet_v6_var/::\/*/::3}
    name=receive-filtered-$vrf
    vrf="vrf: ${vrf}"
  }
