...
# TODO: remove to use master's code instead of the branch
git remote add osherdp https://github.com/osherdp/assisted-test-infra
git fetch osherdp
git checkout test/bootstrap-in-place
...
echo "export OPENSHIFT_INSTALL_RELEASE_IMAGE=registry.svc.ci.openshift.org/sno-dev/openshift-bip:0.1.0" >> /root/config
