...
# For OpenShift 4.21+, use Konflux catalogsource index image by default
if [[ ${MINOR_VERSION} -ge 21 ]]; then
  METALLB_INDEX_IMAGE="quay.io/redhat-user-workloads/ocp-art-tenant/art-fbc:ocp__${CLUSTER_VERSION}__metallb-rhel9-operator"
fi

# Allow overriding the METALLB_INDEX_IMAGE with the Gangway API
if [[ -n "${MULTISTAGE_PARAM_OVERRIDE_METALLB_INDEX_IMAGE:-}" ]]; then
  METALLB_INDEX_IMAGE=${MULTISTAGE_PARAM_OVERRIDE_METALLB_INDEX_IMAGE}
fi

# Check if METALLB_INDEX_IMAGE is not empty and skip the step if it is
if [[ -z "${METALLB_INDEX_IMAGE:-}" ]]; then
    echo "WARNING: METALLB_INDEX_IMAGE is empty or not set"
    echo "Skipping MetalLB CatalogSource step"
    exit 0
else
    echo "METALLB_INDEX_IMAGE is set to: $METALLB_INDEX_IMAGE"
fi
...
function create_catalog_sources {
	local node_name
	echo "Creating CatalogSource: $CATALOG_SOURCE using index image: $METALLB_INDEX_IMAGE"
...
spec:
  displayName: MetalLB CatalogSource
  image: $METALLB_INDEX_IMAGE
  publisher: OpenShift MetalLB
  sourceType: grpc
  updateStrategy:
    registryPoll:
      interval: 15m
EOF
	...
	[[ $status != "READY" ]] && {
      ...
      pod-security.kubernetes.io/enforce=privileged pod-security.kubernetes.io/audit=privileged pod-security.kubernetes.io/warn=privileged --overwrite"
		run "oc -n debug-qe debug node/$node_name -- chroot /host podman pull --authfile /var/lib/kubelet/config.json $METALLB_INDEX_IMAGE"
...
function main {
	echo "Enabling MetalLB CatalogSource"
	set_proxy
