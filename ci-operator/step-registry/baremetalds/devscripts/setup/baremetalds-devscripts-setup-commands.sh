...
if [[ "${ARCHITECTURE}" == "arm64" ]]; then
  export DOCKER_REGISTRY_IMAGE=quay.io/libpod/registry:2.8
  echo "export OPENSHIFT_RELEASE_IMAGE=${OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE}" >> /root/dev-scripts/config_root.sh
  export IRONIC_LOCAL_IMAGE=$(oc adm release info -a "${CLUSTER_PROFILE_DIR}/pull-secret" \
    "${OPENSHIFT_INSTALL_RELEASE_IMAGE_OVERRIDE}" -o json | jq -r '.references.spec.tags[] | select(.name=="ironic").from.name')
  export SUSHY_TOOLS_IMAGE=quay.io/multi-arch/sushy-tools:muiltarch
else
