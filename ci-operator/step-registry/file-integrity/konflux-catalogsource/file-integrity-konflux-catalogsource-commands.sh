...
mirror:
  ...
  - catalog: "${INDEX_IMAGE}"
EOF
  run "/tmp/oc-mirror --v2 --config=${TMP_DIR}/imageset-config.yaml docker://${MIRROR_REGISTRY_HOST} --dest-tls-verify=false --src-tls-verify=false --log-level=debug --continue-on-error --skip-missing"
}
