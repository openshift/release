...
mirror:
  ...
  - catalog: ${INDEX_IMAGE}
    ...
    - name: ${PACKAGE_NAME}
EOF
  run "/tmp/oc-mirror --v2 --config=${TMP_DIR}/imageset-config.yaml --workspace=file://${TMP_DIR} docker://${MIRROR_REGISTRY_HOST} --dest-tls-verify=false --src-tls-verify=false --log-level=debug"
}
