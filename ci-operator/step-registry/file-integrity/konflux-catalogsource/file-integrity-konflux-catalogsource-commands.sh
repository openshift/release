...
mirror:
  ...
  - catalog: "oci://${TMP_DIR}/oci-local-catalog"
    ...
    targetTag: "latest"
EOF
  run "/tmp/oc-mirror --config=${TMP_DIR}/imageset-config.yaml docker://${MIRROR_REGISTRY_HOST} --oci-registries-config=${XDG_RUNTIME_DIR}/containers/registries.conf --dest-skip-tls --source-skip-tls --continue-on-error --skip-missing"
}
