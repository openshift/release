...
SMOKE_NS="kueue-pre-upgrade-smoke"

echo "Setting up pre-upgrade Kueue resources to verify they survive the upgrade..."
...
metadata:
  ...
  namespace: ${SMOKE_NS}
spec:
  clusterQueue: pre-upgrade-cq
EOF

echo "Waiting for LocalQueue to be active..."
oc wait --for=condition=Active localqueue/pre-upgrade-lq -n "${SMOKE_NS}" --timeout=5m \
  || {
    echo "LocalQueue not Active. Debugging..."
    oc get localqueue pre-upgrade-lq -n "${SMOKE_NS}" -o yaml 2>/dev/null || true
    oc get clusterqueue pre-upgrade-cq -o yaml 2>/dev/null || true
    oc get resourceflavor pre-upgrade-flavor -o yaml 2>/dev/null || true
    oc logs deployment/kueue-controller-manager -n "${NAMESPACE}" --tail=50 2>/dev/null || true
    exit 1
  }

echo "Creating pre-upgrade test job..."
oc apply -f - <<EOF
apiVersion: batch/v1
kind: Job
metadata:
  ...
  namespace: ${SMOKE_NS}
  labels:
    kueue.x-k8s.io/queue-name: pre-upgrade-lq
spec:
  template:
    spec:
      containers:
      - name: test
        ...
        command: ["echo", "pre-upgrade-smoke-passed"]
        resources:
          requests:
            cpu: "100m"
            memory: "64Mi"
...
oc wait --for=condition=Complete job/pre-upgrade-job -n "${SMOKE_NS}" --timeout=2m

echo "Pre-upgrade smoke setup complete. Resources will persist through the upgrade."
echo "Post-upgrade smoke test will verify new workloads can still be admitted."
