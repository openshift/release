...
EOF
    ...
    if wait_for_state "deployment/openshift-kueue-operator" "condition=Available" "5m" "openshift-kueue-operator"; then
        ...
        run_command "oc get deployment -n openshift-kueue-operator"
        run_command "oc get csv -n openshift-kueue-operator"
        run_command "oc get subscription kueue-operator -n openshift-kueue-operator -o=yaml"
        run_command "oc get event -n openshift-marketplace | grep kueue"
        exit 1
    fi
    ...
    # Create Kueue CR to start the controller manager
    echo "Creating Kueue CR to start the controller manager..."
    oc apply -f - <<EOF
...
EOF
    ...
    # Wait for Kueue controller manager to be ready
    if wait_for_state "deployment/kueue-controller-manager" "condition=Available" "5m" "openshift-kueue-operator"; then
        echo "Kueue controller manager is ready"
    else
        echo "Timed out waiting for Kueue controller manager. Dumping resources for debugging..."
        run_command "oc get pod -n openshift-kueue-operator"
        run_command "oc get event -n openshift-kueue-operator"
        run_command "oc get deployment -n openshift-kueue-operator"
        run_command "oc get kueue -n openshift-kueue-operator"
        exit 1
    fi
}
