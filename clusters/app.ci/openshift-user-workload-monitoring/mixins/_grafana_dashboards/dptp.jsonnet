...
.addPanel(
    (graphPanel.new(
        ...
        description='rate(prowjob_state_transitions{job_name=~"periodic-ipi-deprovision-.*|periodic-ci-openshift-release-main-deprovision-.*",state="failure"}[30m])',
        datasource='prometheus',
        legend_alignAsTable=true,
        legend_rightSide=true,
        legend_values=true,
    ...
    .addTarget(prometheus.target(
        'rate(prowjob_state_transitions{job_name=~"periodic-ipi-deprovision-.*|periodic-ci-openshift-release-main-deprovision-.*",state="failure"}[30m])',
        legendFormat='{{pod}}',
    )), gridPos={
    h: 9,
    w: 24,
