...
checkconfig:
	$(CONTAINER_ENGINE) run --rm -v "$(CURDIR):/release:z" gcr.io/k8s-prow/checkconfig:v20210901-32a67d8936 --config-path /release/core-services/prow/02_config/_config.yaml --job-config-path /release/ci-operator/jobs/ --plugin-config /release/core-services/prow/02_config/_plugins.yaml --supplemental-plugin-config-dir /release/core-services/prow/02_config --strict --exclude-warning long-job-names --exclude-warning mismatched-tide-lenient
...
clusterimagesets:
	$(CONTAINER_ENGINE) pull registry.ci.openshift.org/ci/clusterimageset-updater:latest
	$(CONTAINER_ENGINE) run --rm -v "$(CURDIR)/clusters/hive/pools:/pools:z" registry.ci.openshift.org/ci/clusterimageset-updater:latest --pools "/pools" --imagesets "/pools"
