...
update-ci-build-clusters:
	...
	$(CONTAINER_ENGINE) run $(CONTAINER_ENGINE_OPTS) $(CONTAINER_USER) --rm -v "$(CURDIR):/release$(VOLUME_MOUNT_FLAGS)" quay.io/openshift/ci-public:ci_cluster-init_latest config generate --release-repo=/release --create-pr=false --update=true
