...
prow-ocp-rpm-secrets:
	...
	oc create secret generic openstack-beta-rhel8-4-2-repos \
		--from-file=cluster/test-deploy/gcp/ops-mirror.pem \
		--from-file=ci-operator/infra/openshift/release-controller/repos/ocp-4.2-openstack-beta-rhel8.repo \
		-o yaml --dry-run | oc apply -n ocp -f -
	...
	oc create secret generic base-openstack-4-3-repos \
		...
		-o yaml --dry-run | oc apply -n ocp -f -
	oc create secret generic openstack-beta-rhel8-4-3-repos \
		--from-file=cluster/test-deploy/gcp/ops-mirror.pem \
		--from-file=ci-operator/infra/openshift/release-controller/repos/ocp-4.3-openstack-beta-rhel8.repo \
		-o yaml --dry-run | oc apply -n ocp -f -
