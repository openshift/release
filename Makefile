...
ci-operator-prowgen:
	...
	$(CONTAINER_ENGINE) run $(CONTAINER_ENGINE_OPTS) $(CONTAINER_USER) --rm -v "$(CURDIR):/go/src/github.com/openshift/release$(VOLUME_MOUNT_FLAGS)" -e GOPATH=/go registry.ci.openshift.org/ci/ci-operator-prowgen:latest --from-release-repo --to-release-repo $(WHAT)
...
analyse-code:
	@snyk code test --project-name=release --sarif --sarif-file-output=${ARTIFACT_DIR}/snyk.sarif.json > /dev/null || true

	@echo The following vulnerabilities fingerprints are found:
	@jq -r '.runs[].results[].fingerprints[]' ${ARTIFACT_DIR}/snyk.sarif.json | awk 'NR==FNR { b[$$0] = 1; next } !b[$$0]' .snyk-ignore -
