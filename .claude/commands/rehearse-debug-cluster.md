...
Execute all steps in one continuous flow without pausing. Add cucushift-hypershift-extended-debug reference to test config and trigger rehearsal.
...
3. **Edit file**: Add `    - ref: cucushift-hypershift-extended-debug` after the `-chain` line in the `{{config_name}}` test block

4. **Commit & PR**:
   ```
   git add . && git commit -m "debug-cluster-{{config_name}}" && gh auth setup-git && git push -u origin debug-cluster-{{config_name}}
   gh pr create --repo openshift/release --title "debug-cluster-{{config_name}}" --body "Add cucushift-hypershift-extended-debug ref to {{config_name}}"
   ```
