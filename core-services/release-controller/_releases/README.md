...
Our strategy for ensuring backport quality and assessing backports for merge is based on acceptance and pass rates in main.
Every backport is first merged into main, then passes automated testing, then can be backported.
This means that a new job must first become blocking on main before becoming blocking on release
branches in order to be confident that our backports will pass our release branch testing.
If an exception is required due to instability of a job on main, that exception must be accepted
